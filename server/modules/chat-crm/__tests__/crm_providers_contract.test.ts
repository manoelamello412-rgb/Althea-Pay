...
function verifyMetaSignature(payload: string, signature: string, secret: string): boolean {
  try {
    ...
    const actualHex = signature.slice(7)
...
function classifyProviderHttpError(status: number) {
  ...
  return { retryable, action: retryable ? ('RETRY' as const) : ('DLQ' as const) }
}

function verifySvixSignature(payload: string, headers: { id: string; timestamp: string; signatures: string }, secret: string): boolean {
  try {
...
run('Chat CRM — Omnichannel provider contracts', () => {
  ...
  it('Meta: validates HMAC-SHA256 and rejects malformed/asymmetric signatures', () => {
  ...
  it('Twilio: classifies retryable HTTP failures separately from terminal failures', () => {
  ...
  it('Resend: validates canonical Svix v1 signatures', () => {
    ...
    const signature = crypto.createHmac('sha256', Buffer.from(secret.slice(6), 'base64')).update(`${id}.${timestamp}.${payload}`).digest('base64')
  ...
  it('Delivery RPC: preserves READ against a late DELIVERED event', async () => {
  ...
  it('Delivery RPC: makes the same provider event idempotent', async () => {
    const event = JSON.stringify({ provider: 'contract_test', event_id: `duplicate-${outboxId}` })
    const first = await pool.query(`select public.crm_record_channel_delivery_status($1,$2,'read',$3::jsonb) as result`, [accountId, externalMessageId, event])
    const second = await pool.query(`select public.crm_record_channel_delivery_status($1,$2,'read',$3::jsonb) as result`, [accountId, externalMessageId, event])
