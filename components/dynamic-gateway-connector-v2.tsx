...
const TECHNICAL_FIELDS = new Set([
  'base_url', 'auth_header', 'auth_prefix', 'health_path', 'create_path', 'status_path', 'refund_path',
  'request_template', 'response_mapping', 'status_mapping', 'custom_headers', 'idempotency_header',
  'create_method', 'status_method', 'refund_method', 'health_method',
])
const WEBHOOK_FIELDS = new Set(['webhook_secret'])
...
const isGenericProvider = (p: Provider | null) => !!p && (p.provider_key === 'generic_http' || p.adapter_key === 'generic_http_json')
...
export const DynamicGatewayConnectorV2: React.FC = () => {
  ...
  const credentialFields = active?.credential_schema.fields.filter(f => !TECHNICAL_FIELDS.has(f.name) && !WEBHOOK_FIELDS.has(f.name)) ?? []
  const webhookFields = active?.credential_schema.fields.filter(f => WEBHOOK_FIELDS.has(f.name)) ?? []
  const advanced = isGenericProvider(active) ? (active?.credential_schema.fields.filter(f => TECHNICAL_FIELDS.has(f.name)) ?? []) : []
  ...
  return <section className="w-full rounded-xl border border-neutral-800 bg-neutral-950 p-5 md:p-6 space-y-5">
    ...
    <div className="space-y-3"><div className="flex items-center justify-between"><h4 className="text-[10px] font-medium uppercase tracking-wide text-neutral-400">Gateways conectados</h4><span className="text-[9px] font-mono text-neutral-600">{gateways.length} conexão(ões)</span></div>{gateways.length===0?<div className="rounded-lg border border-dashed border-neutral-800 p-4 text-xs text-neutral-500">Nenhum gateway cadastrado. Adicione uma conexão abaixo.</div>:<div className="space-y-2">{gateways.map(g=><div key={g.id} className="flex items-center justify-between gap-3 rounded-lg border border-neutral-800 p-3"><div className="min-w-0"><div className="truncate text-xs font-semibold text-white">{g.display_name||'Gateway sem nome'}</div><div className="mt-1 text-[9px] font-mono uppercase text-neutral-500">{g.provider} · {g.environment} · {statusLabel(g.status)}</div></div><div className="flex shrink-0 items-center gap-1.5"><span className={`h-2 w-2 rounded-full ${['connected','degraded'].includes(g.status.toLowerCase())?'bg-emerald-500':'bg-neutral-600'}`}/><button type="button" onClick={()=>edit(g)} className="rounded-md border border-neutral-700 p-2 text-neutral-300"><Pencil className="h-3.5 w-3.5"/></button><button type="button" onClick={()=>void test(g.id)} disabled={testing!==null} className="rounded-md border border-neutral-700 px-2.5 py-2 text-[9px] font-bold font-mono text-white">{testing===g.id?<Loader2 className="h-3 w-3 animate-spin"/>:'TESTAR'}</button><button type="button" onClick={()=>void toggle(g)} disabled={testing!==null||!g.credential_id} className="rounded-md border border-neutral-700 p-2 text-neutral-300"><Power className="h-3.5 w-3.5"/></button><button type="button" onClick={()=>void disconnect(g)} disabled={testing!==null} className="rounded-md border border-neutral-700 p-2 text-neutral-300"><Unplug className="h-3.5 w-3.5"/></button></div></div>)}</div>}</div>
    {!loading&&<form onSubmit={submit} className="space-y-5 border-t border-neutral-900 pt-5"><div className="flex items-center justify-between"><div><h4 className="text-[10px] font-medium uppercase tracking-wide text-neutral-200">{editing?'Editar conexão':'Nova conexão de gateway'}</h4><p className="mt-1 text-[10px] text-neutral-600">A conexão operacional usa o adapter registrado para o provedor selecionado.</p></div>{editing&&<button type="button" onClick={reset} className="text-neutral-500"><X className="h-4 w-4"/></button>}</div>
      <label className="block space-y-1.5"><span className="text-[10px] font-mono uppercase text-neutral-400">Nome do gateway</span><input value={name} onChange={e=>setName(e.target.value)} placeholder="Ex.: Meu Gateway" className="w-full rounded-lg border border-neutral-800 bg-neutral-900 p-3 text-sm text-white outline-none"/></label>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2"><label className="space-y-1.5"><span className="text-[10px] font-mono uppercase text-neutral-400">Provedor</span><select value={providerKey} onChange={e=>setProviderKey(e.target.value)} disabled={!!editing} className="w-full rounded-lg border border-neutral-800 bg-neutral-900 p-3 text-sm text-white outline-none"><option value="">Selecione o provedor...</option>{providers.map(p=><option key={p.id} value={p.provider_key}>{adapterName(p)}</option>)}</select></label><label className="space-y-1.5"><span className="text-[10px] font-mono uppercase tracking-wide text-neutral-400">Ambiente</span><select value={environment} onChange={e=>setEnvironment(e.target.value as 'sandbox'|'production')} className="w-full rounded-lg border border-neutral-800 bg-neutral-900 p-3 text-sm text-white outline-none"><option value="production">Produção</option><option value="sandbox">Sandbox / Testes</option></select></label></div>
      {active&&credentialFields.length>0&&<div className="space-y-4 rounded-xl border border-neutral-800 bg-neutral-900/40 p-4"><div><h5 className="text-xs font-semibold uppercase tracking-wide text-white">Credenciais</h5><p className="mt-1 text-[10px] text-neutral-500">Os campos abaixo são definidos pelo provedor selecionado.</p></div><div className="grid grid-cols-1 gap-4 md:grid-cols-2">{credentialFields.map(field)}</div></div>}
      {active&&webhookFields.length>0&&<div className="space-y-4 rounded-xl border border-neutral-800 bg-neutral-900/30 p-4"><div><h5 className="text-xs font-semibold uppercase tracking-wide text-white">Webhook</h5><p className="mt-1 text-[10px] text-neutral-500">Chave usada exclusivamente para validar notificações recebidas do provedor.</p></div><div className="grid grid-cols-1 gap-4 md:grid-cols-2">{webhookFields.map(field)}</div></div>}
      {active&&advanced.length>0&&<details className="rounded-xl border border-neutral-800 p-4"><summary className="cursor-pointer text-xs font-semibold uppercase tracking-wide text-neutral-300">Configuração técnica avançada — somente HTTP genérico</summary><p className="mt-2 text-[10px] text-neutral-600">Use esta área apenas quando a ALTHEA estiver conectando uma API sem adapter nativo. Para provedores nativos, endpoints, headers, métodos e mappings são definidos pelo adapter.</p><div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">{advanced.map(field)}</div></details>}
