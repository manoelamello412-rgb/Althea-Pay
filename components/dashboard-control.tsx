...
const chargeback=(s:any)=>['chargeback','lost'].includes(lower(s))
...
function buildCalendar(month:Date){const first=new Date(month.getFullYear(),month.getMonth(),1);const start=new Date(first);start.setDate(first.getDate()-first.getDay());return Array.from({length:42},(_,i)=>{const d=new Date(start);d.setDate(start.getDate()+i);return{value:iso(d),day:d.getDate(),current:d.getMonth()===month.getMonth()}})}
function paymentMethod(s:Sale){const d=obj(s.data);return lower(d.payment_method??d.paymentMethod??d.method??d.payment?.method??d.payment?.payment_method??'outros').replace(/[- ]/g,'_')}
function explicitFee(s:Sale){const d=obj(s.data);const value=d.fee_amount??d.feeAmount??d.fee??d.fees;return value===undefined||value===null||value===''?null:Math.max(0,Number(value)||0)}

export default function DashboardControl(){
  const db=useMemo(()=>createSupabaseBrowserClient() as any,[])
  const today=localISO(),minDate=addDays(today,-89)
  const [userId,setUserId]=useState(''),[range,setRange]=useState<Range>({start:today,end:today}),[draft,setDraft]=useState<Range>({start:today,end:today}),[periodOpen,setPeriodOpen]=useState(false),[calendarOpen,setCalendarOpen]=useState(false),[month,setMonth]=useState(parse(today)),[sales,setSales]=useState<Sale[]>([]),[disputes,setDisputes]=useState<Dispute[]>([]),[gateways,setGateways]=useState<Gateway[]>([]),[health,setHealth]=useState<Health[]>([]),[clientCount,setClientCount]=useState(0),[loading,setLoading]=useState(true),[error,setError]=useState<string|null>(null),[lastSync,setLastSync]=useState<Date|null>(null)
  ...
  const load=useCallback(async()=>{
    ...
    try{
      ...
      if(s.error)throw s.error;if(d.error)throw d.error;if(g.error)throw g.error;if(h.error)throw h.error;if(c.error)throw c.error
      setSales((s.data||[]) as Sale[]);setDisputes((d.data||[]) as Dispute[]);setGateways((g.data||[]) as Gateway[])
      const latest=new Map<string,Health>();(h.data||[]).forEach((x:any)=>{const key=String(x.gateway_id||x.gateway_name||'');if(key&&!latest.has(key))latest.set(key,x)});setHealth([...latest.values()]);setClientCount(c.count||0);setLastSync(new Date())
    }catch(e:any){console.error('[ALTHEA-DASHBOARD]',e);setError('Não foi possível sincronizar o Dashboard.');setSales([]);setDisputes([]);setGateways([]);setHealth([])}finally{setLoading(false)}
  },[db,today])

  useEffect(()=>{void load()},[load])
  useEffect(()=>{if(!db||!userId)return;const channel=db.channel(`dashboard-${userId}`).on('postgres_changes',{event:'*',schema:'public',table:'sales',filter:`user_id=eq.${userId}`},()=>void load()).on('postgres_changes',{event:'*',schema:'public',table:'disputes',filter:`user_id=eq.${userId}`},()=>void load()).on('postgres_changes',{event:'*',schema:'public',table:'gateways',filter:`user_id=eq.${userId}`},()=>void load()).on('postgres_changes',{event:'*',schema:'public',table:'gateway_health_snapshots',filter:`user_id=eq.${userId}`},()=>void load()).subscribe();return()=>{void db.removeChannel(channel)}},[db,userId,load])
  ...
  const stats=useMemo(()=>{const gross=periodSales.filter(s=>approved(s.status)).reduce((n,s)=>n+amountOf(s),0);const feeValues=periodSales.filter(s=>approved(s.status)).map(explicitFee).filter((v):v is number=>v!==null);const fees=feeValues.reduce((n,v)=>n+v,0);const refundAmount=periodSales.filter(s=>refunded(s.status)).reduce((n,s)=>n+amountOf(s),0);const approvedCount=periodSales.filter(s=>approved(s.status)).length;const pendingCount=periodSales.filter(s=>pending(s.status)).length;const failedCount=periodSales.filter(s=>failed(s.status)).length;const chargebackAmount=periodDisputes.filter(d=>chargeback(d.status)).reduce((n,d)=>n+Number(d.amount||0),0);const chargebacks=periodDisputes.filter(d=>chargeback(d.status)).length;return{gross,fees,refundAmount,approvedCount,pendingCount,failedCount,total:periodSales.length,net:gross-fees-refundAmount-chargebackAmount,approval:periodSales.length?approvedCount/periodSales.length*100:0,ticket:approvedCount?gross/approvedCount:0,chargebacks,chargebackAmount,feesAvailable:feeValues.length>0}},[periodSales,periodDisputes])
  const series=useMemo(()=>{const map=new Map<string,number>();periodSales.filter(s=>approved(s.status)).forEach(s=>{const d=saleDate(s).slice(0,10);map.set(d,(map.get(d)||0)+amountOf(s))});const out:{date:string,value:number}[]=[];let cur=range.start;while(cur<=range.end){out.push({date:cur,value:map.get(cur)||0});cur=addDays(cur,1)}return out},[periodSales,range])
  const chart=useMemo(()=>{const max=Math.max(1,...series.map(x=>x.value)),width=300,height=120,padX=8,padY=10;const points=series.map((p,i)=>{const x=series.length===1?width/2:padX+i*(width-padX*2)/(series.length-1);const y=height-padY-(p.value/max)*(height-padY*2);return`${x.toFixed(1)},${y.toFixed(1)}`}).join(' ');return{points,max}},[series])
  const methods=useMemo(()=>{const labels:[string,string][]=[['pix','Pix'],['credit_card','Cartão'],['boleto','Boleto'],['debit_card','Débito'],['outros','Outros']];const total=periodSales.filter(s=>approved(s.status)).reduce((n,s)=>n+amountOf(s),0);return labels.map(([key,label])=>{const value=periodSales.filter(s=>approved(s.status)&&paymentMethod(s)===key).reduce((n,s)=>n+amountOf(s),0);return{label,value,percent:total?value/total*100:0}}).filter(x=>x.value>0)},[periodSales])
  ...
  const donutPercent=methods.length?methods[0].percent:0
  ...
  const calendarDays=buildCalendar(month),monthLabel=month.toLocaleDateString('pt-BR',{month:'long',year:'numeric'}),minMonth=parse(minDate),maxMonth=parse(today)
  const canPrev=month.getFullYear()>minMonth.getFullYear()||month.getMonth()>minMonth.getMonth(),canNext=month.getFullYear()<maxMonth.getFullYear()||month.getMonth()<maxMonth.getMonth()
  ...
  return <section className="althea-mobile-dashboard" aria-label="Dashboard mobile">
    <header className="amd-header"><div className="amd-brand"><img src="/althea-logo.png" alt="Althea Pay"/></div><button className="amd-icon-button" type="button" aria-label="Sincronizar agora" onClick={()=>void load()}><MoreHorizontal size={19}/></button></header>
    ...
    <div className="amd-kpis">{[['Receita Bruta',money(stats.gross),'Volume aprovado'],['Faturamento Líquido',money(stats.net),stats.feesAvailable?'Após taxas, reembolsos e chargebacks':'Taxas reais ainda não registradas'],['Taxas Acumuladas',money(stats.fees),stats.feesAvailable?'Taxas registradas nas vendas':'Nenhuma taxa registrada'],['Taxa de Aprovação',`${stats.approval.toFixed(1)}%`,`${stats.approvedCount} aprovadas de ${stats.total}`],['Ticket Médio',money(stats.ticket),'Vendas aprovadas'],['Reembolsos',money(stats.refundAmount),'Valor reembolsado'],['Chargebacks',String(stats.chargebacks),money(stats.chargebackAmount)],['Pagamentos Pendentes',String(stats.pendingCount),'Aguardando conclusão']].map(([label,value,note])=><article key={label} className={`amd-card amd-kpi ${loading?'amd-loading':''}`}><span>{label}</span>{loading?<div className="amd-skeleton-line"/>:<strong>{value}</strong>}<small>{note}</small></article>)}</div>
