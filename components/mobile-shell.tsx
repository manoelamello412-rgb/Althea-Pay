...
const menuSections: MenuSection[] = [
  ...
  { title: 'INTEGRAÇÕES', items: [
    ...
    { label: 'Webhooks', href: '/dashboard/webhooks', icon: Network },
  ] },
  { title: 'ADMINISTRAÇÃO', items: [
    { label: 'Configurações', href: '/dashboard/settings' },
    { label: 'Membros e acessos', href: '/dashboard/members' },
    { label: 'Segurança', href: '/dashboard/security' },
  ] },
  { title: 'SUPORTE', items: [
    { label: 'Central de ajuda', href: '/dashboard/help' },
  ] },
]
      ...
      <header className="fixed inset-x-0 top-0 z-[100] h-[76px] border-b border-white/[0.06] bg-[rgba(11,11,13,0.97)] px-4 backdrop-blur-xl">
        <div className="mx-auto flex h-full w-full max-w-[1440px] items-center">
          ...
          <div className="ml-auto flex shrink-0 items-center gap-2">
            ...
            <button type="button" aria-label="Abrir menu" aria-expanded={menuOpen} onClick={() => setMenuOpen((value) => !value)} className={`grid h-11 w-11 place-items-center rounded-xl transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[rgba(29,184,84,0.4)] ${menuOpen ? 'bg-[rgba(29,184,84,0.08)] text-[var(--althea-brand)]' : 'text-[var(--althea-muted)] hover:bg-white/[0.03] hover:text-white'}`}>{menuOpen ? <X size={25} strokeWidth={1.55} /> : <Menu size={25} strokeWidth={1.55} />}</button>
          </div>
        </div>
        <AnimatePresence initial={false}>
          {searchOpen && <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 42 }} exit={{ opacity: 0, height: 0 }} className="absolute left-0 right-0 top-[76px] border-b border-white/[0.04] bg-[rgba(11,11,13,0.98)] px-4 py-2 backdrop-blur-xl"><div className="mx-auto flex h-[42px] max-w-[1180px] items-center gap-2 rounded-xl border border-white/[0.04] bg-[var(--althea-surface)] px-3"><Search size={15} className="text-[var(--althea-muted)]" /><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Pesquisar..." aria-label="Pesquisar na plataforma" className="min-w-0 flex-1 bg-transparent text-sm text-white outline-none placeholder:text-[var(--althea-muted)]" /><button type="button" aria-label="Fechar pesquisa" onClick={() => { setQuery(''); setSearchOpen(false) }} className="grid h-9 w-9 place-items-center text-[var(--althea-muted)] hover:text-white"><X size={16} /></button></div></motion.div>}
        </AnimatePresence>
      </header>

      <AnimatePresence>
        {menuOpen && <><motion.button aria-label="Fechar menu" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setMenuOpen(false)} className="fixed inset-0 z-[110] bg-black/60 backdrop-blur-sm" /><motion.aside initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', stiffness: 320, damping: 32 }} className="fixed right-0 top-0 z-[120] flex h-full w-[min(390px,92vw)] flex-col border-l border-white/[0.08] bg-[rgba(8,12,10,0.98)] shadow-[-30px_0_80px_rgba(0,0,0,0.6)]">
          <div className="flex items-center gap-3 border-b border-white/[0.06] px-5 py-5"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-full border border-white/[0.08] bg-white/[0.04] text-sm font-semibold text-white">{user.name.slice(0, 1).toUpperCase()}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-white">{user.name}</p><p className="truncate text-xs text-[var(--althea-muted)]">{user.email}</p></div><button type="button" aria-label="Fechar menu" onClick={() => setMenuOpen(false)} className="grid h-10 w-10 place-items-center rounded-xl text-[var(--althea-muted)] hover:bg-white/[0.04] hover:text-white"><X size={20} /></button></div>
          <div className="flex-1 overflow-y-auto px-4 py-5 pb-8"><p className="mb-5 text-[11px] font-bold tracking-[0.22em] text-[var(--althea-brand)]">ALTHEA PAY</p>{menuSections.map((section) => <section key={section.title} className="mb-5"><h2 className="mb-2 px-2 text-[10px] font-bold tracking-[0.18em] text-[var(--althea-muted)]">{section.title}</h2><div className="space-y-1">{section.items.map((item) => { const Icon = item.icon; const active = tabIsActive(pathname, item.href); return <button key={`${section.title}-${item.label}`} type="button" onClick={() => navigateMenuItem(item)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm transition ${active ? 'bg-[rgba(29,184,84,0.08)] text-[var(--althea-brand)]' : 'text-white/80 hover:bg-white/[0.04] hover:text-white'}`}>{Icon ? <Icon size={17} strokeWidth={1.7} /> : <span className="h-[17px] w-[17px]" />}<span>{item.label}</span></button> })}</div></section>)}</div>
        </motion.aside></>}
      </AnimatePresence>

      <main className="min-w-0 pt-[76px] pb-[128px] lg:pb-10"><div className="mx-auto w-full max-w-[1440px] px-4 py-6 sm:px-5 lg:px-8">{children}</div></main>
      <nav className="fixed inset-x-0 bottom-0 z-[100] px-3 pb-[max(8px,env(safe-area-inset-bottom))] pt-2 lg:hidden" aria-label="Navegação móvel"><div className="mx-auto grid h-[72px] w-full max-w-[760px] grid-cols-5 items-stretch gap-1 rounded-[36px] border border-white/[0.06] bg-[rgba(15,26,22,0.94)] p-1.5 shadow-[0_32px_64px_rgba(0,0,0,0.7)] backdrop-blur-xl">{bottomTabs.map(({ id, label, icon: Icon, href }) => <Nav key={id} id={id} label={label} icon={Icon} active={tabIsActive(pathname, href)} onClick={selectTab} mobile />)}</div></nav>
    </div>
  )
}
