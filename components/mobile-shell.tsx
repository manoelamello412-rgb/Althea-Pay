...
export default function MobileShell({ activeTab, onTabChange, children }: Props) {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      window.dispatchEvent(
        new CustomEvent('althea-global-search', {
  ...
  return (
    <div className="min-h-screen bg-[#020203] text-white antialiased">
          ...
          <div className="flex shrink-0 items-center gap-2">
            <button
              ...
              className={`grid h-9 w-9 place-items-center rounded-full border transition ${
                searchOpen
                  ? 'border-[#1DB854]/50 bg-[#0d1a13] text-[#1DB854]'
                  : 'border-white/[0.06] bg-transparent text-[#a8b0ac] hover:text-white'
              }`}
            >
              <Search size={17} />
            </button>
            ...
            <button
              ...
              className="grid h-9 w-9 place-items-center rounded-full border border-white/[0.06] bg-transparent text-[#a8b0ac] transition hover:border-[#1DB854]/40 hover:bg-[#0d1a13] hover:text-[#1DB854]"
            >
        ...
        <AnimatePresence initial={false}>
          {searchOpen && (
            <motion.div
              ...
              className="mx-auto mt-3 flex max-w-xl items-center gap-2 overflow-hidden rounded-xl border border-white/[0.08] bg-[#0b0d0c] px-3 lg:max-w-none"
            >
              ...
              <input
                ...
                className="min-w-0 flex-1 bg-transparent text-xs text-white outline-none placeholder:text-[#66716c]"
              />
              <button
                type="button"
      ...
      <nav
        className="fixed inset-x-0 bottom-0 z-50 border-t border-white/[0.10] bg-[#080a09]/96 px-3 pb-[max(10px,env(safe-area-inset-bottom))] pt-2 backdrop-blur-2xl lg:left-[250px]"
...
function Nav({
  ...
  return (
    <motion.button
      ...
      onClick={() => onClick(id)}
      whileTap={{ scale: 0.94 }}
      className={`group relative flex items-center justify-center gap-2 rounded-xl transition ${
        mobile ? 'min-h-[58px] flex-col px-1 py-2' : 'w-full justify-start px-3 py-3'
      } ${active ? 'text-[#1DB854]' : 'text-[#7d8782] hover:text-white'}`}
    >
      {active && (
        <span
          className={`absolute ${
            mobile ? 'inset-x-3 bottom-0 h-[2px]' : 'inset-y-2 left-0 w-[2px]'
          } rounded-full bg-[#1DB854]`}
        />
      )}
