...
export function MobileBottomNav() {
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-0 z-[70] px-3 pb-[max(10px,env(safe-area-inset-bottom))] pt-2 sm:px-4">
      <nav aria-label="Navegação principal" className="pointer-events-auto mx-auto flex h-[72px] w-full max-w-xl items-center justify-between gap-1 rounded-[38px] border border-white/[0.12] bg-[#090b0a]/96 px-1.5 shadow-[0_18px_55px_rgba(0,0,0,0.62)] backdrop-blur-2xl">
        {navItems.map((item) => {
          ...
          return (
            <button key={item.id} type="button" onClick={() => selectTab(item.id)} aria-current={isActive ? 'page' : undefined} aria-label={item.label} className={`group relative flex h-[66px] min-w-0 flex-1 flex-col items-center justify-center gap-1 rounded-[32px] px-1.5 py-2 transition-all duration-200 ${isActive ? 'text-[#e9fff0]' : 'text-[#777f7b] hover:text-white'}`}>
              ...
              <span className={`relative z-10 text-[11px] leading-none ${isActive ? 'font-medium' : 'font-medium'}`}>{item.label}</span>
            </button>
          )
        })}
      </nav>
