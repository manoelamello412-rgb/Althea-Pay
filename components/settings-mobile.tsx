...
import SegurancaSettingsPage from '@/app/dashboard/settings/seguranca/page'
import { createSupabaseBrowserClient } from '@/lib/supabase/client'

type SubPage = 'menu' | 'perfil' | 'empresa' | 'integracoes' | 'seguranca'
...
const items = [
  ...
  ['Segurança & Auditoria', ShieldCheck, 'Acesso, segurança e auditoria', 'seguranca'],
] as const
...
export default function SettingsMobile() {
  ...
  useEffect(() => { const handler = () => setCurrentSubPage('menu'); window.addEventListener('althea-settings-back', handler); return () => window.removeEventListener('althea-settings-back', handler) }, [])
  ...
  if (currentSubPage === 'integracoes') return <div className="relative min-h-[500px] space-y-5 pb-32 font-['Space_Grotesk'] text-white"><button type="button" onClick={() => setCurrentSubPage('menu')} className="text-xs font-bold text-zinc-500 hover:text-white">← Voltar</button><IntegracoesSettingsPage /></div>
  if (currentSubPage === 'seguranca') return <div className="relative min-h-[500px] space-y-5 pb-32 font-['Space_Grotesk'] text-white"><SegurancaSettingsPage /></div>
  ...
  return <section className="althea-mobile-settings" aria-label="Configurações mobile"><main className="relative min-h-[500px] space-y-5 pb-32">
    ...
    <section className="space-y-2 pt-1">{items.map(([label, Icon, desc, target])=><button key={label} type="button" onClick={()=>target==='empresa'?setCurrentSubPage('empresa'):target==='integracoes'?setCurrentSubPage('integracoes'):target==='seguranca'?setCurrentSubPage('seguranca'):target==='recuperacao'?window.location.assign('/dashboard/settings/recuperacao'):setNotice(`${label}: área pronta para configuração.`)} className="flex w-full items-center justify-between gap-3 rounded-xl border border-zinc-900/60 bg-[#090F11] p-4 text-left transition-all duration-200 active:scale-[0.99] hover:border-zinc-800"><span className="flex min-w-0 items-center gap-3"><span className="amsg-icon"><Icon size={22}/></span><span><strong className="block text-sm font-bold text-zinc-200">{label}</strong><small className="mt-0.5 block text-[11px] text-zinc-500">{desc}</small></span></span><ChevronRight size={20} className="shrink-0 text-zinc-600" /></button>)}</section>
