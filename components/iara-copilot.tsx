...
const WELCOME: Message = { id: 'iara-welcome', sender: 'iara', created_at: new Date(0).toISOString(), content: 'Olá! Eu sou a Iara, sua copiloto da Althea Pay. Posso te ajudar com a operação, vendas, gateways, checkout, funis e configurações disponíveis no painel. O que você precisa?' }
...
export default function IaraCopilot() {
  ...
  const createSession = useCallback(async () => {
    ...
    setActiveId(session.id); setMessages([]); setError(null)
  }, [db])
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setSessions(rows)
      if (rows[0]) setActiveId(rows[0].id); else await createSession()
      setLoading(false)
    }
  ...
  useEffect(() => {
    ...
    const channel = db.channel(`iara-chat-${activeId}`).on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'chat_messages', filter: `session_id=eq.${activeId}` }, (payload) => {
      ...
      setMessages((current) => current.some((item) => item.id === fresh.id) ? current : [...current, fresh]); scroll()
    }).subscribe()
  ...
  const send = async () => {
    ...
    try {
      ...
      scroll()
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Falha ao enviar mensagem.'); setInput(text) }
  ...
  return <section className="flex min-h-[calc(100dvh-5rem)] w-full flex-col overflow-hidden bg-[#060608] pb-32 text-white">
    ...
    <div className="grid min-h-0 flex-1 md:grid-cols-[260px_minmax(0,1fr)]">
      <aside className="hidden min-h-0 overflow-y-auto border-r border-[#191921] bg-[#0b0b0f] p-2 md:block"><div className="px-3 py-3 text-[10px] font-bold uppercase tracking-[.15em] text-zinc-600">Histórico</div>{sessions.map((session) => <button key={session.id} type="button" onClick={() => setActiveId(session.id)} className={`mb-1 flex min-h-11 w-full items-center rounded-xl px-3 text-left text-xs ${activeId === session.id ? 'border border-[#191921] bg-[#121217] text-white' : 'text-zinc-500 hover:bg-[#121217] hover:text-zinc-200'}`}>{session.title}</button>)}</aside>
      <div className="flex min-h-0 min-w-0 flex-col">
        ...
        <div className="shrink-0 border-t border-[#191921] bg-[#060608] px-3 py-3 sm:px-6"><form onSubmit={(event) => { event.preventDefault(); void send() }} className="mx-auto max-w-3xl">
