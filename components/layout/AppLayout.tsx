...
import { LayoutGrid, MessageSquare, Network, Settings, Sparkles } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen bg-[var(--althea-bg)] text-white antialiased selection:bg-[var(--althea-brand)] selection:text-black md:flex">
      ...
      <div className="min-w-0 flex-1">
        <header className="sticky top-0 z-50 flex h-14 items-center justify-between border-b border-white/[0.04] bg-[var(--althea-bg)]/90 px-4 backdrop-blur-xl md:hidden"><Link href="/dashboard" aria-label="Voltar ao Dashboard" className="flex min-w-0 items-center gap-2"><BrandLogo variant="inner" priority alt="ALTHEA PAY" className="h-6 w-auto max-w-[132px] object-contain" /></Link><span className="truncate pl-3 font-mono text-[10px] uppercase tracking-wider text-[var(--althea-muted)]">{activeRoute.label}</span></header>
        <main className="min-w-0 px-4 py-6 pb-32 md:px-8 md:py-8 md:pb-10"><div className="mx-auto w-full max-w-[1280px]">{children}</div></main>
      </div>
      <div className="pointer-events-none fixed inset-x-0 bottom-0 z-50 bg-gradient-to-t from-[var(--althea-bg)] via-[var(--althea-bg)]/90 to-transparent px-4 pb-[max(12px,env(safe-area-inset-bottom))] pt-4 md:hidden"><nav aria-label="Navegação móvel" className="pointer-events-auto mx-auto grid h-16 w-full max-w-md grid-cols-5 items-center gap-1 rounded-2xl border border-white/[0.06] bg-[rgba(13,54,45,0.72)] p-1.5 shadow-[0_32px_64px_rgba(0,0,0,0.7)] backdrop-blur-xl">{routes.map((route) => { const Icon = route.icon; const active = isRouteActive(pathname, route); return <Link key={route.id} href={route.href} aria-current={active ? 'page' : undefined} className={`flex h-full min-w-0 flex-col items-center justify-center gap-1 rounded-xl transition-all ${active ? 'bg-[rgba(29,187,84,0.08)] text-[var(--althea-brand)] shadow-[inset_0_0_0_1px_rgba(29,187,84,0.2)]' : 'text-zinc-500 hover:text-zinc-300'}`}><Icon className="h-4 w-4 shrink-0" /><span className="max-w-full truncate px-1 text-[9px] font-medium leading-none">{route.label}</span></Link> })}</nav></div>
    </div>
  )
}
