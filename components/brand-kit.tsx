...
export function hydrateAltheaBrand() {
  try {
    ...
    const next = stored ? { ...DEFAULT_ALTHEA_BRAND, ...JSON.parse(stored) } : DEFAULT_ALTHEA_BRAND
    applyBrand(next)
  } catch {
...
export function BrandKit() {
  ...
  useEffect(() => { hydrateAltheaBrand(); try { const stored = localStorage.getItem(STORAGE_KEY); if (stored) setBrand({ ...DEFAULT_ALTHEA_BRAND, ...JSON.parse(stored) }) } catch {} }, [])

  function update(key: keyof Brand, value: string) {
    setSaved(false)
    setBrand(current => { const next = { ...current, [key]: value }; applyBrand(next); return next })
  }
  ...
  return (
    <div className="brand-kit">
      <div className="brand-kit-hero">
        <div><span className="brand-kit-kicker">BRAND KIT · ALTHEA PAY</span><h2>Marca e Identidade</h2><p>Controle a identidade visual da Althea Pay e aplique a linguagem da marca de forma consistente nos componentes digitais.</p></div>
      ...
      <div className="brand-kit-actions"><button className="brand-reset" onClick={reset}>Restaurar padrão</button><button className="primary" onClick={save}>{saved ? 'Identidade salva ✓' : 'Salvar identidade'}</button></div>
    </div>
  )
}
