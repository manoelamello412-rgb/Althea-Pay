...
async function runAction(rule: any, c: any) {
  ...
  if (type === "update_transaction") {
    ...
    let data: any = null;
    if (cfg.status) {
      const { data: transition, error } = await db.rpc("transition_gateway_transaction_status", { p_transaction_id: c.transaction_id, p_user_id: c.user_id, p_next_status: String(cfg.status), p_failure_code: cfg.error_message ? String(cfg.error_message) : null, p_external_id: null });
      if (error) throw error;
      data = transition;
    }
    if (cfg.error_message) {
      const { data: updated, error } = await db.from("gateway_transactions").update({ error_message: String(cfg.error_message) }).eq("id", c.transaction_id).eq("user_id", c.user_id).select("id,status,error_message").single();
      if (error) throw error;
      data = updated;
    }
