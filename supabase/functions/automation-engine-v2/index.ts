...
async function stableKey(c: any, ruleId: string) {
  ...
  if (explicit) return `${String(explicit)}:${ruleId}`;
  const canonical = JSON.stringify({ event_type: c.event_type, funnel_id: c.funnel_id, conversation_id: c.conversation_id, payload: c.payload });
