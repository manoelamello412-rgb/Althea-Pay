...
Deno.serve(withSupabase({ auth: 'none' }, async (req, ctx) => {
  ...
  try {
    ...
    if (transactionId) {
      ...
      transaction = r.data
      if (transaction && nextState && canTransitionFinancialState(transaction.status, nextState)) {
    ...
    if (purchase) {
      ...
      const wr = await fetch(webhookUrl, { method: 'POST', headers: { 'content-type': 'application/json', 'x-internal-secret': secret }, body: JSON.stringify({ user_id: userId, event_id: event.id, event_type: 'order.approved', event_db_id: event.id, payload: { ...p, sale_id: saleId, transaction_id: transactionId, funnel_id: funnelId } }) })
