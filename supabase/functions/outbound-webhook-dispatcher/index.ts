...
async function sign(secret:string,body:string){const key=await crypto.subtle.importKey("raw",enc.encode(secret),{name:"HMAC",hash:"SHA-256"},false,["sign"]);const sig=await crypto.subtle.sign("HMAC",key,enc.encode(body));return Array.from(new Uint8Array(sig)).map(v=>v.toString(16).padStart(2,"0")).join("")}
...
Deno.serve(async req=>{
 ...
 for(const wh of webhooks??[]){if(!Array.isArray(wh.events)||(!wh.events.includes(eventType)&&!wh.events.includes("*")))continue;const idempotencyKey=`${eventId}:${wh.id}:${eventType}`;const secret=String(wh.secret_ref??"");let signature="";if(secret)signature=await sign(secret,body);
  ...
  const guard=await assertPublicHttpsUrl(String(wh.endpoint_url));
  if(!guard.ok){status="failed";errorMessage=`blocked_${guard.reason}`;await db.from("outbound_webhook_deliveries").upsert({webhook_id:wh.id,user_id:userId,event_id:b.event_db_id??null,event_type:eventType,idempotency_key:idempotencyKey,status,attempt:1,response_code:null,response_time_ms:Date.now()-started,error_message:errorMessage,payload,delivered_at:null,next_retry_at:null},{onConflict:"webhook_id,idempotency_key"});results.push({webhook_id:wh.id,status,response_code:null,error:errorMessage});continue}
  try{const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),10000);const r=await fetch(guard.url,{method:"POST",redirect:"error",signal:controller.signal,headers:{"content-type":"application/json","x-althea-event":eventType,"x-althea-event-id":eventId,"x-althea-idempotency-key":idempotencyKey,...(signature?{"x-althea-signature":signature}:{})},body});clearTimeout(timeout);code=r.status;if(r.ok)status="delivered";else errorMessage=`http_${r.status}`}catch(e){errorMessage=e instanceof Error?e.message:"delivery_failed"}
