...
Deno.serve(async req=>{if(req.method==='OPTIONS')return new Response(null,{status:204,headers:CORS}); if(req.method!=='POST')return out({error:'method_not_allowed'},405); if(!URL||!KEY)return out({error:'server_not_configured'},500);
 ...
 const p=payload as Record<string,unknown>; const sessionKey=typeof body?.session_key==='string'?body.session_key.trim():(typeof p?.session_key==='string'?String(p.session_key).trim():null);
 if(sessionKey){await db.rpc('project_attribution_event',{p_user_id:uid,p_funnel_id:funnelId,p_session_key:sessionKey,p_event_type:type,p_payload:payload}).catch(()=>{});}
