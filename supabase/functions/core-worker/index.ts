...
Deno.serve(withSupabase({ auth: 'none' }, async (req, ctx) => {
  ...
  const limit = Math.min(Math.max(Number((await req.clone().json().catch(() => ({})))?.limit || 10), 1), 50)
  ...
  for (const job of jobs) {
    try {
      if (job.job_type === 'integration_event_retry') {
        ...
        const url = `${Deno.env.get('SUPABASE_URL')}/functions/v1/integration-event-processor`
