...
export default class StripeAdapter implements GatewayAdapter {
  ...
  private toGatewayResponse(pi: Stripe.PaymentIntent): GatewayResponse {
  ...
  async authorize(params: GatewayAuthParams): Promise<GatewayResponse> {
    ...
    if (!this.stripe) return { id: 'stripe_unconfigured', status: 'error', amount: params.amount, currency: params.currency.toUpperCase(), failureClass: 'unavailable', failureCode: 'provider_not_configured' }
    const amountInCents = Math.round(params.amount)
    const pi = await this.stripe.paymentIntents.create({ amount: amountInCents, currency: params.currency.toLowerCase(), capture_method: 'manual', metadata: params.metadata || {} })
  ...
  async capture(authorizationId: string, amount?: number): Promise<GatewayResponse> {
    ...
    if (!this.stripe) return { id: authorizationId, status: 'error', amount: amount || 0, currency: 'USD', failureClass: 'unavailable', failureCode: 'provider_not_configured' }
    const opts: Stripe.PaymentIntentCaptureParams = typeof amount === 'number' ? { amount_to_capture: Math.round(amount) } : {}
  ...
  async refund(transactionId: string, amount?: number): Promise<GatewayResponse> {
    ...
    if (!this.stripe) return { id: transactionId, status: 'error', amount: amount || 0, currency: 'USD', failureClass: 'unavailable', failureCode: 'provider_not_configured' }
    const refundParams: Stripe.RefundCreateParams = { payment_intent: transactionId }
    if (typeof amount === 'number') refundParams.amount = Math.round(amount)
    const refund = await this.stripe.refunds.create(refundParams)
    return normalizeGatewayResponse({ id: refund.id, status: refund.status === 'succeeded' ? 'approved' : 'pending', amount: refund.amount || amount || 0, currency: (refund.currency || 'usd').toUpperCase(), raw: refund })
  }

  async void(authorizationId: string): Promise<GatewayResponse> {
    logJSON('info', 'stripe.void', { authorizationId })
    if (!this.stripe) return { id: authorizationId, status: 'error', amount: 0, currency: 'USD', failureClass: 'unavailable', failureCode: 'provider_not_configured' }
  ...
  async healthCheck(): Promise<{ ok: boolean; details?: unknown }> {
    ...
    try {
      ...
      return { ok: false, details: { configured: true, error: String(error) } }
    }
  }
  ...
  async handleWebhook(rawBody: unknown, headers: Record<string, string>): Promise<{ ok: boolean; event?: unknown }> {
    ...
    try {
      ...
      logJSON('warn', 'stripe.webhook.invalid_signature', { error: String(error) })
      return { ok: false }
    }
  }
}
