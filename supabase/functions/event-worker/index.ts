  ...
  try {
    ...
    for (const event of events ?? []) {
      ...
      try {
        ...
        await db.from("integration_events").update({ status: terminal ? "dead_letter" : "failed", retry_count: retryCount, error_message: message, next_retry_at: nextRetryAt, claimed_at: null }).eq("id", event.id).eq("status", "processing");
