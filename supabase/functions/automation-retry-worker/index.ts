  ...
  for(const row of rows){
   try{
    ...
    if(allowed===false){
      ...
      const {error:updateError}=await db.from("automation_executions").update({status:"failed",error_message:"automation_rate_limited",next_retry_at:nextRetry,completed_at:new Date().toISOString()}).eq("id",row.id).eq("status","running");
      if(updateError)throw updateError;
      results.push({execution_id:row.id,status:"failed",reason:"rate_limited",kind:row.__kind,next_retry_at:nextRetry});continue;
    }
   ...
   catch(e){const message=e instanceof Error?e.message:String(e);const terminal=Number(row.attempt_count)>=Number(row.max_attempts);if(terminal)await db.rpc("crm_mark_automation_dead_letter",{p_execution_id:row.id,p_error:message});else {const nextRetry=new Date(Date.now()+automationBackoffMs(Number(row.attempt_count||1))).toISOString();const{error:updateError}=await db.from("automation_executions").update({status:"failed",error_message:message,next_retry_at:nextRetry,completed_at:new Date().toISOString()}).eq("id",row.id).eq("status","running");if(updateError)throw updateError;}results.push({execution_id:row.id,status:terminal?"dead_letter":"failed",error:message,kind:row.__kind});}
  }
