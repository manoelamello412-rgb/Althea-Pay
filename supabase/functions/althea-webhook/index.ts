...
Deno.serve(
  withSupabase({ auth: 'none' }, async (req, ctx) => {
    ...
    try {
      ...
      const tenantResult = await db.from('webhook_integrations').select('organization_id').eq('id', integration.id).maybeSingle()
      if (tenantResult.error) throw tenantResult.error
      const organizationId = tenantResult.data?.organization_id ? String(tenantResult.data.organization_id) : ''
      if (!organizationId) return Response.json({ ok: false, error: 'webhook_organization_required' }, { status: 400, headers: corsHeaders })
