...
Deno.serve(
  withSupabase({ auth: 'none' }, async (req, ctx) => {
    ...
    try {
      if (!/^\d+$/.test(timestamp)) {
        return Response.json({ ok: false, error: 'invalid_timestamp' }, { status: 400, headers: corsHeaders })
      }
      if (Math.abs(Date.now() - Number(timestamp)) > 300000) {
        return Response.json({ ok: false, error: 'stale_webhook' }, { status: 401, headers: corsHeaders })
      }
      if (!eventId) {
        return Response.json({ ok: false, error: 'event_id_required' }, { status: 400, headers: corsHeaders })
      }
      ...
      if (key) {
        const result = await db
          ...
          .select('id,user_id,funnel_id,provider,status,secret,vault_secret_id')
          .eq('endpoint_key', key)
          .eq('status', 'active')
          .maybeSingle()
        ...
        if (!integration) {
          return Response.json({ ok: false, error: 'webhook_integration_not_found' }, { status: 404, headers: corsHeaders })
        }
      }

      const secret = integration?.secret || Deno.env.get('ALTHEA_WEBHOOK_SECRET') || ''
      if (!secret) {
        return Response.json({ ok: false, error: 'webhook_secret_not_configured' }, { status: 503, headers: corsHeaders })
      }
      ...
      if (!eventType || !userId || !funnelId) {
        return Response.json({ ok: false, error: 'event_user_and_funnel_required' }, { status: 400, headers: corsHeaders })
      }
      ...
      if (existing.data) {
        ...
        return Response.json(
          { ok: true, duplicate: true, event_id: existing.data.id, status: existing.data.status },
          { headers: corsHeaders },
        )
      }
      ...
      const purchase =
        ...
        (eventType.toLowerCase().includes('payment') ||
          eventType.toLowerCase().includes('purchase') ||
          SUCCESS.includes(eventType.toLowerCase()))
      ...
      if (reversal) {
        ...
        const updated = await db
          .from('sales')
          .update({
            status: status === 'chargeback' || norm(eventType) === 'chargeback' ? 'chargeback' : 'refunded',
            data: payload,
          })
          .eq('user_id', userId)
          .eq('external_id', saleExternalId)
        ...
        if (purchase) {
          ...
          const webhookResponse = await fetch(webhookUrl, {
            ...
            body: JSON.stringify({
              ...
              event_db_id: event.data.id,
              payload: { ...payload, sale_id: saleId, transaction_id: transactionId, funnel_id: funnelId },
            }),
          })
      ...
      return Response.json(
        {
          ...
          processed: true,
          sale_id: saleId,
          sale_synced: purchase || reversal,
          automation_triggered: automationTriggered,
          universal_webhook_triggered: universalWebhookTriggered,
        },
        { headers: corsHeaders },
      )
    } catch (error) {
      ...
      if (eventIdDb) {
        ...
        await db
          ...
          .update({
            status: 'retry',
            retry_count: Number(existingEvent.data?.retry_count ?? 0) + 1,
      ...
      return Response.json(
        { ok: false, error: message, event_id: eventIdDb, retryable: Boolean(eventIdDb) },
