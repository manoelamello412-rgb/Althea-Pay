...
begin
 ...
 insert into public.automation_execution_attempts(user_id,execution_id,attempt_no,status,error_message,finished_at)
 values(new.user_id,new.id,n,new.status,new.error_message,case when new.status in ('completed','failed','dead_letter') then coalesce(new.completed_at,now()) else null end)
