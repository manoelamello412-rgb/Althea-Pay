    ...
    metrics as (
      select
        ...
        (select count(*)::bigint
          ...
          where a.organization_id=v_org and a.created_at>=v_start) as api_requests,
        (select count(*)::bigint
           from public.api_request_logs a
          where a.user_id=v_uid and a.created_at>=v_start
        ...
        (select coalesce(round(percentile_cont(0.95) within group(order by a.latency_ms))::numeric,0)
           from public.api_request_logs a
          where a.user_id=v_uid and a.created_at>=v_start and a.latency_ms is not null) as api_p95_ms,
    ...
    queues as (
      select jsonb_build_array(
        ...
        jsonb_build_object(
          ...
          'count',(select count(*) from public.automation_executions a where a.user_id=v_uid and (a.status in ('failed','retry_scheduled','scheduled','running') or (a.next_retry_at is not null and a.completed_at is null))),
          'href','/dashboard/crm'
        ),
        jsonb_build_object(
    ...
    incidents as (
      select * from (
        ...
        select 2,'warning','automation','automation_failure',
          'Automação falhou',a.status,a.created_at,'/dashboard/crm'
        from public.automation_executions a
        where a.user_id=v_uid and a.created_at>=v_start
        ...
        from public.api_request_logs a
        where a.user_id=v_uid and a.created_at>=v_start and coalesce(a.status_code,0)>=500
      ) x
    )
