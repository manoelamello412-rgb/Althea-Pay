...
WITH eligible AS (
  ...
  SELECT e.gateway_id,
         COALESCE(public.gateway_effective_cost_bps(p_user_id,e.gateway_id,p_card_brand,p_amount*100),
...
SET search_path = public
AS $$
  SELECT * FROM public.rank_gateway_candidates(
    p_user_id,p_gateway_ids,p_amount,p_currency,p_environment,NULL::text
  );
$$;
...
BEGIN
  ...
  RETURN QUERY SELECT * FROM public.rank_gateway_candidates(
    p_user_id,p_gateway_ids,p_amount,upper(p_currency),lower(p_environment),p_card_brand
  );
END;
$$;
