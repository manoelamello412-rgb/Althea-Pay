...
BEGIN
  ...
  SELECT t.user_id, t.funnel_id
    INTO v_tx_user, v_tx_funnel
  ...
  IF NOT EXISTS (
    ...
    SELECT
      ...
      NEW.transaction_id,
      NEW.gateway_id,
      COALESCE(g.provider, NEW.gateway_name),
      NEW.idempotency_key,
      NEW.failure_class,
      ...
      0,
      now(),
      LEFT(COALESCE(NEW.error_message, 'ambiguous_gateway_attempt'), 2000)
