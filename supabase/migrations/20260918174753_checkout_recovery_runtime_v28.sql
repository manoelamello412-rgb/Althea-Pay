...
create index if not exists recovery_events_org_status_next_idx
  on public.recovery_events(organization_id,status,next_attempt_at,created_at)
  where organization_id is not null;

create index if not exists recovery_events_outbox_idx
  on public.recovery_events(outbox_id)
...
create or replace function public.update_checkout_recovery_worker_v1(
  p_event_id uuid,
  p_status text,
...
create trigger crm_outbox_checkout_recovery_sync_trg
after insert or update of status,sent_at,failed_at,last_error
on public.crm_channel_message_outbox
for each row execute function public.sync_checkout_recovery_from_outbox_v1();
...
create trigger checkout_recovery_recovered_trg
before update of status,completed_at on public.checkout_sessions
for each row execute function public.mark_checkout_recovery_recovered_v1();
...
revoke all on function public.enqueue_checkout_recovery_events(integer) from public,anon,authenticated;
revoke all on function public.claim_checkout_recovery_events_v1(integer) from public,anon,authenticated;
revoke all on function public.update_checkout_recovery_worker_v1(uuid,text,text,uuid,uuid,text,integer) from public,anon,authenticated;
revoke all on function public.recovery_operations_v1(integer) from public,anon;
revoke all on function public.recovery_operator_requeue_checkout_v1(uuid) from public,anon;

grant execute on function public.enqueue_checkout_recovery_events(integer) to service_role;
grant execute on function public.claim_checkout_recovery_events_v1(integer) to service_role;
grant execute on function public.update_checkout_recovery_worker_v1(uuid,text,text,uuid,uuid,text,integer) to service_role;
grant execute on function public.recovery_operations_v1(integer) to authenticated;
grant execute on function public.recovery_operator_requeue_checkout_v1(uuid) to authenticated;
