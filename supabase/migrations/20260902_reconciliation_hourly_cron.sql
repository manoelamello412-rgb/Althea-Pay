...
begin
  ...
  if internal_secret is null or internal_secret = '' then
    raise exception 'ALTHEA internal secret is not configured';
  end if;
  select extensions.http_post(
    url := 'https://hkraryqoziravulvqkid.supabase.co/functions/v1/reconciliation-worker',
...
begin
  select jobid into job_id from cron.job where jobname='althea-reconciliation-hourly' limit 1;
  if job_id is not null then perform cron.unschedule(job_id); end if;
  perform cron.schedule('althea-reconciliation-hourly','0 * * * *','select public.trigger_reconciliation_worker();');
end $$;
