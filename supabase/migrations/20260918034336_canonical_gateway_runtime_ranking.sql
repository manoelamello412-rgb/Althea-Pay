-- Forward repair: recover the ranking body from the recorded migration
-- 20260911040252 (gateway_routing_cost_unit_alignment_v1).
-- Keep one algorithm in the canonical runtime RPC; preserve the old five-argument
-- contract only as a delegating overload. No historical migration is changed.
begin;
create or replace function public.gateway_runtime_route_candidates(
 p_user_id uuid, p_gateway_ids text[], p_amount numeric,
 p_currency text, p_environment text, p_card_brand text default null
...
eligible as (
 ...
 from public.gateways g left join lateral (select hs.is_healthy,hs.latency_ms,hs.circuit_state from public.gateway_health_snapshots hs where hs.gateway_id=g.circuit_id and hs.user_id=p_user_id order by hs.checked_at desc limit 1) h on true
...
create or replace function public.gateway_runtime_route_candidates(
 p_user_id uuid, p_gateway_ids text[], p_amount numeric,
 p_currency text, p_environment text
) returns table(gateway_id text,routing_score numeric,approval_rate numeric,latency_ms integer,healthy boolean,circuit_state text,cost_bps numeric)
language sql security definer set search_path to 'pg_catalog','public'
as $function$
 select * from public.gateway_runtime_route_candidates(p_user_id,p_gateway_ids,p_amount,p_currency,p_environment,null::text);
$function$;
...
revoke all on function public.gateway_runtime_route_candidates(uuid,text[],numeric,text,text) from public,anon,authenticated;
