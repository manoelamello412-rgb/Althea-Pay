CREATE OR REPLACE FUNCTION public.create_gateway_payment_instrument(
  p_customer_ref text,
  p_instrument_fingerprint text DEFAULT NULL,
...
BEGIN
  ...
  INSERT INTO public.gateway_payment_instruments(user_id,customer_ref,instrument_fingerprint,brand,last4)
  VALUES(v_user,trim(p_customer_ref),NULLIF(trim(p_instrument_fingerprint),''),NULLIF(lower(trim(p_brand)),''),NULLIF(trim(p_last4),''))
