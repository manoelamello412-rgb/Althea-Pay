...
variants as (
 ...
 count(distinct x.subject_key) as exposures,
 count(distinct case when o.outcome='converted' then o.subject_key end) as conversions,
 coalesce(sum(case when o.outcome='revenue' then o.value else 0 end),0) as revenue
...
scored as (
 select v.*,
 case when exposures>0 then round(conversions::numeric/exposures,6) else 0 end as conversion_rate
 from variants v
),
...
annotated as (
 select s.*, case when (select exposures from best)>0 and s.id<>(select id from best) then round(((s.conversion_rate-(select conversion_rate from best))*100),2) else 0 end as uplift_vs_best_pp,
 case when s.id=(select id from best) then true else false end as current_leader
 from scored s
)
