...
begin
  ...
  if supplied_customer_id is not null then
    select cl.id into canonical_customer_id
      from public.clients cl
     where cl.user_id=new.user_id and cl.id=supplied_customer_id
     limit 1;
  end if;
  ...
  if supplied_token is not null then
    select id into conv from public.crm_conversations
     where user_id=new.user_id and funnel_id=new.funnel_id and public_token=supplied_token
     for update;
    ...
    if canonical_customer_id is not null then
      select id into conv from public.crm_conversations
       where user_id=new.user_id and funnel_id=new.funnel_id
         and customer_id=canonical_customer_id and status in ('open','pending')
       order by updated_at desc limit 1 for update;
    end if;
    if conv is null and canonical_customer_id is null and normalized_email is not null then
      select id into conv from public.crm_conversations
       where user_id=new.user_id and funnel_id=new.funnel_id
         and status in ('open','pending') and lower(buyer_email)=normalized_email
       order by updated_at desc limit 1 for update;
    end if;
    if conv is null then
      begin
        ...
        values(new.user_id,new.funnel_id,nullif(p->>'product_id',''),nullif(p->>'transaction_id',''),
          nullif(coalesce(c->>'name',p->>'name',p->>'buyer_name'),''),normalized_email,'open',p,canonical_customer_id)
      ...
      update public.crm_conversations
         set metadata=p, customer_id=coalesce(customer_id,canonical_customer_id), updated_at=now()
       where id=conv;
    end if;
  end if;
  ...
  if new.event_type='chat_message' then
    ...
    values(conv,new.user_id,'inbound','funnel_chat',coalesce(nullif(p->>'message',''),nullif(p->>'body',''),'[mensagem sem conteúdo]'),
      jsonb_build_object('integration_event_id',new.id,'funnel_id',new.funnel_id,'external_id',new.external_id),
      'althea_funnel',nullif(new.external_id,''))
...
begin
 ...
 with base as (
   select c.*, cl.id as canonical_customer_id, cl.data as canonical_customer_data,
          lower(nullif(trim(coalesce(cl.data->>'email',cl.data->>'buyer_email',cl.data->>'customer_email',c.buyer_email)),'')) as email_key
   from public.crm_conversations c left join public.clients cl on cl.user_id=c.user_id and cl.id=c.customer_id
   where c.id=p_conversation_id and c.user_id=v_user
 ),
 related_conversations as (
   select c.* from public.crm_conversations c,base b
   where c.user_id=v_user and ((b.canonical_customer_id is not null and c.customer_id=b.canonical_customer_id)
      or (b.canonical_customer_id is null and b.email_key is not null and lower(c.buyer_email)=b.email_key))
 ),
 related_sales as (
   select distinct s.* from public.sales s,base b
   where s.user_id=v_user and ((b.canonical_customer_id is not null and coalesce(s.data->>'customer_id','')=b.canonical_customer_id)
      or (b.canonical_customer_id is null and b.email_key is not null and lower(coalesce(s.data->>'email',s.data->>'buyer_email',s.data->>'customer_email',''))=b.email_key)
      or (b.transaction_id is not null and s.transaction_id::text=b.transaction_id))
 ),
 related_checkouts as (
   select cs.* from public.checkout_sessions cs,base b where cs.user_id=v_user and b.email_key is not null and lower(coalesce(cs.customer->>'email',cs.customer->>'buyer_email',''))=b.email_key
 ),
 ...
 related_events as (
   ...
   where e.user_id=v_user and ((b.transaction_id is not null and e.transaction_id=b.transaction_id)
      or (b.canonical_customer_id is null and b.email_key is not null and lower(e.buyer_email)=b.email_key))
 ),
