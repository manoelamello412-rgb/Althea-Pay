...
begin
  ...
  insert into public.audit_logs(user_id,actor_id,organization_id,action,resource_type,resource_id,metadata) values(auth.uid(),auth.uid(),v_org,'product.updated','product',v_product.id,jsonb_build_object('version',v_product.version));
  return v_product;
exception when unique_violation then raise exception 'PRODUCT_SLUG_OR_SKU_ALREADY_EXISTS' using errcode='23505'; end $$;
