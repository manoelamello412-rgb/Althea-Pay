...
create index if not exists idx_transaction_audit_idempotency on public.transaction_audit_events(organization_id, idempotency_key) where idempotency_key is not null;
...
create or replace function public.reserve_transaction_audit_event(
...
returns table(reserved boolean, audit_event_id uuid)
...
begin
  ...
  if v_id is not null then return query select true,v_id; else return query select false,id from public.transaction_audit_events where organization_id=p_organization_id and idempotency_key=p_idempotency_key limit 1; end if;
end;
$$;
