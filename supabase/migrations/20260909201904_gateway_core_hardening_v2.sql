...
begin
  ...
  select g.credential_id into v_credential_id from public.gateways g where g.id = p_gateway_id and g.credential_id is not null limit 1;
  if v_credential_id is null then raise exception 'gateway_credential_not_bound'; end if;
...
create policy gateway_circuit_states_select on public.gateway_circuit_states for select to authenticated using (user_id = (select auth.uid()));

create index if not exists gateway_webhook_events_provider_event_idx on public.gateway_webhook_events(provider, provider_event_id);

comment on column public.gateways.credential_id is 'Server-side binding to user_gateway_credentials. Credential plaintext is never stored on gateways.';
comment on column public.gateways.provider is 'Canonical provider key used by the Gateway Adapter Registry.';
comment on column public.gateways.environment is 'Explicit runtime environment. Sandbox and production are never inferred from adapter behavior.';