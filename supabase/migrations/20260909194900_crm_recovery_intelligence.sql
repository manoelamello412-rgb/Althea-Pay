...
WITH raw AS (
  ...
  SELECT
    b.*,
    CASE
      WHEN b.event_status IN ('failed','declined') THEN 60
      WHEN b.event_status IN ('pending','waiting') THEN 50
      ELSE 40
    END
...
SELECT
  ...
  END,
  CASE
    WHEN s.event_status IN ('failed','declined') THEN 'Recuperar pagamento e oferecer alternativa de pagamento.'
    WHEN s.event_status IN ('pending','waiting') THEN 'Acompanhar pendência e conduzir o cliente à conclusão.'
    ELSE 'Retomar a jornada e devolver o cliente ao checkout.'
  END
FROM scored s
WHERE s.processed_at IS NULL
ORDER BY s.priority DESC, s.received_at DESC;
$$;
