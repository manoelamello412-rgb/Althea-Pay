...
begin
  if new.gateway_id is null or btrim(new.gateway_id)='' then
    raise exception 'gateway_attempt_gateway_id_required';
  end if;
  if not exists (select 1 from public.gateways g where g.id=new.gateway_id and g.user_id=new.user_id) then
    raise exception 'gateway_attempt_gateway_tenant_mismatch';
  end if;
  return new;
end;
$$;
...
create trigger trg_gateway_attempt_gateway_tenant_integrity
before insert or update of gateway_id,user_id on public.gateway_payment_attempts
for each row execute function public.gateway_attempt_gateway_tenant_integrity();
...
as $$
with eligible as (
  select g.id::text gateway_id,g.capabilities,g.environment,
  ...
  left join lateral (
    ...
    order by hs.checked_at desc limit 1
  ) h on true
  ...
  select e.gateway_id,
         count(*) filter(where a.status in ('approved','declined','pending','error'))::numeric total,
         count(*) filter(where a.status='approved')::numeric approved
  from eligible e
  left join public.gateway_payment_attempts a
    on a.gateway_id=e.gateway_id and a.user_id=p_user_id and a.created_at>=now()-interval '30 days'
  group by e.gateway_id
), scored as (
  select e.gateway_id,
    round((e.circuit_ok*100.0)
      ...
      -(case when e.capabilities->>'cost_bps'~'^[0-9]+(\\.[0-9]+)?$' then least(15,(e.capabilities->>'cost_bps')::numeric/100.0) else 0 end),4) routing_score,
    case when coalesce(a.total,0)=0 then 1 else round((a.approved/nullif(a.total,0))*100,4) end approval_rate,
    e.latency_ms,e.healthy,e.circuit_state,
...
from scored where circuit_state<>'open'
