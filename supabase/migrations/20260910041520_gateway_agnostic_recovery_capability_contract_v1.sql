DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'recovery_state') THEN CREATE TYPE public.recovery_state AS ENUM ('UNKNOWN','RECOVERY','STATUS_CHECK','APPROVED','DECLINED','PENDING','DEAD_LETTER'); END IF; END $$;
...
ALTER TABLE public.gateway_recovery_queue
  ...
  ADD CONSTRAINT gateway_recovery_queue_execution_logs_object_array_ck CHECK (jsonb_typeof(execution_logs) = 'array');
...
DECLARE v_state public.recovery_state; v_version bigint;
BEGIN
  IF auth.role() <> 'service_role' THEN RAISE EXCEPTION 'service_role_required'; END IF;
  IF p_next_state IS NULL THEN RAISE EXCEPTION 'recovery_state_required'; END IF;
  IF p_next_state = 'UNKNOWN' THEN
    RAISE EXCEPTION 'recovery_unknown_is_initial_only';
  END IF;
  UPDATE public.gateway_recovery_queue
  SET recovery_state = p_next_state,
      ...
      execution_logs = execution_logs || jsonb_build_array(jsonb_build_object('at', clock_timestamp(), 'from_version', state_version, 'to', p_next_state::text, 'note', p_note)),
      updated_at = clock_timestamp()
  WHERE id = p_job_id AND state_version = p_expected_version
  RETURNING true, gateway_recovery_queue.state_version, gateway_recovery_queue.recovery_state INTO v_state, v_version, v_state;
  IF NOT FOUND THEN
    RETURN QUERY SELECT false, NULL::bigint, NULL::public.recovery_state;
  ELSE
    RETURN QUERY SELECT true, v_version, v_state;
  END IF;
END $$;
