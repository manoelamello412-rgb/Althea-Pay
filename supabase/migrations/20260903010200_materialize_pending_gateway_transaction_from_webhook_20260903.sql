...
begin
  ...
  if v_status in ('processing') then v_status := 'pending';
  ...
  if v_status in ('pending','approved','failed') then
    for v_external_id in select x from jsonb_array_elements_text(jsonb_build_array(nullif(p_payload->>'external_id',''),nullif(p_payload->>'transaction_id',''),nullif(p_payload->>'id',''))) as t(x) where x is not null loop
      ...
      where a.external_transaction_id=v_external_id and a.user_id is not null
      order by a.created_at desc limit 1;
      exit when v_attempt.id is not null;
    end loop;
    if v_attempt.id is not null then
      select * into v_route from public.gateway_routes r
      where r.id=v_attempt.routing_rule_id and r.user_id=v_attempt.user_id and r.gateway_id=v_attempt.gateway_id limit 1;
      select * into v_routing from public.transaction_routing_logs l
      where l.user_id=v_attempt.user_id and l.idempotency_key=split_part(v_attempt.idempotency_key,':',1)
      order by l.created_at desc limit 1;
      if v_route.id is not null and v_routing.id is not null then
        select gt.id into v_existing from public.gateway_transactions gt
        where gt.user_id=v_attempt.user_id and gt.external_id=v_attempt.external_transaction_id
        ...
        if v_existing is null then
          ...
          values(v_attempt.user_id,v_route.funnel_id,v_attempt.product_id,v_attempt.gateway_id::text,v_attempt.external_transaction_id,split_part(v_attempt.idempotency_key,':',1),v_routing.amount,upper(v_routing.currency),v_status,coalesce(case when jsonb_typeof(p_payload->'customer')='object' then p_payload->'customer' else null end,'{}'::jsonb),coalesce(p_payload,'{}'::jsonb),greatest(coalesce(v_attempt.attempt_order,1),1),jsonb_build_object('materialized_from_gateway_webhook',true,'provider',lower(trim(p_provider)),'provider_event_id',trim(p_provider_event_id),'routing_log_id',v_routing.id,'payment_attempt_id',v_attempt.id));
        end if;
      end if;
    end if;
  end if;
