...
declare
  ...
  funnel_org_id uuid;
begin
  select f.user_id, f.organization_id into owner_id, funnel_org_id
    from public.funnels f
   where f.id=target_funnel and f.deleted_at is null;
  if owner_id is null or funnel_org_id is null then raise exception 'FUNNEL_NOT_FOUND'; end if;
  if not private.is_org_member(funnel_org_id) or not private.has_org_role(funnel_org_id,array['owner','admin','manager','operator']) then
    raise exception 'FORBIDDEN' using errcode='42501';
  end if;
  insert into public.funnel_steps (funnel_id,user_id,organization_id,step_key,step_type,name,position,status)
  values
...
begin
  ...
  select * into v_funnel
    from public.funnels
   where id=trim(p_funnel_id) and deleted_at is null
   for update;
  if not found then raise exception using errcode='P0002', message='funnel_not_found'; end if;
  if not private.is_org_member(v_funnel.organization_id) then raise exception using errcode='42501', message='forbidden'; end if;
  if not private.has_org_role(v_funnel.organization_id,array['owner','admin','manager','operator']) then raise exception using errcode='42501', message='forbidden'; end if;
  ...
  update public.funnels
     set nome=trim(p_name), url=nullif(trim(coalesce(p_url,'')),''), status='active', last_communication=coalesce(last_communication,v_now)
  ...
  update public.funnel_connections
     set config=coalesce(config,'{}'::jsonb)||v_config,
         ...
         updated_at=v_now
   where funnel_id=v_funnel.id and organization_id=v_funnel.organization_id;
  ...
  if not found then
    ...
    values(v_user_id,v_funnel.id,v_funnel.organization_id,'script','active',v_config,'unknown',v_now,v_now);
  end if;
