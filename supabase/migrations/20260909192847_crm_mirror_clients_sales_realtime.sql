...
create or replace function public.crm_mirror_sale_event()
returns trigger language plpgsql security definer set search_path=public
as $$
declare v_payload jsonb; v_name text; v_email text; v_status text;
begin
  v_payload:=coalesce(new.data,'{}'::jsonb)||jsonb_build_object('sale_id',new.id,'funnel_id',new.funnel_id,'product_id',new.product_id,'checkout_id',new.checkout_id,'transaction_id',new.transaction_id,'amount',new.amount,'currency',new.currency,'status',new.status,'source',new.source,'medium',new.medium,'campaign',new.campaign,'content',new.content,'term',new.term,'click_id',new.click_id,'external_id',new.external_id,'gateway_id',new.gateway_id,'occurred_at',new.occurred_at);
  ...
  insert into public.crm_webhook_events(user_id,idempotency_key,transaction_id,status,buyer_email,buyer_name,payload,received_at)
  values(new.user_id,'sale:'||new.id||':'||v_status||':'||coalesce(to_char(coalesce(new.occurred_at,new.created_at),'YYYYMMDDHH24MISSMS'),'na'),new.transaction_id::text,v_status,v_email,v_name,v_payload,coalesce(new.occurred_at,new.created_at,timezone('utc',now())))
  on conflict (user_id,idempotency_key) where idempotency_key is not null do nothing;
  return new;
end;
$$;
