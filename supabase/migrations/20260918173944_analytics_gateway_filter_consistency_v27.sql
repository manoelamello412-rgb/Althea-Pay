...
begin
  ...
  current_checkout as (
    ...
    where c.organization_id=v_org
      ...
      and (
        p_gateway_id is null or p_gateway_id='' or exists(
          select 1
          from public.gateway_transactions gt
          where gt.organization_id=v_org
            and gt.gateway_id=p_gateway_id
            and gt.metadata->>'checkout_session_id'=c.id::text
        )
      )
  ),
