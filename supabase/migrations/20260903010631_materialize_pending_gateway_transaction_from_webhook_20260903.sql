...
begin
  ...
  if v_status in ('pending','approved','failed') then
    ...
    if v_attempt.id is not null then
      select * into v_route
      from public.gateway_routes r
      where r.id=v_attempt.routing_rule_id
        and r.user_id=v_attempt.user_id
        and r.gateway_id=v_attempt.gateway_id
      limit 1;

      select * into v_routing
      from public.transaction_routing_logs l
      where l.user_id=v_attempt.user_id
        and l.idempotency_key=split_part(v_attempt.idempotency_key,':',1)
      ...
      if v_route.id is not null and v_routing.id is not null then
        ...
        where gt.user_id=v_attempt.user_id
          and gt.external_id=v_attempt.external_transaction_id
        order by gt.created_at desc
        limit 1;

        if v_existing is null then
          insert into public.gateway_transactions(
            user_id,funnel_id,product_id,gateway_id,external_id,idempotency_key,
            amount,currency,status,customer,metadata,attempt_count,routing_metadata
          ) values (
            v_attempt.user_id,
            v_route.funnel_id,
            v_attempt.product_id,
            v_attempt.gateway_id::text,
