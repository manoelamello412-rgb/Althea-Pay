...
-- Canonical provider cost matrix. This is intentionally keyed by the canonical text gateway id;
-- the legacy gateway_configurations UUID model is not used by ALTHEA PAY.
...
CREATE INDEX IF NOT EXISTS idx_gateway_interchange_fees_lookup
  ON public.gateway_interchange_fees(user_id, gateway_id, card_brand, active);
...
-- Operational trace graph. The graph is generated from canonical transaction/attempt records,
-- so it cannot be forged by a browser client and remains useful even when a provider call is ambiguous.
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_gateway_orchestration_trace_transaction
  ON public.gateway_orchestration_traces(tenant_id, transaction_id)
  WHERE transaction_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_gateway_orchestration_traces_lookup
  ON public.gateway_orchestration_traces(tenant_id, idempotency_key, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_gateway_orchestration_traces_funnel
  ON public.gateway_orchestration_traces(tenant_id, funnel_id, created_at DESC);
...
-- Server-owned append/refresh surface. Browser clients never write traces directly.
...
AS $function$
DECLARE
  tx public.gateway_transactions%ROWTYPE;
  trace jsonb;
BEGIN
  SELECT * INTO tx
  ...
  SELECT jsonb_build_object(
    'request', jsonb_build_object(
      ...
      'funnel_id', tx.funnel_id,
      'product_id', tx.product_id,
      'idempotency_key', tx.idempotency_key,
      'trace_id', tx.routing_metadata->>'trace_id'
    ),
    'policy_evaluation', jsonb_build_object(
      'policy_id', tx.routing_metadata->>'policy_id',
      'policy_version', NULLIF(tx.routing_metadata->>'policy_version','')::bigint,
      'source', tx.routing_metadata->>'source',
      'decision_trace', COALESCE(tx.routing_metadata->'decision_trace','[]'::jsonb),
      'ranking', COALESCE(tx.routing_metadata->'ranking','[]'::jsonb)
    ),
    'attempts', COALESCE((
      SELECT jsonb_agg(
        jsonb_build_object(
          ...
          'provider', a.gateway_name,
          'status', a.status,
          'outcome', CASE WHEN a.status='approved' THEN 'APPROVED' WHEN a.status='declined' THEN 'DECLINED' WHEN a.status='pending' THEN 'PENDING' ELSE upper(a.failure_class) END,
          'failure_class', a.failure_class,
          'external_transaction_id', a.external_transaction_id,
      ...
      WHERE a.user_id = tx.user_id AND a.transaction_id = tx.id
    ), '[]'::jsonb),
  ...
  INSERT INTO public.gateway_orchestration_traces(
    transaction_id, idempotency_key, tenant_id, funnel_id, trace_graph, updated_at
  ) VALUES (
    tx.id, COALESCE(tx.idempotency_key, NEW.idempotency_key), tx.user_id, tx.funnel_id, trace, now()
  )
...
AFTER INSERT OR UPDATE OF status, external_transaction_id, failure_class, provider_request_id, duration_ms, decision_reason, routing_policy_id, routing_policy_version
ON public.gateway_payment_attempts
FOR EACH ROW
WHEN (NEW.transaction_id IS NOT NULL)
EXECUTE FUNCTION public.gateway_refresh_orchestration_trace();

-- Trace immutability is append/refresh-only from the server trigger; callers cannot delete or mutate it.
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    RAISE EXCEPTION 'gateway_orchestration_trace_immutable';
  END IF;
...
CREATE TRIGGER trg_gateway_orchestration_trace_guard
BEFORE UPDATE OR DELETE ON public.gateway_orchestration_traces
FOR EACH ROW EXECUTE FUNCTION public.gateway_orchestration_trace_guard();

-- Cost helper: returns effective basis points for one gateway/brand.
CREATE OR REPLACE FUNCTION public.gateway_effective_cost_bps(
  p_user_id uuid,
  p_gateway_id text,
  p_card_brand text,
...
AS $function$
  SELECT CASE
    WHEN p_amount_minor IS NULL OR p_amount_minor <= 0 THEN NULL
    ELSE round((f.percentage_fee * 10000) + ((f.fixed_fee_minor / p_amount_minor) * 10000), 4)
  END
  FROM public.gateway_interchange_fees f
  WHERE f.user_id = p_user_id
    AND f.gateway_id = p_gateway_id
    AND f.active
    AND lower(f.card_brand) = lower(COALESCE(p_card_brand, 'default'))
  LIMIT 1;
$function$;
...
-- Keep operational timestamps consistent.
...
CREATE TRIGGER trg_gateway_interchange_fees_touch
BEFORE UPDATE ON public.gateway_interchange_fees
FOR EACH ROW EXECUTE FUNCTION public.gateway_interchange_fees_touch();
