...
create index if not exists funnel_steps_funnel_position_idx
  on public.funnel_steps(funnel_id, position);
...
create index if not exists funnel_step_links_from_idx
  on public.funnel_step_links(from_step_id, priority);
...
create index if not exists funnel_offers_funnel_idx
  on public.funnel_offers(funnel_id, created_at desc);
...
create index if not exists funnel_automation_rules_funnel_idx
  on public.funnel_automation_rules(funnel_id, enabled);
...
create policy funnel_steps_member_select on public.funnel_steps
  for select to authenticated
  using (public.is_org_member(organization_id));

create policy funnel_step_links_member_select on public.funnel_step_links
  for select to authenticated
  using (public.is_org_member(organization_id));

create policy funnel_offers_member_select on public.funnel_offers
  for select to authenticated
  using (public.is_org_member(organization_id));

create policy funnel_automation_rules_member_select on public.funnel_automation_rules
  for select to authenticated
  using (public.is_org_member(organization_id));
...
begin
  ...
  insert into public.funnel_steps (organization_id, funnel_id, step_key, step_type, name, position, status)
  values
  ...
  select target_org, target_funnel, a.id, b.id, 0
  ...
  return query
  select * from public.funnel_steps where funnel_id = target_funnel order by position;
end;
$$;
