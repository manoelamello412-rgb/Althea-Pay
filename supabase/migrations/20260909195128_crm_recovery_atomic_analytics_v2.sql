...
CREATE TABLE IF NOT EXISTS public.crm_agents (
  ...
  name text NOT NULL, status text NOT NULL DEFAULT 'available' CHECK (status IN ('available','busy','offline')),
  last_assigned_at timestamptz NOT NULL DEFAULT '1970-01-01T00:00:00Z'::timestamptz, created_at timestamptz NOT NULL DEFAULT now()
);
...
WITH bounds AS(SELECT greatest(1,least(p_days,365))::integer d), c AS(SELECT * FROM crm_conversations,bounds WHERE user_id=auth.uid() AND created_at>=now()-make_interval(days=>bounds.d)), e AS(SELECT * FROM crm_webhook_events,bounds WHERE user_id=auth.uid() AND received_at>=now()-make_interval(days=>bounds.d)), s AS(SELECT * FROM sales,bounds WHERE user_id=auth.uid() AND coalesce(occurred_at,created_at)>=now()-make_interval(days=>bounds.d))
...
$$;
REVOKE ALL ON FUNCTION public.crm_recovery_execute(uuid) FROM PUBLIC; GRANT EXECUTE ON FUNCTION public.crm_recovery_execute(uuid) TO authenticated;
REVOKE ALL ON FUNCTION public.crm_analytics(integer) FROM PUBLIC; GRANT EXECUTE ON FUNCTION public.crm_analytics(integer) TO authenticated;
REVOKE ALL ON FUNCTION public.althea_pay_calculate_tmr(uuid) FROM PUBLIC; GRANT EXECUTE ON FUNCTION public.althea_pay_calculate_tmr(uuid) TO authenticated;
COMMIT;