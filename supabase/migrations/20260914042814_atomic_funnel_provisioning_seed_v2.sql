...
declare v_user_id uuid := auth.uid(); v_funnel_id text; v_token text; v_funnel jsonb; v_connection jsonb; v_token_row jsonb;
begin
 ...
 insert into public.funnels(id,nome,url,endpoint,status,funnel_type,user_id) values(v_funnel_id,btrim(p_name),nullif(btrim(p_url),''),p_event_endpoint,'active',p_funnel_type,v_user_id) returning jsonb_build_object('id',id,'nome',nome,'url',url,'endpoint',endpoint,'status',status,'funnel_type',funnel_type,'created_at',created_at) into v_funnel;
 insert into public.funnel_connections(user_id,funnel_id,connection_type,status,health_status,config,connected_at) values(v_user_id,v_funnel_id,p_connection_type,'active','unknown',jsonb_build_object('protocol_version','2026-09','event_endpoint',p_event_endpoint,'funnel_type',p_funnel_type),now()) returning jsonb_build_object('id',id,'funnel_id',funnel_id,'connection_type',connection_type,'status',status,'health_status',health_status,'connected_at',connected_at) into v_connection;
 insert into public.funnel_ingestion_tokens(user_id,funnel_id,token_prefix,token_hash,enabled) values(v_user_id,v_funnel_id,left(v_token,14),encode(digest(v_token,'sha256'),'hex'),true) returning jsonb_build_object('id',id,'funnel_id',funnel_id,'token_prefix',token_prefix,'enabled',enabled,'created_at',created_at) into v_token_row;
