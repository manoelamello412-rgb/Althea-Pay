...
begin
  ...
  if tg_op = 'INSERT' then
    ...
    target_old_role := old.role;
    if target_old_role = 'owner' and actor_role <> 'owner' then
...
create trigger enforce_organization_member_authority
...
begin
  if new.default_organization_id is not null
     and not exists (
       ...
       where om.organization_id = new.default_organization_id
...
create trigger enforce_profile_default_organization_membership
