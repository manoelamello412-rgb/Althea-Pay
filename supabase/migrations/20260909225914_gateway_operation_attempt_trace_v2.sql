create or replace function public.persist_gateway_attempt_operation_trace()
returns trigger
language plpgsql
security definer
set search_path=public
as $$
declare v_tx jsonb; v_policy_id uuid; v_policy_version bigint; v_score numeric;
begin
  if new.transaction_id is null or new.external_transaction_id is null then return new; end if;
  select coalesce(metadata,'{}'::jsonb), coalesce(routing_metadata,'{}'::jsonb)
    into v_tx, v_tx
    from public.gateway_transactions
   where id=new.transaction_id and user_id=new.user_id;
  begin v_policy_id:=(v_tx->>'policy_id')::uuid; exception when others then v_policy_id:=null; end;
  begin v_policy_version:=(v_tx->>'policy_version')::bigint; exception when others then v_policy_version:=null; end;
...
create trigger trg_persist_gateway_attempt_operation_trace
after insert or update of transaction_id on public.gateway_payment_attempts
for each row execute function public.persist_gateway_attempt_operation_trace();
