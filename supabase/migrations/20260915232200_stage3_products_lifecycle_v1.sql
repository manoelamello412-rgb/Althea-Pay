create or replace function public.set_product_status(p_product_id text, p_version bigint, p_status text) returns public.products language plpgsql security definer set search_path = pg_catalog, public as $$
declare v_org uuid; v_product public.products;
begin
  if auth.uid() is null then raise exception 'AUTH_REQUIRED' using errcode='42501'; end if;
  if p_status not in ('draft','active','paused','archived') then raise exception 'PRODUCT_STATUS_INVALID'; end if;
  ...
  insert into public.audit_logs(user_id,actor_id,organization_id,action,resource_type,resource_id,metadata) values(auth.uid(),auth.uid(),v_org,'product.status_changed','product',v_product.id,jsonb_build_object('status',p_status,'version',v_product.version));
  return v_product;
end $$;
