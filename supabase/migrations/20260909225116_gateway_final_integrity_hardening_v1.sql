...
-- Reconciliation provider event identity is tenant/gateway scoped; reject blank event ids
-- when an item claims to originate from a provider event.
...
-- Make operation tracing explicitly bounded so malformed telemetry cannot pollute the trace store.
