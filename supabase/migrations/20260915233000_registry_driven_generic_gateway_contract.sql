...
alter table public.gateway_provider_registry
  drop constraint if exists gateway_provider_registry_execution_config_chk;
alter table public.gateway_provider_registry
  add constraint gateway_provider_registry_execution_config_chk
  check (jsonb_typeof(execution_config) = 'object');

alter table public.gateway_provider_registry
  drop constraint if exists gateway_provider_registry_webhook_config_chk;
alter table public.gateway_provider_registry
  add constraint gateway_provider_registry_webhook_config_chk
  check (jsonb_typeof(webhook_config) = 'object');

create index if not exists gateway_provider_registry_operational_idx
  on public.gateway_provider_registry(is_active, operational, provider_key);

-- Runtime/HTTP behavior belongs to the provider definition. The Front only
-- collects per-connection credentials; the Generic Adapter receives the
-- registry contract from the trusted service-role resolver.
update public.gateway_provider_registry
set
  credential_schema = jsonb_build_object(
    'fields', jsonb_build_array(
      jsonb_build_object('name','api_key','label','API Key / Token','type','password','required',false),
      jsonb_build_object('name','access_token','label','Access Token','type','password','required',false),
      jsonb_build_object('name','secret_key','label','Secret Key','type','password','required',false),
      jsonb_build_object('name','token','label','Token','type','password','required',false)
    )
  ),
  execution_config = jsonb_build_object(
    ...
    'status_method', 'GET',
    'refund_path', '/payments/{id}/refund',
    'refund_method', 'POST',
    'health_path', '/health',
    'health_method', 'GET',
    'request_template', '',
    'response_mapping', jsonb_build_object('id','id','status','status','amount','amount','currency','currency'),
...
update public.gateway_provider_registry
set
  execution_config = (select execution_config from public.gateway_provider_registry where provider_key='generic_http'),
  webhook_config = (select webhook_config from public.gateway_provider_registry where provider_key='generic_http'),
  schema_version = 1,
  updated_at = now()
where provider_key = 'custom_rest';

create or replace function public.register_gateway_provider_definition(
  p_provider_key text,
  p_display_name text,
  p_credential_schema jsonb default '{"fields":[]}'::jsonb,
  p_capabilities jsonb default '{}'::jsonb,
  p_adapter_key text default null,
  p_adapter_url text default null,
  p_execution_config jsonb default '{}'::jsonb,
  p_webhook_config jsonb default '{}'::jsonb
)
returns jsonb
language plpgsql
security definer
...
as $$
declare
  k text := lower(trim(p_provider_key));
  a text := nullif(lower(trim(coalesce(p_adapter_key,p_provider_key))), '');
  u text := nullif(trim(p_adapter_url),'');
...
begin
  ...
  if jsonb_typeof(coalesce(p_credential_schema,'{}'::jsonb)) <> 'object'
     or jsonb_typeof(coalesce(p_capabilities,'{}'::jsonb)) <> 'object'
     or jsonb_typeof(coalesce(p_execution_config,'{}'::jsonb)) <> 'object'
     or jsonb_typeof(coalesce(p_webhook_config,'{}'::jsonb)) <> 'object'
  then raise exception 'invalid_provider_schema'; end if;

  insert into public.gateway_provider_registry(
    provider_key, display_name, credential_schema, capabilities,
    adapter_key, adapter_url, operational, is_active,
    execution_config, webhook_config, schema_version, updated_at
  ) values (
    k, trim(p_display_name), coalesce(p_credential_schema,'{}'::jsonb), coalesce(p_capabilities,'{}'::jsonb),
    a, u, true, true, coalesce(p_execution_config,'{}'::jsonb),
    coalesce(p_webhook_config,'{}'::jsonb), 1, now()
  )
...
declare
  ...
  c public.user_gateway_credentials%rowtype;
  r public.gateway_provider_registry%rowtype;
  secret text;
...
begin
  ...
  select * into r from public.gateway_provider_registry
  where provider_key=lower(trim(g.provider)) and is_active=true limit 1;
  if not found then raise exception 'provider_not_registered'; end if;

  select decrypted_secret into secret
  from vault.decrypted_secrets where id=c.secret_ref::uuid limit 1;
  if coalesce(secret,'')='' then raise exception 'gateway_credential_secret_unavailable'; end if;

  begin
    credential_json := secret::jsonb;
  exception when others then
    raise exception 'gateway_credential_bundle_invalid';
  end;

  if jsonb_typeof(credential_json) <> 'object' then
    raise exception 'gateway_credential_bundle_invalid';
  end if;
  ...
  return jsonb_build_object(
    ...
    'credentials',merged,
    'provider_config',coalesce(r.execution_config,'{}'::jsonb),
    'webhook_config',coalesce(r.webhook_config,'{}'::jsonb),
