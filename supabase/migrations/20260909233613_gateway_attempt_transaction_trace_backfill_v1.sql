...
begin
  pid:=nullif(m->>'policy_id','')::uuid;
  pver:=nullif(m->>'policy_version','')::bigint;
  ...
  set transaction_id=new.id,
      ...
      duration_ms=coalesce(a.duration_ms,extract(epoch from (coalesce(a.completed_at,a.updated_at)-a.created_at)*1000)::integer)
  where a.user_id=new.user_id
    and a.gateway_id=(select g.id from public.gateways g where g.id=new.gateway_id limit 1)
...
create trigger trg_link_gateway_transaction_to_attempts
after insert on public.gateway_transactions
for each row execute function public.link_gateway_transaction_to_attempts();