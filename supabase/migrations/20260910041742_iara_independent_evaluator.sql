...
create index if not exists idx_iara_evaluation_claims_evaluation on public.iara_evaluation_claims(evaluation_id);
...
create index if not exists idx_iara_evaluation_failures_evaluation on public.iara_evaluation_failures(evaluation_id);
...
create or replace function public.persist_iara_independent_evaluation(
  p_tenant_id uuid, p_execution_id uuid, p_evaluator_version text,
  p_overall_score numeric, p_hallucination_risk numeric, p_grounding_score numeric,
  p_tool_call_accuracy numeric, p_evidence_coverage numeric, p_causal_confidence numeric,
  p_data_confidence numeric, p_total_cost_minor bigint, p_latency_ms integer,
  p_decision text, p_summary text, p_assertions jsonb, p_claims jsonb,
  p_failures jsonb, p_tool_calls jsonb
) returns uuid
