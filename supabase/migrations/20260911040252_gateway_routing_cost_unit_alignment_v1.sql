...
split_matches as (
 ...
 from public.gateway_routing_split_policies s
...
weighted as (
 ...
 case when exists(select 1 from exact_overrides) then case when s.gateway_id=(select gateway_id from exact_overrides limit 1) then 1 else 0 end else 1 end forced_ok
 from scored s
)
select gateway_id,round(base_score*weight,4) routing_score,approval_rate,latency_ms,healthy,circuit_state,cost_bps
from weighted where circuit_state<>'open' and forced_ok=1 order by routing_score desc,gateway_id asc;
$function$;
commit;