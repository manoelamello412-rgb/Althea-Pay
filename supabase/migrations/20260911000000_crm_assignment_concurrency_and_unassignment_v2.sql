...
begin
  ...
  -- Serialize all assignment mutations for one conversation.
  ...
  if p_agent_id is not null and not exists(select 1 from public.crm_agents where id=p_agent_id and user_id=v_uid) then
    raise exception 'AGENT_NOT_FOUND';
  end if;
  if p_team_id is not null and not exists(select 1 from public.crm_teams where id=p_team_id and user_id=v_uid and active) then
