...
SELECT pgmq.create('iara-telemetry-process') WHERE NOT EXISTS (
  SELECT 1 FROM pgmq.list_queues() WHERE queue_name = 'iara-telemetry-process'
);
SELECT pgmq.create('iara-crm-injection') WHERE NOT EXISTS (
  SELECT 1 FROM pgmq.list_queues() WHERE queue_name = 'iara-crm-injection'
);
SELECT pgmq.create('iara-financial-pix-retry') WHERE NOT EXISTS (
  SELECT 1 FROM pgmq.list_queues() WHERE queue_name = 'iara-financial-pix-retry'
);
SELECT pgmq.create('iara-dead-letter') WHERE NOT EXISTS (
  SELECT 1 FROM pgmq.list_queues() WHERE queue_name = 'iara-dead-letter'
);
...
CREATE INDEX IF NOT EXISTS idx_iara_queue_jobs_tenant_status_created
  ON public.iara_queue_jobs(tenant_id, status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_iara_queue_jobs_queue_status
  ON public.iara_queue_jobs(queue_type, status, created_at ASC);
...
CREATE INDEX IF NOT EXISTS idx_iara_queue_dlq_tenant_quarantined
  ON public.iara_queue_dead_letters(tenant_id, quarantined_at DESC);
...
CREATE POLICY iara_queue_jobs_select_own
  ON public.iara_queue_jobs FOR SELECT TO authenticated
  USING (tenant_id = auth.uid());

DROP POLICY IF EXISTS iara_queue_dlq_select_own ON public.iara_queue_dead_letters;
CREATE POLICY iara_queue_dlq_select_own
  ON public.iara_queue_dead_letters FOR SELECT TO authenticated
  USING (tenant_id = auth.uid());
...
CREATE OR REPLACE FUNCTION public.iara_enqueue_job(
  p_tenant_id UUID,
  p_job_id UUID,
...
AS $$
DECLARE
  v_existing public.iara_queue_jobs;
  v_message_id BIGINT;
BEGIN
  ...
  FROM public.iara_queue_jobs
  WHERE tenant_id = p_tenant_id AND idempotency_key = p_idempotency_key;

  IF FOUND THEN
    SELECT msg_id INTO v_message_id
    FROM pgmq.q_iara_telemetry_process WHERE message->>'jobId' = v_existing.id::text
    UNION ALL
    SELECT msg_id FROM pgmq.q_iara_crm_injection WHERE message->>'jobId' = v_existing.id::text
    UNION ALL
    SELECT msg_id FROM pgmq.q_iara_financial_pix_retry WHERE message->>'jobId' = v_existing.id::text
    LIMIT 1;
    RETURN QUERY SELECT v_existing.id, COALESCE(v_message_id, 0), true;
    RETURN;
  END IF;
...
CREATE OR REPLACE FUNCTION public.iara_claim_queue_jobs(
  p_queue_name TEXT,
...
CREATE OR REPLACE FUNCTION public.iara_complete_queue_job(
  p_queue_name TEXT,
  p_message_id BIGINT,
  p_job_id UUID
) RETURNS BOOLEAN
...
CREATE OR REPLACE FUNCTION public.iara_retry_queue_job(
  p_queue_name TEXT,
  p_message_id BIGINT,
  p_job_id UUID,
...
BEGIN
  ...
  SELECT pgmq.send(p_queue_name, jsonb_build_object(
    'jobId', v_job.id,
    'tenantId', v_job.tenant_id,
    'type', v_job.queue_type,
    'payload', v_job.payload,
    'attemptsMade', p_next_attempt,
    'maxAttempts', v_job.max_attempts,
    'enqueuedAt', v_job.created_at
  ), p_delay_seconds) INTO v_new_message_id;

  UPDATE public.iara_queue_jobs
  SET attempts_made=p_next_attempt,status='RETRYING',last_error_code=left(p_error_code,200),last_error_message=left(p_error_message,1000),updated_at=now()
  WHERE id=p_job_id;
...
CREATE OR REPLACE FUNCTION public.iara_dead_letter_queue_job(
  p_queue_name TEXT,
  p_message_id BIGINT,
  p_job_id UUID,
  p_error_code TEXT,
...
BEGIN
  ...
  VALUES(v_job.id,v_job.tenant_id,v_job.queue_type,v_job.attempts_made,v_job.payload,left(p_error_code,200),left(p_error_message,1000))
  ON CONFLICT (job_id) DO NOTHING;

  SELECT pgmq.send('iara-dead-letter', jsonb_build_object(
    'jobId', v_job.id,
    'tenantId', v_job.tenant_id,
    'type', v_job.queue_type,
    'payload', v_job.payload,
    'attemptsMade', v_job.attempts_made,
    'errorCode', left(p_error_code,200),
    'errorMessage', left(p_error_message,1000),
    'quarantinedAt', now()
  )) INTO v_dlq_message_id;
...
GRANT EXECUTE ON FUNCTION public.iara_dead_letter_queue_job(TEXT,BIGINT,UUID,INTEGER,TEXT,TEXT) TO service_role;
