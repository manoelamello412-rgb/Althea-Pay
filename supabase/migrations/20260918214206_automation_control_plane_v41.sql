...
begin
  ...
  perform pg_advisory_xact_lock(
    hashtext('automation-rate-org:'||p_organization_id::text||':'||p_rule_id::text)
  );
...
begin
  ...
  return public.crm_check_automation_rate_limit_org(
    v_org,p_rule_id,p_limit,p_window_seconds
  );
end;
$function$;
...
create or replace function public.crm_cancel_automation_execution(
  p_execution_id uuid,
...
begin
  ...
  if not private.has_org_role(v_org,array['owner','admin','manager']) then
  ...
  set status='cancelled',
      cancelled_at=now(),
      cancellation_reason=left(coalesce(p_reason,'manual cancellation'),1000),
      next_retry_at=null,
      scheduled_at=null
  where id=p_execution_id
...
create or replace function public.crm_replay_automation_execution(
  p_execution_id uuid,
  p_reason text default null
)
returns public.automation_executions
...
begin
  ...
  if not private.has_org_role(r.organization_id,array['owner','admin','manager']) then
    raise exception using errcode='42501',message='forbidden';
  end if;

  v_key:='automation-replay:'||r.id::text||':'||
    extract(epoch from coalesce(r.dead_lettered_at,r.created_at))::text;

  select * into v_existing
  from public.idempotency_keys
  where user_id=auth.uid()
    and scope='automation_replay'
  ...
  insert into public.idempotency_keys(
    user_id,scope,idempotency_key,status,resource_type,resource_id,response_payload,request_digest
  )
  values(
    auth.uid(),'automation_replay',v_key,'processing','automation_execution',r.id::text,
    jsonb_build_object('execution_id',r.id,'reason',p_reason),
  ...
  values(
    auth.uid(),auth.uid(),'automation.replay','automation_execution',r.id::text,
    jsonb_build_object(
      'organization_id',r.organization_id,
      'replay_idempotency_id',v_idem_id,
      'replay_key',v_key,
      'reason',p_reason,
  ...
  set status='completed',
      ...
      response_payload=jsonb_build_object(
        'execution_id',r.id,'status',r.status,'replay_count',r.replay_count
      ),
...
begin
  ...
  if v_org is null
     or not private.has_org_role(v_org,array['owner','admin','manager']) then
    raise exception using errcode='42501',message='forbidden';
  end if;
  ...
  if pg_column_size(p_trigger_config)>20000 or pg_column_size(p_action_config)>30000 then
    raise exception using errcode='22023',message='automation_config_too_large';
  end if;

  v_event_type:=coalesce(nullif(btrim(p_trigger_config->>'event_type'),''),'*');
  if v_event_type<>'*'
     and not exists(
       select 1 from public.funnel_event_types
       where enabled=true and canonical_event_type=v_event_type
     ) then
    raise exception using errcode='22023',message='invalid_automation_event_type';
  end if;

  v_funnel_id:=nullif(btrim(p_trigger_config->>'funnel_id'),'');
  if v_funnel_id is not null
     and v_funnel_id<>'*'
     and not exists(
       select 1 from public.funnels
       where id=v_funnel_id and organization_id=v_org and deleted_at is null
     ) then
    raise exception using errcode='22023',message='automation_funnel_not_found';
  end if;

  if p_trigger_config ? 'conditions' then
    if jsonb_typeof(p_trigger_config->'conditions')<>'array'
       or jsonb_array_length(p_trigger_config->'conditions')>20 then
      raise exception using errcode='22023',message='invalid_automation_conditions';
    end if;
  end if;

  v_actions:=case
    when jsonb_typeof(p_action_config->'actions')='array'
      then p_action_config->'actions'
    else jsonb_build_array(p_action_config)
  end;

  if jsonb_array_length(v_actions)<1 or jsonb_array_length(v_actions)>5 then
    raise exception using errcode='22023',message='invalid_automation_actions';
  end if;
  ...
  loop
    ...
    v_action_type:=lower(coalesce(
      nullif(btrim(v_action->>'type'),''),
      nullif(btrim(v_action->>'action'),'')
    ));

    if v_action_type not in ('log','alert','send_crm_message','set_conversation_status') then
      raise exception using errcode='22023',message='unsupported_automation_action';
    end if;
    ...
    if v_action_type='send_crm_message' then
      ...
      if lower(coalesce(v_action->>'status','')) not in ('open','pending','closed') then
  ...
  if p_rule_id is null then
    ...
    values(v_uid,v_org,btrim(p_name),v_status,p_trigger_config,p_action_config)
    returning * into v_rule;
  else
    update public.automation_rules
    set name=btrim(p_name),status=v_status,trigger_config=p_trigger_config,
        action_config=p_action_config,updated_at=now()
...
begin
  ...
  if v_org is null or not private.has_org_role(v_org,array['owner','admin']) then
    raise exception using errcode='42501',message='forbidden';
  end if;

  if exists(
    select 1 from public.automation_executions
    where rule_id=p_rule_id and organization_id=v_org
  ) then
    update public.automation_rules
...
begin
  ...
  return (
    with rule_rows as (
      select
        r.id,r.name,r.status,r.trigger_config,r.action_config,r.created_at,r.updated_at,
      ...
      left join lateral (
        select
          count(*) filter(where e.created_at>=now()-interval '24 hours') as executions_24h,
    ...
    execution_rows as (
      select
        e.id,e.rule_id,r.name as rule_name,e.status,e.action_type,e.event_id,e.execution_key,
        e.error_message,e.attempt_count,e.max_attempts,e.next_retry_at,e.scheduled_at,
        e.dead_lettered_at,e.cancelled_at,e.replay_count,e.created_at,e.started_at,e.completed_at,
      ...
      left join lateral (
        ...
        from public.automation_execution_attempts aa
    ...
    event_types as (
      select coalesce(
        jsonb_agg(jsonb_build_object('event_type',t.canonical_event_type,'protocol_version',t.protocol_version) order by t.canonical_event_type),
        '[]'::jsonb
      ) value
      from (
        select distinct canonical_event_type,protocol_version
        from public.funnel_event_types where enabled=true
      ) t
    ),
    funnels as (
      select coalesce(
        jsonb_agg(jsonb_build_object('id',f.id,'name',f.nome) order by f.nome),
        '[]'::jsonb
      ) value
    ...
    select jsonb_build_object(
      ...
      'capabilities',jsonb_build_object(
        'actions',jsonb_build_array('log','alert','send_crm_message','set_conversation_status'),
        'max_actions',5,'max_conditions',20,'financial_actions_direct',false
      )
    )
  );
end;
