...
begin
 ...
 if lower(coalesce(new.failure_class,'')) in ('timeout','unknown','technical','unavailable')
   ...
   select new.user_id,new.id,new.transaction_id,new.gateway_id,
          coalesce(new.gateway_name,''),new.idempotency_key,new.failure_class,
          new.external_transaction_id,'queued',0,now(),new.error_message
   where not exists (
     select 1 from public.gateway_recovery_queue q
     where q.user_id=new.user_id and q.attempt_id=new.id and q.status in ('queued','processing')
   );
 end if;
 return new;
end;
...
begin
 ...
 select l.id,l.user_id,l.instrument_id,l.gateway_id,l.provider,l.status,l.token_fingerprint
 into r from public.gateway_payment_token_links l
 where l.id=p_link_id and l.user_id=p_user_id and l.status='active';
...
begin
 if new.routing_policy_id is not null then
   select id,version,user_id into p from public.gateway_routing_policies
   where id=new.routing_policy_id and user_id=new.user_id and is_active=true;
   if not found then raise exception 'gateway_attempt_routing_policy_invalid'; end if;
   if new.routing_policy_version is null or new.routing_policy_version <> p.version then
     raise exception 'gateway_attempt_routing_policy_version_mismatch';
   end if;
 end if;
 return new;
end;
