...
CREATE INDEX IF NOT EXISTS idx_gateway_circuit_states_user_state ON public.gateway_circuit_states (user_id, circuit_state, updated_at DESC);
...
CREATE POLICY gateway_circuit_states_select ON public.gateway_circuit_states FOR SELECT TO authenticated USING (user_id = auth.uid());
...
END; $$;
CREATE OR REPLACE FUNCTION public.record_gateway_circuit_success(p_user_id UUID,p_gateway_id UUID,p_gateway_name TEXT)
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, public AS $$
DECLARE v_key BIGINT;
BEGIN
 IF auth.role() <> 'service_role' THEN RAISE EXCEPTION 'forbidden'; END IF;
 v_key := hashtextextended(p_user_id::text || ':' || p_gateway_id::text,0); PERFORM pg_advisory_xact_lock(v_key);
 INSERT INTO public.gateway_circuit_states(user_id,gateway_id,gateway_name,failure_count,circuit_state,opened_at,probe_until) VALUES(p_user_id,p_gateway_id,lower(trim(coalesce(p_gateway_name,p_gateway_id::text))),0,'closed',NULL,NULL)
 ON CONFLICT (user_id,gateway_id) DO UPDATE SET gateway_name=excluded.gateway_name,failure_count=0,circuit_state='closed',opened_at=NULL,probe_until=NULL,updated_at=timezone('utc',now()),version=public.gateway_circuit_states.version+1;
END; $$;
...
RETURNS TABLE(circuit_state TEXT,failure_count INTEGER) LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, public AS $$
