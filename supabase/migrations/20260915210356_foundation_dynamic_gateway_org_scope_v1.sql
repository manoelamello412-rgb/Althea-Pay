-- Foundation: enforce current organization membership inside SECURITY DEFINER gateway/funnel functions.
-- These functions bypass table RLS, so tenant authorization must be explicit in the function body.
...
begin
  ...
  select g.credential_id, g.organization_id into c, v_org_id from public.gateways g where g.id=p_gateway_id and g.user_id=u for update;
...
create or replace function public.get_gateway_panel_metrics(p_gateway_id text)
returns jsonb language plpgsql security definer
set search_path = public, pg_catalog
as $function$
...
begin
  if u is null then raise exception 'unauthorized' using errcode='42501'; end if;
  select * into g from public.gateways where id=p_gateway_id and user_id=u;
...
create or replace function public.update_dynamic_gateway(p_gateway_id text,p_display_name text,p_environment text,p_credentials jsonb default null::jsonb)
returns jsonb language plpgsql security definer
...
begin
  ...
  if owner_id <> auth.uid() or not private.is_org_member(funnel_org_id) then raise exception 'FORBIDDEN' using errcode='42501'; end if;
  insert into public.funnel_steps (funnel_id,user_id,step_key,step_type,name,position,status) values (target_funnel,owner_id,'entry','entry','Entrada',0,'active'),(target_funnel,owner_id,'sales-page','sales_page','Página de vendas',1,'draft'),(target_funnel,owner_id,'checkout','checkout','Checkout',2,'draft'),(target_funnel,owner_id,'payment','payment','Pagamento',3,'draft'),(target_funnel,owner_id,'thank-you','thank_you','Obrigado',4,'draft') on conflict (funnel_id,step_key) do nothing;
  insert into public.funnel_step_links (funnel_id,user_id,from_step_id,to_step_id,priority) select target_funnel,owner_id,a.id,b.id,0 from public.funnel_steps a join public.funnel_steps b on b.funnel_id=target_funnel and b.position=a.position+1 where a.funnel_id=target_funnel on conflict (from_step_id,to_step_id,priority) do nothing;
