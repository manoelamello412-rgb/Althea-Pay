...
begin
  ...
  if p_agent_id is not null and p_team_id is not null and not exists(select 1 from public.crm_team_members where user_id=v_uid and team_id=p_team_id and agent_id=p_agent_id and active) then raise exception 'AGENT_TEAM_MISMATCH'; end if;
  ...
  set assigned_to=p_agent_id,
      ...
      updated_at=now(),
      metadata=case when p_team_id is null then coalesce(metadata,'{}'::jsonb)-'team_id' else jsonb_set(coalesce(metadata,'{}'::jsonb),'{team_id}',to_jsonb(p_team_id::text),true) end
  where id=p_conversation_id and user_id=v_uid
  returning jsonb_build_object('id',id,'assigned_to',assigned_to,'priority',priority,'team_id',metadata->>'team_id','updated_at',updated_at) into v;
  return v;
end
$function$;
