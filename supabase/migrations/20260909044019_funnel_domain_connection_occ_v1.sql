...
begin
  ...
  if nullif(trim(p_external_funnel_id), '') is null then
    raise exception using errcode = '22023', message = 'external funnel id is required';
  end if;
  ...
  if p_expected_updated_at is not null and v_funnel.created_at is not null then
    if coalesce((to_jsonb(v_funnel)->>'updated_at')::timestamptz, v_funnel.created_at) <> p_expected_updated_at then
  ...
  if v_rows = 0 then
    insert into public.funnel_connections(user_id, funnel_id, connection_type, status, config, health_status, connected_at, updated_at)
    values(v_user_id, v_funnel.id, 'script', 'active', v_config, 'unknown', v_now, v_now);
  end if;
