...
CREATE OR REPLACE FUNCTION public.register_gateway_payment_token_link(
...
BEGIN
  ...
  INSERT INTO gateway_payment_token_links(user_id,instrument_id,gateway_id,provider,secret_ref,token_fingerprint,status)
  VALUES(v_user,p_instrument_id,p_gateway_id,lower(trim(p_provider)),v_secret::text,p_token_fingerprint,'active')
  ON CONFLICT(user_id,instrument_id,gateway_id) DO UPDATE SET provider=excluded.provider,secret_ref=excluded.secret_ref,token_fingerprint=excluded.token_fingerprint,status='active',updated_at=now()
...
BEGIN
  SELECT secret_ref INTO v_ref FROM gateway_payment_token_links WHERE id=p_link_id AND status='active';
