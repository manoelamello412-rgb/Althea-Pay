...
begin
  ...
  select * into t
   ...
   where user_id=c.user_id
     and (metadata->>'checkout_id')=p_checkout_id::text
  ...
  if t.status <> 'approved' then
    return jsonb_build_object('ok',false,'reason','transaction_not_approved','status',t.status);
  end if;
  ...
  insert into public.sales(
    ...
    sid,c.user_id,c.funnel_id,c.product_id,c.id,t.id,t.amount,t.currency,'approved',
    c.attribution,c.attribution->>'source',c.attribution->>'medium',c.attribution->>'campaign',
    c.attribution->>'content',c.attribution->>'term',c.attribution->>'click_id',t.external_id,
    t.gateway_id,coalesce(t.completed_at,now()),
    jsonb_build_object('projected_by','project_checkout_purchase')
  )
