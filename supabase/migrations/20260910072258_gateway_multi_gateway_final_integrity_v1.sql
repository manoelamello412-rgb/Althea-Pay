...
BEGIN
  ...
  -- The parent transaction row always exists. Locking it serializes allocation
  -- even when this is the first attempt and no child row exists yet.
  SELECT * INTO v_tx
  ...
  IF NOT EXISTS (
    ...
    WHERE g.id = p_gateway_id AND g.user_id = p_user_id
  ) THEN
  ...
  INSERT INTO public.gateway_payment_attempts(
    user_id,sale_id,product_id,gateway_id,gateway_name,routing_rule_id,
    idempotency_key,attempt_order,status,failure_class,external_transaction_id,
    transaction_id,routing_policy_id,routing_policy_version,decision_reason,
    provider_request_id,duration_ms,created_at,updated_at,completed_at
  ) VALUES (
...
REVOKE ALL ON FUNCTION public.allocate_and_insert_gateway_payment_attempt(uuid,uuid,text,text,text,text,text,text,uuid,uuid,bigint,text,text,integer,text,text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.allocate_and_insert_gateway_payment_attempt(uuid,uuid,text,text,text,text,text,text,uuid,uuid,bigint,text,text,integer,text,text) FROM anon;
REVOKE ALL ON FUNCTION public.allocate_and_insert_gateway_payment_attempt(uuid,uuid,text,text,text,text,text,text,uuid,uuid,bigint,text,text,integer,text,text) FROM authenticated;
...
-- Keep the canonical sale journal trigger as the sole automatic sale-ledger authority.
-- No second webhook ledger writer is introduced here.
