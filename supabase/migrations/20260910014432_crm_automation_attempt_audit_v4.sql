...
begin
 ...
 if tg_op='INSERT' then
   ...
   insert into public.automation_execution_attempts(user_id,execution_id,attempt_no,status,started_at,error_message,finished_at)
   values(new.user_id,new.id,n,new.status,coalesce(new.started_at,new.created_at,now()),new.error_message,case when terminal then coalesce(new.completed_at,now()) else null end)
   ...
   n:=greatest(coalesce(new.attempt_count,1),1);
   insert into public.automation_execution_attempts(user_id,execution_id,attempt_no,status,started_at,error_message,finished_at)
   values(new.user_id,new.id,n,new.status,coalesce(new.started_at,now()),new.error_message,case when terminal then coalesce(new.completed_at,now()) else null end)
   on conflict(execution_id,attempt_no) do update set status=excluded.status,error_message=excluded.error_message,finished_at=excluded.finished_at;
 elsif terminal then
