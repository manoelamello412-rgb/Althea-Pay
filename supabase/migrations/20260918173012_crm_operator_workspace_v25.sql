CREATE OR REPLACE FUNCTION public.crm_operator_add_note_v1(p_conversation_id uuid, p_body text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.crm_operator_create_task_v1(p_conversation_id uuid, p_title text, p_description text DEFAULT NULL::text, p_priority text DEFAULT 'normal'::text, p_due_at timestamp with time zone DEFAULT NULL::timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_catalog'
...
CREATE OR REPLACE FUNCTION public.crm_operator_set_tag_v1(p_conversation_id uuid, p_tag_id uuid, p_enabled boolean DEFAULT true)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_catalog'
...
CREATE OR REPLACE FUNCTION public.crm_operator_workspace_v1(p_conversation_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
...
revoke all on function public.crm_operator_add_note_v1(p_conversation_id uuid, p_body text) from public,anon;
grant execute on function public.crm_operator_add_note_v1(p_conversation_id uuid, p_body text) to authenticated;

revoke all on function public.crm_operator_create_task_v1(p_conversation_id uuid, p_title text, p_description text, p_priority text, p_due_at timestamp with time zone) from public,anon;
grant execute on function public.crm_operator_create_task_v1(p_conversation_id uuid, p_title text, p_description text, p_priority text, p_due_at timestamp with time zone) to authenticated;

revoke all on function public.crm_operator_set_tag_v1(p_conversation_id uuid, p_tag_id uuid, p_enabled boolean) from public,anon;
grant execute on function public.crm_operator_set_tag_v1(p_conversation_id uuid, p_tag_id uuid, p_enabled boolean) to authenticated;

revoke all on function public.crm_operator_workspace_v1(p_conversation_id uuid) from public,anon;
grant execute on function public.crm_operator_workspace_v1(p_conversation_id uuid) to authenticated;
