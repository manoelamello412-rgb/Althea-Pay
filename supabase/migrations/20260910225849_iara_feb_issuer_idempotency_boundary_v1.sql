...
begin
  if p_tenant_id is null or nullif(trim(p_idempotency_key),'') is null or nullif(trim(p_tool_key),'') is null or p_tool_version <= 0 or nullif(trim(p_request_hash),'') is null or p_execution_id is null then
    raise exception 'invalid_feb_idempotency_arguments';
  end if;
  begin
