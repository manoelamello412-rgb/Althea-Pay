...
begin
  ...
  from public.gateways where id = trim(p_gateway_id) limit 1;
  ...
  if v_id is null then
    select e.id into v_id from public.gateway_webhook_events e
    where e.provider = v_provider and e.provider_event_id = trim(p_provider_event_id)
    limit 1;
...
create policy gateway_webhook_events_service_only on public.gateway_webhook_events
