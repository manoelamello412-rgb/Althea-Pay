...
begin
  ...
  if not (v_current = v_next or (v_current = 'created' and v_next in ('pending','approved','failed')) or (v_current = 'pending' and v_next in ('approved','failed','refunded','chargeback')) or (v_current = 'approved' and v_next in ('refunded','chargeback'))) then
  ...
  set status = v_next,
      ...
      completed_at = case when v_next in ('approved','refunded','chargeback') then coalesce(t.completed_at, now()) else t.completed_at end,
      updated_at = now()
...
$$;
revoke all on function public.transition_gateway_transaction_status(uuid, uuid, text, text, text) from public;
revoke all on function public.transition_gateway_transaction_status(uuid, uuid, text, text, text) from anon;
revoke all on function public.transition_gateway_transaction_status(uuid, uuid, text, text, text) from authenticated;
