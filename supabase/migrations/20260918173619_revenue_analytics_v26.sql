...
create index if not exists gateway_transactions_org_funnel_created_idx
  on public.gateway_transactions(organization_id,funnel_id,created_at desc)
  where funnel_id is not null;

create index if not exists gateway_transactions_org_gateway_created_idx
  on public.gateway_transactions(organization_id,gateway_id,created_at desc)
  where gateway_id is not null;

create index if not exists checkout_sessions_org_funnel_created_idx
  on public.checkout_sessions(organization_id,funnel_id,created_at desc)
  where funnel_id is not null;

create or replace function public.revenue_analytics_v1(
