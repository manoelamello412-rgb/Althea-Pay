...
begin
  ...
  where user_id=auth.uid() and scope='automation_replay' and idempotency_key=v_key
  for update;
  ...
  values(auth.uid(),auth.uid(),'automation.replay','automation_execution',r.id::text,jsonb_build_object('replay_idempotency_id',v_idem_id,'replay_key',v_key,'reason',p_reason,'replay_count',r.replay_count));
