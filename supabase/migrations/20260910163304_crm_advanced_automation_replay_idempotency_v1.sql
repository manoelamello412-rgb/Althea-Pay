...
begin
  ...
  values(auth.uid(),'automation_replay',v_key,'processing','automation_execution',r.id::text,jsonb_build_object('execution_id',r.id,'reason',p_reason),encode(digest(v_key,'sha256'),'hex')) returning id into v_idem_id;
  update public.automation_executions set status='scheduled',dead_lettered_at=null,error_message=null,next_retry_at=null,scheduled_at=now(),replayed_at=now(),replay_count=coalesce(replay_count,0)+1,cancellation_reason=left(coalesce(p_reason,'manual replay'),1000) where id=r.id returning * into r;
  insert into public.audit_logs(user_id,actor_id,action,resource_type,resource_id,metadata) values(auth.uid(),auth.uid(),'automation.replay','automation_execution',r.id::text,jsonb_build_object('replay_idempotency_id',v_idem_id,'replay_key',v_key,'reason',p_reason,'replay_count',r.replay_count));
  update public.idempotency_keys set status='completed',response_code=200,response_payload=jsonb_build_object('execution_id',r.id,'status',r.status,'replay_count',r.replay_count),resource_id=r.id::text where id=v_idem_id;
  return r;
end;
$function$;
