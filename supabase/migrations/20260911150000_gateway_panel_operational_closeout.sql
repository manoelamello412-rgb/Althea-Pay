...
BEGIN
  ...
  IF NOT public.validate_gateway_credential_schema(r.credential_schema,p_credentials) THEN
    RAISE EXCEPTION 'credential_schema_validation_failed';
  END IF;

  secret_name := 'gateway_credential_bundle:'||u::text||':'||r.provider_key||':'||replace(gen_random_uuid()::text,'-','');
  PERFORM vault.create_secret(p_credentials::text,secret_name,'ALTHEA PAY dynamic gateway credential bundle: '||r.provider_key);
  ...
  VALUES(u,lower(r.provider_key)||':'||lower(regexp_replace(clean_name,'[^a-zA-Z0-9]+','_','g')),null,secret_name,coalesce(p_metadata,'{}'::jsonb),true,1,timezone('utc',now()))
  ON CONFLICT (user_id,gateway_name) DO UPDATE
    SET api_key_encrypted=null,secret_ref=excluded.secret_ref,metadata=excluded.metadata,is_active=true,updated_at=timezone('utc',now())
  RETURNING id INTO c;

  gateway_id_text := 'gw_'||replace(gen_random_uuid()::text,'-','');
  gateway_status := CASE WHEN r.operational THEN 'inactive' ELSE 'inactive' END;
  ...
  IF webhook_secret IS NOT NULL THEN
    ...
    ON CONFLICT (gateway_id) DO UPDATE
      SET user_id=excluded.user_id,secret_ref=excluded.secret_ref,is_active=true,updated_at=timezone('utc',now());
  END IF;
...
CREATE OR REPLACE FUNCTION public.update_dynamic_gateway(
  p_gateway_id text,
  p_display_name text,
  p_environment text,
...
BEGIN
  ...
  IF credentials_supplied THEN
    ...
    UPDATE public.user_gateway_credentials
       SET api_key_encrypted=null,secret_ref=secret_name,is_active=true,updated_at=timezone('utc',now())
     WHERE id=g.credential_id AND user_id=u;
    IF NOT FOUND THEN RAISE EXCEPTION 'credential_not_found'; END IF;

    webhook_secret := nullif(trim(coalesce(p_credentials->>'webhook_secret','')), '');
    IF webhook_secret IS NOT NULL THEN
      PERFORM vault.create_secret(webhook_secret,'gateway_webhook:'||u::text||':'||p_gateway_id||':'||replace(gen_random_uuid()::text,'-',''),'ALTHEA PAY gateway webhook signing secret');
  ...
  UPDATE public.gateways
     SET display_name=clean_name,environment=env,
         status=CASE WHEN status='disabled' THEN 'inactive' ELSE status END
   WHERE id=p_gateway_id AND user_id=u;
...
BEGIN
  ...
  UPDATE public.user_gateway_credentials
     SET is_active=coalesce(p_is_active,false),updated_at=timezone('utc',now())
   WHERE id=p_credential_id AND user_id=u;
  IF NOT FOUND THEN RAISE EXCEPTION 'credential_not_found'; END IF;
  IF coalesce(p_is_active,false) THEN
    IF g.status='disabled' THEN
      UPDATE public.gateways SET status='inactive' WHERE id=g.id AND user_id=u;
    END IF;
  ELSE
    UPDATE public.gateways SET status='disabled' WHERE id=g.id AND user_id=u;
  END IF;
