create or replace function public.crm_predictive_evaluation_summary(p_model_version text default null)
returns jsonb language plpgsql stable security invoker set search_path=public as $$
declare v_user uuid:=auth.uid(); v_total bigint; v_evaluated bigint; v_pending bigint; v_conv_mae numeric; v_rec_mae numeric; v_conv_brier numeric; v_rec_brier numeric;
begin
 ...
 return jsonb_build_object('model_version',p_model_version,'total',v_total,'evaluated',v_evaluated,'pending',v_pending,'conversion_mae',v_conv_mae,'recovery_mae',v_rec_mae,'conversion_brier',v_conv_brier,'recovery_brier',v_rec_brier,'status',case when v_evaluated=0 then 'awaiting_real_outcomes' else 'evaluated' end);
end $$;
