-- Funnel workspace: external provider identifiers are optional at initial creation.
-- The funnel itself is an Althea orchestration object and may be connected to an
-- external funnel later; creation must not require a fabricated external ID.
...
begin
  ...
  select * into v_funnel from public.funnels
   where id=p_funnel_id and user_id=v_user_id and deleted_at is null for update;
  ...
  update public.funnels set nome=trim(p_name), url=nullif(trim(p_url),''), status='active', last_communication=coalesce(last_communication,v_now)
  ...
  update public.funnel_connections set config=coalesce(config,'{}'::jsonb)||v_config,status='active',health_status=coalesce(health_status,'unknown'),updated_at=v_now
