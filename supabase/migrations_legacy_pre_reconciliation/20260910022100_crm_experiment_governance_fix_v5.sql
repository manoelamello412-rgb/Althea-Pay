...
begin
 ...
 if e.status not in ('active','running') then raise exception 'experiment_not_active'; end if;
 ...
 update public.crm_experiments set status='completed',updated_at=now() where id=p_experiment_id and user_id=auth.uid();
