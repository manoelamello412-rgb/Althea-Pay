...
begin
  ...
  select p0.id,p0.name,p0.description,p0.product_type,p0.billing_type,p0.billing_interval,p0.interval_count,p0.unit_amount,p0.currency,p0.metadata into p from public.products p0 where p0.id=o.product_id and p0.organization_id=f.organization_id and p0.deleted_at is null and lower(p0.status)='active' limit 1;
  if not found then raise exception 'CHECKOUT_PRODUCT_NOT_FOUND' using errcode='22023'; end if;
  return jsonb_build_object('funnel',jsonb_build_object('id',f.id,'name',f.nome),'offer',jsonb_build_object('id',o.id,'name',o.name,'price',o.price,'currency',o.currency,'product_id',o.product_id,'type',o.offer_type),'product',jsonb_build_object('id',p.id,'name',p.name,'description',p.description,'product_type',p.product_type,'billing_type',p.billing_type,'billing_interval',p.billing_interval,'interval_count',p.interval_count,'unit_amount',p.unit_amount,'currency',p.currency,'metadata',p.metadata));
end;
$function$;

create or replace function public.create_public_checkout_session(p_funnel_id text,p_offer_id uuid,p_customer jsonb default '{}'::jsonb,p_attribution jsonb default '{}'::jsonb,p_metadata jsonb default '{}'::jsonb,p_idempotency_key text default null)
returns jsonb language plpgsql security definer set search_path = pg_catalog, public as $function$
declare f record; o record; p record; v_existing public.checkout_sessions%rowtype; v_session public.checkout_sessions%rowtype; v_key text:=nullif(trim(p_idempotency_key),''); v_customer jsonb:=coalesce(p_customer,'{}'::jsonb); v_attribution jsonb:=coalesce(p_attribution,'{}'::jsonb); v_metadata jsonb:=coalesce(p_metadata,'{}'::jsonb); v_email text;
begin
  if p_funnel_id is null or length(trim(p_funnel_id))<1 or length(trim(p_funnel_id))>200 then raise exception 'INVALID_FUNNEL_ID' using errcode='22023'; end if;
  ...
  insert into public.checkout_sessions(user_id,funnel_id,product_id,status,currency,amount,customer,attribution,metadata,idempotency_key,organization_id) values(f.user_id,f.id,p.id,'started',upper(o.currency),o.price,v_customer,v_attribution,v_metadata,v_key,f.organization_id) returning * into v_session;
  insert into public.checkout_items(user_id,checkout_id,product_id,name,unit_amount,quantity,kind,organization_id) values(f.user_id,v_session.id,p.id,coalesce(o.name,p.name),o.price,1,'main',f.organization_id);
  insert into public.checkout_events(user_id,checkout_id,event_type,external_id,payload,organization_id) values(f.user_id,v_session.id,'checkout_started','checkout:'||v_session.id,jsonb_build_object('funnel_id',f.id,'offer_id',o.id,'product_id',p.id),f.organization_id) on conflict do nothing;
