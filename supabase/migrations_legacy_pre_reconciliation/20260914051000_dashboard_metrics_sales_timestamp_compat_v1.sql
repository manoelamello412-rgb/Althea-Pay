...
begin
  ...
  measured_at := greatest(measured_at, coalesce((select max(checked_at) from public.gateway_health_snapshots where user_id = uid), '-infinity'::timestamptz));
  measured_at := greatest(measured_at, coalesce((select max(updated_at) from public.checkout_sessions where user_id = uid), '-infinity'::timestamptz));
  if measured_at = '-infinity'::timestamptz then measured_at := now(); end if;
