-- Do not treat a missing email as a shared identity.
-- Chat sessions without an email must create/use a conversation only by explicit
-- conversation_token, never by matching NULL/empty buyer_email.
...
begin
    ...
    if conv is null then
      begin
        insert into public.crm_conversations(
          ...
          new.user_id,new.funnel_id,nullif(p->>'product_id',''),nullif(p->>'transaction_id',''),
          nullif(coalesce(c->>'name',p->>'name',p->>'buyer_name'),''),normalized_email,'open',p
  ...
  if new.event_type='chat_message' then
    ...
    values(
      conv,new.user_id,'inbound','funnel_chat',
