  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (mode === 'login') {
        ...
        setMessage('Login realizado. Abrindo seu painel...'); router.replace('/'); router.refresh()
      } else {
        ...
        if (error) throw error
        if (data.session) { setMessage('Conta criada e acesso liberado. Abrindo seu painel...'); router.replace('/'); router.refresh() }
        else setMessage('Conta criada. Se a confirmação por e-mail estiver ativada, será necessário confirmar o endereço antes de entrar.')
      }
    } catch (err) { setError(friendlyAuthError(err instanceof Error ? err.message : 'Não foi possível concluir a operação.')) }
  ...
  return <main className="auth-shell">
    <section className="auth-card">
      <div className="brand-lockup">
        <Image src="/althea-pay-lockup.svg" alt="Althea Pay" width={294} height={230} priority style={{ width: 'min(100%, 294px)', height: 'auto', objectFit: 'contain', filter: 'none' }} />
      </div>
      ...
      <button type="button" className="auth-switch" onClick={() => { setMode(mode === 'login' ? 'signup' : 'login'); setError(''); setMessage('') }}>{mode === 'login' ? 'Ainda não tenho conta → Criar acesso' : 'Já tenho uma conta → Entrar'}</button>
    </section>
  </main>
}