...
function LoginForm() {
  ...
  return <main className="auth-shell"><section className="auth-card">
    ...
    <button className="auth-switch" onClick={()=>{setMode(mode === 'login' ? 'signup' : 'login');setError('');setMessage('')}}>{mode === 'login'?'Ainda não tenho conta → Criar acesso':'Já tenho uma conta → Entrar'}</button>
  </section></main>
}

export default function LoginPage(){return <Suspense fallback={<main className="auth-shell"><section className="auth-card"><div className="althea-brand auth-brand">ALTHEA PAY<span>Control Center</span></div><div className="auth-heading"><span>ACESSO SEGURO</span><h1>Carregando acesso</h1><p>Preparando o ambiente seguro.</p></div></section></main>}><LoginForm/></Suspense>}
