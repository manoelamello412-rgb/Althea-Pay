...
type PaymentResult = { status: string; id: string }
...
export default function PublicCheckoutPage() {
  ...
  useEffect(() => {
    if (!result?.id || terminalStatuses.has(result.status)) return
    let active = true
    let timer: ReturnType<typeof setTimeout> | null = null
    async function refreshStatus() {
      const { data, error: statusError } = await supabase.rpc('get_checkout_transaction_status', { p_checkout_session_id: result!.id })
      if (!active) return
      if (!statusError && data) {
        ...
        if (statusData?.transaction_id === result!.id && nextStatus) {
  ...
  }, [result?.id, result?.status, supabase])
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setResult({ id: paymentData.transaction.id, status })
