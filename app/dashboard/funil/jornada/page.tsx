...
const paymentMethods = [['pix', 'PIX'], ['card', 'Cartão'], ['boleto', 'Boleto'], ['wallet', 'Carteira'], ['bank_transfer', 'Transferência'], ['other', 'Outra']]
...
export default function FunnelJourneyPage() {
  ...
  const [checkoutStepId, setCheckoutStepId] = useState('')
  const [checkoutOfferId, setCheckoutOfferId] = useState('')
  const [checkoutMethods, setCheckoutMethods] = useState<string[]>(['pix', 'card', 'boleto'])
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = body as Journey
      setJourney(data)
      const configured = data.steps.find((step) => ['checkout', 'payment'].includes(step.step_type) && step.config && typeof step.config.offer_id === 'string')
      if (configured) {
        setCheckoutStepId(configured.id)
        setCheckoutOfferId(String(configured.config?.offer_id ?? ''))
        const configuredMethods = Array.isArray(configured.config?.payment_methods) ? configured.config?.payment_methods.filter((value): value is string => typeof value === 'string') : []
        if (configuredMethods.length) setCheckoutMethods(configuredMethods)
      }
  ...
  const checkoutSteps = useMemo(() => journey?.steps.filter((step) => ['checkout', 'payment'].includes(step.step_type)) ?? [], [journey])
  const primaryGateway = journey?.gateway_bindings.find((item) => item.is_primary && item.status === 'active')
  ...
  async function configureCheckout() {
    if (!checkoutStepId || !checkoutOfferId || checkoutMethods.length === 0 || saving) return
    if (!primaryGateway) { setError('Configure um gateway primário operacional antes de ativar o checkout.'); return }
    setSaving('checkout'); setError(''); setMessage('')
    try {
      const response = await fetch('/api/funnels/journey/checkout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ step_id: checkoutStepId, offer_id: checkoutOfferId, payment_methods: checkoutMethods }), cache: 'no-store' })
      const body = await response.json().catch(() => ({}))
      if (!response.ok) throw new Error(typeof body?.error === 'string' ? body.error : 'Não foi possível configurar o checkout.')
      setMessage('Checkout configurado e persistido. A etapa está pronta para a execução do pagamento.')
      await load()
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Falha ao configurar o checkout.') } finally { setSaving('') }
  }

  function togglePaymentMethod(method: string) {
    setCheckoutMethods((current) => current.includes(method) ? current.filter((item) => item !== method) : [...current, method])
  }
  ...
  return <main className="min-h-screen bg-[#020203] px-4 py-6 text-white">
    <div className="mx-auto max-w-3xl space-y-5">
      ...
      {journey && <>
        ...
        <section className="rounded-2xl border border-white/[0.05] bg-[#0b0b0d] p-4">
          <div className="mb-4 flex items-center justify-between gap-3"><div><h2 className="text-sm font-bold">Checkout operacional</h2><p className="mt-1 text-[9px] text-zinc-600">Vincule a etapa de checkout à oferta real e defina os meios aceitos.</p></div><span className={`rounded-full border px-2 py-1 text-[8px] ${primaryGateway ? 'border-emerald-500/20 bg-emerald-500/10 text-emerald-300' : 'border-amber-500/20 bg-amber-500/10 text-amber-300'}`}>{primaryGateway ? 'GATEWAY PRONTO' : 'GATEWAY NECESSÁRIO'}</span></div>
          {checkoutSteps.length === 0 ? <div className="rounded-xl border border-dashed border-white/[0.08] p-4 text-xs text-zinc-500">Adicione uma etapa do tipo Checkout ou Pagamento para configurar a operação.</div> : <div className="space-y-3">
            <div className="grid gap-2 sm:grid-cols-2"><label className="text-[9px] uppercase tracking-wider text-zinc-500">Etapa<select value={checkoutStepId} onChange={(event) => setCheckoutStepId(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-white/[0.05] bg-[#121214] px-3 text-[10px] text-zinc-300">{checkoutSteps.map((step) => <option key={step.id} value={step.id}>{step.name} · {labels[step.step_type]}</option>)}</select></label><label className="text-[9px] uppercase tracking-wider text-zinc-500">Oferta<select value={checkoutOfferId} onChange={(event) => setCheckoutOfferId(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-white/[0.05] bg-[#121214] px-3 text-[10px] text-zinc-300"><option value="">Selecione uma oferta</option>{journey.offers.map((offer) => <option key={offer.id} value={offer.id}>{offer.name} · {offer.currency} {Number(offer.price).toFixed(2)}</option>)}</select></label></div>
            <div><p className="text-[9px] uppercase tracking-wider text-zinc-500">Meios de pagamento</p><div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3">{paymentMethods.map(([value, label]) => <button key={value} type="button" onClick={() => togglePaymentMethod(value)} className={`rounded-xl border px-3 py-2.5 text-[9px] font-semibold transition ${checkoutMethods.includes(value) ? 'border-emerald-400/30 bg-emerald-400/10 text-emerald-300' : 'border-white/[0.06] text-zinc-500 hover:bg-white/[0.03]'}`}>{label}</button>)}</div></div>
            <button type="button" onClick={() => void configureCheckout()} disabled={!checkoutStepId || !checkoutOfferId || checkoutMethods.length === 0 || saving === 'checkout'} className="flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-emerald-500 px-4 text-[10px] font-bold text-black disabled:cursor-not-allowed disabled:opacity-40">{saving === 'checkout' ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Save className="h-3.5 w-3.5" />} SALVAR CONFIGURAÇÃO DO CHECKOUT</button>
          </div>}
        </section>
