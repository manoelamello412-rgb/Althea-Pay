...
export default function FunnelGatewaySelector({ funnelId }: { funnelId: string }) {
  ...
  async function load() {
    ...
    try {
      ...
      setGateways(rows.filter((item) => ['connected', 'degraded'].includes(String(item.status ?? '').toLowerCase())))
  ...
  return (
    <section className="space-y-3 rounded-2xl border border-emerald-500/10 bg-[#0c0c0e] p-4">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2"><div className="flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-500/10"><Zap className="h-4 w-4 text-emerald-400" /></div><div><h3 className="text-xs font-bold text-zinc-200">Gateway de pagamento</h3><p className="text-[9px] text-zinc-500">Conexão operacional deste funil.</p></div></div>
      ...
      {message && <div className="flex items-center gap-1.5 rounded-lg border border-emerald-500/20 bg-emerald-950/20 p-2 text-[10px] text-emerald-300"><Check className="h-3.5 w-3.5" />{message}</div>}
      {loading ? <div className="flex h-10 items-center justify-center text-[10px] text-zinc-500"><Loader2 className="mr-2 h-3.5 w-3.5 animate-spin" />Sincronizando conexões...</div> : gateways.length === 0 ? <div className="rounded-xl border border-white/[0.04] bg-[#121214] p-3 text-[10px] text-zinc-500">Nenhum gateway validado disponível. Conecte e valide um gateway no módulo Gateways para selecioná-lo aqui.</div> : <>
        <div className="relative"><select value={next} onChange={(event) => setNext(event.target.value)} disabled={saving} className="h-11 w-full appearance-none rounded-xl border border-white/[0.06] bg-[#121214] px-3 pr-9 text-xs text-zinc-200 outline-none focus:border-emerald-500/40"><option value="">Selecione um gateway</option>{gateways.map((gateway) => <option key={gateway.id} value={gateway.id}>{gateway.display_name || gateway.provider || gateway.id} · {gateway.environment || 'default'}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 top-3.5 h-4 w-4 text-zinc-600" /></div>
        {selected && <div className="flex items-center justify-between rounded-xl border border-white/[0.04] bg-[#121214] px-3 py-2"><div><span className="block text-[10px] font-semibold text-zinc-200">{selected.display_name || selected.provider || selected.id}</span><span className="text-[8px] font-mono text-zinc-600">{selected.provider || 'provider-agnostic'} · {selected.environment || 'default'}</span></div><span className="text-[8px] font-bold text-emerald-400">{next === current ? 'ATUAL' : 'NOVA SELEÇÃO'}</span></div>}
        <button type="button" onClick={() => void switchGateway()} disabled={!next || next === current || saving} className="flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-emerald-500 px-4 text-[10px] font-bold text-black hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-35">{saving ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Zap className="h-3.5 w-3.5" />}{saving ? 'TROCANDO...' : 'ALTERAR GATEWAY DO FUNIL'}</button>
      </>}
    </section>
  )
}
