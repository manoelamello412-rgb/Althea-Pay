...
export default function FunnelGatewaySelector({ funnelId }: { funnelId: string }) {
  ...
  return (
    <section className="space-y-3 rounded-2xl border border-emerald-500/10 bg-[#0c0c0e] p-4">
      ...
      {loading ? <div className="flex h-10 items-center justify-center text-[10px] text-zinc-500"><Loader2 className="mr-2 h-3.5 w-3.5 animate-spin" />Sincronizando conexões...</div> : gateways.length === 0 ? <div className="rounded-xl border border-white/[0.04] bg-[#121214] p-3 text-[10px] text-zinc-500">Nenhum gateway operacional conectado. Conecte um gateway no módulo Gateways para selecioná-lo aqui.</div> : <>
