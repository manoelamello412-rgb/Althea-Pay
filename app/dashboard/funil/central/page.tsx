...
type Metrics = { sales: number; revenue: number | null; currency: string; currencyCount: number; hasUnknownCurrency: boolean; events: number; failedEvents: number; approval: number }
...
export default function FunnelCentralPage() {
  ...
  const [metrics, setMetrics] = useState<Metrics>({ sales: 0, revenue: 0, currency: 'BRL', currencyCount: 0, hasUnknownCurrency: false, events: 0, failedEvents: 0, approval: 0 })
  ...
  const load = useCallback(async (preferredId?: string) => {
    ...
    setSelectedId(id)
    if (!id) { setSteps([]); setOffers([]); setAutomations([]); setEvents([]); setMetrics({ sales: 0, revenue: 0, currency: 'BRL', currencyCount: 0, hasUnknownCurrency: false, events: 0, failedEvents: 0, approval: 0 }); return }
    ...
    const hasUnknownCurrency = approved.some((sale) => !sale.currency?.trim())
    const currencies = [...new Set(approved.map((sale) => sale.currency?.trim().toUpperCase()).filter((currency): currency is string => Boolean(currency)))]
    const revenue = !hasUnknownCurrency && currencies.length <= 1 ? approved.reduce((sum, sale) => sum + Number(sale.amount ?? 0), 0) : null
    ...
    setMetrics({ sales: approved.length, revenue, currency: currencies[0] || 'BRL', currencyCount: currencies.length, hasUnknownCurrency, events: Number(connectionResult.data?.event_count ?? eventList.length), failedEvents: Number(connectionResult.data?.error_count ?? eventList.filter((e) => e.status === 'failed' || e.error_message).length), approval: sales.length ? approved.length / sales.length * 100 : 0 })
  }, [selectedId, supabase])
