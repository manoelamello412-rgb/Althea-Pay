...
function Empty({ title, text, href }: { title: string; text: string; href?: string }) { return <div className="rounded-2xl border border-dashed border-white/[0.08] bg-[var(--althea-surface)] p-10 text-center"><Plus className="mx-auto h-6 w-6 text-[var(--althea-brand)]" /><h2 className="mt-4 text-sm font-bold">{title}</h2><p className="mx-auto mt-2 max-w-sm text-xs text-zinc-500">{text}</p>{href && <a href={href} className="mt-5 inline-flex rounded-xl bg-[var(--althea-brand)] px-4 py-2.5 text-xs font-bold text-black">IR PARA CONEXÃO</a>}</div> }
