...
export default function FunnelCentralPage() {
  ...
  return <div className="w-full space-y-5 text-white">
    ...
    <main className="space-y-5">
      ...
      <nav className="flex gap-2 overflow-x-auto pb-1">{tabs.map((item) => <button key={item.id} type="button" onClick={() => setTab(item.id)} className={`shrink-0 rounded-full border px-3 py-2 text-[10px] font-bold ${tab === item.id ? 'border-emerald-500/25 bg-emerald-500/[0.08] text-[var(--althea-brand)]' : 'border-white/[0.06] text-zinc-500 hover:text-zinc-200'}`}>{item.label}</button>)}</nav>
