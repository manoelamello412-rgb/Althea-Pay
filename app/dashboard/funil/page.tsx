  ...
  return (
    <div className="min-h-screen bg-[#020203] pb-28 font-sans text-white antialiased selection:bg-emerald-500 selection:text-black">
      ...
      <main className="mx-auto w-full max-w-md space-y-5 px-4 py-5">
        ...
        {selectedFunnelId && <section className="grid grid-cols-2 gap-2"><button type="button" onClick={() => router.push(`/dashboard/funil/central?funnel=${encodeURIComponent(selectedFunnelId)}`)} className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-white/[0.05] bg-[#0c0c0e] px-3 text-[10px] font-bold text-zinc-300 transition hover:border-emerald-500/25 hover:text-emerald-400"><Activity className="h-3.5 w-3.5" /> CENTRAL COMERCIAL</button><button type="button" onClick={() => router.push(`/dashboard/funil/jornada?funnel=${encodeURIComponent(selectedFunnelId)}`)} className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-white/[0.05] bg-[#0c0c0e] px-3 text-[10px] font-bold text-zinc-300 transition hover:border-emerald-500/25 hover:text-emerald-400"><Layers className="h-3.5 w-3.5" /> JORNADA</button></section>}
