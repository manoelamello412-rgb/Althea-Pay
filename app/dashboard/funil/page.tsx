...
const funnelTypes: Array<{ id: FunnelType; title: string; description: string }> = [
...
export default function FunilDominioPage() {
  ...
  useEffect(() => {
    ...
    setSelectedFunnelId('')
    setFunnelType('sales')
    setCommercial({ productId: '', gatewayId: '' })
    setError('')
    setSuccess('')
  ...
  async function handleSave(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (creatingNewFunnel && !commercial.productId) { setError('Selecione um produto ativo para o funil.'); setSaving(false); return }
    if (creatingNewFunnel && !commercial.gatewayId) { setError('Selecione um gateway operacional para o funil.'); setSaving(false); return }
    ...
    try {
      ...
      if (!funnelId) {
        const response = await fetch('/api/funnels/provision', { method: 'POST', headers: { 'Content-Type': 'application/json' }, cache: 'no-store', body: JSON.stringify({ name, url: url || null, connection_type: method, funnel_type: funnelType, product_id: commercial.productId, gateway_id: commercial.gatewayId }) })
  ...
  return (
    <div className="min-h-screen bg-[#020203] pb-28 font-sans text-white antialiased selection:bg-emerald-500 selection:text-black">
      <main className="mx-auto w-full max-w-6xl space-y-5 px-0 py-2 sm:px-2 sm:py-4 lg:px-0">
        <section className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-2"><div className="flex h-8 w-8 items-center justify-center rounded-lg border border-emerald-500/20 bg-emerald-500/10"><Layers className="h-4 w-4 text-emerald-400" /></div><div><h1 className="text-xl font-bold tracking-tight">Funis</h1><p className="text-[10px] text-zinc-600">Gestão e conexão operacional</p></div></div><div className="flex flex-wrap items-center justify-end gap-1.5">{selectedFunnelId && <><button type="button" onClick={() => router.push(`/dashboard/funil/central?funnel=${encodeURIComponent(selectedFunnelId)}`)} className="flex h-9 shrink-0 items-center gap-1.5 rounded-lg border border-white/[0.06] bg-[#0c0c0e] px-2.5 text-[9px] font-bold text-zinc-300 transition hover:border-emerald-500/25 hover:text-emerald-300"><Activity className="h-3 w-3" /> CENTRAL</button><button type="button" onClick={() => router.push(`/dashboard/funil/jornada?funnel=${encodeURIComponent(selectedFunnelId)}`)} className="flex h-9 shrink-0 items-center gap-1.5 rounded-lg border border-white/[0.06] bg-[#0c0c0e] px-2.5 text-[9px] font-bold text-zinc-300 transition hover:border-emerald-500/25 hover:text-emerald-300"><Zap className="h-3 w-3" /> JORNADA</button></>}<button type="button" onClick={startNewFunnel} className="flex h-9 shrink-0 items-center gap-1.5 whitespace-nowrap rounded-lg border border-emerald-500/25 bg-emerald-500/[0.07] px-3 text-[10px] font-bold text-emerald-400 transition hover:border-emerald-500/50 hover:bg-emerald-500/[0.12]"><Plus className="h-3 w-3" /> NOVO FUNIL</button><button type="button" onClick={() => void refresh()} disabled={refreshing} aria-label="Atualizar" className="flex h-9 w-9 items-center justify-center rounded-lg border border-white/[0.06] bg-[#0c0c0e] text-zinc-400 transition hover:text-white disabled:opacity-50"><RefreshCw className={`h-3.5 w-3.5 ${refreshing ? 'animate-spin' : ''}`} /></button></div></section>
