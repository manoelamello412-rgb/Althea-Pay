  ...
  return (
    <div className="min-h-screen bg-[#020203] pb-28 font-sans text-white antialiased selection:bg-emerald-500 selection:text-black">
      ...
      <main className="mx-auto w-full max-w-md space-y-5 px-4 py-5">
        ...
        {error && <div className="rounded-xl border border-rose-500/25 bg-rose-950/20 p-3 text-xs text-rose-300"><div className="flex items-start gap-2"><X className="mt-0.5 h-4 w-4 shrink-0" /><span>{error}</span></div></div>}
        {success && <div className="rounded-xl border border-emerald-500/20 bg-emerald-950/20 p-3 text-xs text-emerald-300"><div className="flex items-start gap-2"><Check className="mt-0.5 h-4 w-4 shrink-0" /><span>{success}</span></div></div>}

        <form onSubmit={handleSave} className="space-y-4 rounded-2xl border border-white/[0.05] bg-[#0c0c0e] p-4 shadow-xl">
          <div className="flex items-center gap-2 border-b border-white/[0.04] pb-3"><ShieldCheck className="h-4 w-4 text-emerald-400" /><div><h3 className="text-xs font-bold text-zinc-200">{creatingNewFunnel ? 'Identidade e conexão' : 'Configuração operacional'}</h3><p className="text-[9px] font-mono text-zinc-500">Persistência no ambiente do cliente.</p></div></div>
          <label className="block space-y-1"><span className="text-[9px] font-bold uppercase tracking-wider text-zinc-500">Nome do Funil *</span><div className="relative"><Layers className="absolute left-3 top-3 h-3.5 w-3.5 text-zinc-600" /><input required value={funnelName} onChange={(event) => setFunnelName(event.target.value)} placeholder="Ex.: Funil Produto X" className="h-10 w-full rounded-xl border border-white/[0.04] bg-[#121214] pl-9 pr-3 text-xs font-mono text-zinc-200 outline-none focus:border-emerald-500/40" /></div></label>
          <label className="block space-y-1"><span className="text-[9px] font-bold uppercase tracking-wider text-zinc-500">Link da Página</span><div className="relative"><Link2 className="absolute left-3 top-3 h-3.5 w-3.5 text-zinc-600" /><input type="url" value={pageLink} onChange={(event) => setPageLink(event.target.value)} placeholder="https://seusite.com.br" className="h-10 w-full rounded-xl border border-white/[0.04] bg-[#121214] pl-9 pr-3 text-xs font-mono text-zinc-200 outline-none focus:border-emerald-500/40" /></div></label>
          {!creatingNewFunnel && <label className="block space-y-1"><span className="text-[9px] font-bold uppercase tracking-wider text-zinc-500">ID do Funil Externo *</span><div className="relative"><span className="absolute left-3 top-2.5 text-xs font-bold text-zinc-600">#</span><input required value={externalId} onChange={(event) => setExternalId(event.target.value)} placeholder="ID do seu curso/funil" className="h-10 w-full rounded-xl border border-white/[0.04] bg-[#121214] pl-9 pr-3 text-xs font-mono text-zinc-200 outline-none focus:border-emerald-500/40" /></div></label>}
          <div className="space-y-2"><span className="block text-[9px] font-bold uppercase tracking-wider text-zinc-500">Método de Conexão</span><div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => setMethod('script')} className={`flex items-center gap-2 rounded-xl border p-3 text-left transition ${method === 'script' ? 'border-emerald-500/30 bg-emerald-500/[0.06]' : 'border-white/[0.04] bg-[#121214]'}`}><KeyRound className={`h-4 w-4 ${method === 'script' ? 'text-emerald-400' : 'text-zinc-600'}`} /><span><b className="block text-[10px] text-zinc-200">Script</b><small className="text-[8px] text-zinc-600">Credencial de eventos</small></span></button><button type="button" onClick={() => setMethod('webhook')} className={`flex items-center gap-2 rounded-xl border p-3 text-left transition ${method === 'webhook' ? 'border-emerald-500/30 bg-emerald-500/[0.06]' : 'border-white/[0.04] bg-[#121214]'}`}><Webhook className={`h-4 w-4 ${method === 'webhook' ? 'text-emerald-400' : 'text-zinc-600'}`} /><span><b className="block text-[10px] text-zinc-200">Webhook</b><small className="text-[8px] text-zinc-600">Endpoint de eventos</small></span></button></div></div>
          <label className="block space-y-1"><span className="text-[9px] font-bold uppercase tracking-wider text-zinc-500">Pixel / Tracking ID</span><div className="relative"><Activity className="absolute left-3 top-3 h-3.5 w-3.5 text-zinc-600" /><input value={pixelId} onChange={(event) => setPixelId(event.target.value)} placeholder="Opcional" className="h-10 w-full rounded-xl border border-white/[0.04] bg-[#121214] pl-9 pr-3 text-xs font-mono text-zinc-300 outline-none focus:border-emerald-500/40" /></div></label>
          <div className="flex items-center justify-between rounded-xl border border-white/[0.04] bg-[#121214] px-3 py-2.5"><div><span className="block text-[10px] font-semibold text-zinc-200">Chat do Funil</span><span className="text-[8px] text-zinc-600">Atendimento centralizado no CRM.</span></div><button type="button" role="switch" aria-checked={chatEnabled} onClick={() => setChatEnabled((value) => !value)} className={`h-5 w-9 rounded-full p-0.5 transition ${chatEnabled ? 'bg-emerald-500' : 'bg-zinc-800'}`}><span className={`block h-4 w-4 rounded-full bg-white shadow transition-transform ${chatEnabled ? 'translate-x-4' : 'translate-x-0'}`} /></button></div>
          <div className="flex gap-2"><button type="submit" disabled={saving || !funnelName.trim()} className="flex h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-emerald-500 px-4 text-xs font-bold text-black transition hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-40">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Zap className="h-4 w-4" />}{saving ? 'SALVANDO...' : creatingNewFunnel ? 'CRIAR E ATIVAR' : 'SALVAR E ATIVAR'}</button>{creatingNewFunnel && <button type="button" onClick={cancelNewFunnel} disabled={saving} className="h-11 rounded-xl border border-white/[0.06] bg-[#121214] px-4 text-xs font-bold text-zinc-400 hover:text-white">CANCELAR</button>}</div>
        </form>
