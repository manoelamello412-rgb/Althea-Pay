    ...
    <section className="space-y-3 rounded-2xl border border-white/[.055] bg-[var(--althea-surface)] p-5">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div><h3 className="text-xs font-bold text-white">Comercial do Funil</h3><p className="text-[9px] text-[var(--althea-muted)]">Produto e gateway são obrigatórios para ativação.</p></div>
        </div>
      ...
      {loading ? <div className="flex h-16 items-center justify-center text-[10px] text-[var(--althea-muted)]"><Loader2 className="mr-2 h-3.5 w-3.5 animate-spin" />Carregando opções comerciais...</div> : <>
        <label className="block space-y-1.5"><span className="text-[9px] font-bold uppercase tracking-wider text-[var(--althea-muted)]">Produto *</span><div className="relative"><select required value={value.productId} onChange={(event) => onChange({ ...value, productId: event.target.value })} className="h-11 w-full appearance-none rounded-xl border border-white/[.055] bg-[var(--althea-bg)] px-3 pr-9 text-xs text-white outline-none focus:border-[rgba(29,184,84,.32)]"><option value="">Selecione o produto ativo</option>{products.map((product) => <option key={product.id} value={product.id}>{product.name} · {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: product.currency }).format(Number(product.unit_amount))}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 top-3.5 h-4 w-4 text-[#5f6e66]" /></div></label>
        ...
        <label className="block space-y-1.5"><span className="text-[9px] font-bold uppercase tracking-wider text-[var(--althea-muted)]">Gateway de pagamento *</span><div className="relative"><select required value={value.gatewayId} onChange={(event) => onChange({ ...value, gatewayId: event.target.value })} className="h-11 w-full appearance-none rounded-xl border border-white/[.055] bg-[var(--althea-bg)] px-3 pr-9 text-xs text-white outline-none focus:border-[rgba(29,184,84,.32)]"><option value="">Selecione o gateway conectado</option>{gateways.map((gateway) => <option key={gateway.id} value={gateway.id}>{gateway.display_name || gateway.provider || gateway.id} · {gateway.environment || 'default'}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 top-3.5 h-4 w-4 text-[#5f6e66]" /></div></label>
        ...
        {gateways.length === 0 && <div className="rounded-xl border border-amber-500/15 bg-amber-950/10 p-3 text-[10px] text-amber-300">Nenhum gateway operacional disponível. Conecte um gateway antes de criar o funil.</div>}
        <div className={`flex items-center gap-2 rounded-xl border p-2.5 text-[9px] ${ready ? 'border-[rgba(29,184,84,.14)] bg-[rgba(29,184,84,.045)] text-[#8edca5]' : 'border-white/[.045] bg-[var(--althea-bg)] text-[var(--althea-muted)]'}`}><span className={`h-1.5 w-1.5 rounded-full ${ready ? 'bg-[var(--althea-brand)]' : 'bg-zinc-700'}`} />{ready ? 'Produto + Gateway selecionados. O vínculo será criado atomicamente com o funil.' : 'Selecione Produto e Gateway para habilitar a criação.'}</div>
      </>}
    </section>
  )
}
