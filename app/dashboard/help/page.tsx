...
export default function HelpPage() {
  ...
  return <div className="w-full space-y-5 text-slate-100">
      <header className="mb-8"><div className="mb-2 flex items-center gap-2 text-[10px] font-black uppercase tracking-[.28em] text-[var(--althea-brand)]"><BookOpen size={14} /> ALTHEA PAY // CENTRAL DE AJUDA</div><h1 className="text-3xl font-black tracking-tight">Central de ajuda</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Referência operacional para entender como os módulos se conectam sem duplicar infraestrutura.</p></header>
      <label className="mb-6 flex h-12 max-w-2xl items-center gap-3 rounded-xl border border-white/[.055] bg-[var(--althea-surface)] px-4 text-slate-500"><Search size={17} /><input value={query} onChange={event => setQuery(event.target.value)} className="w-full bg-transparent text-sm text-slate-100 outline-none placeholder:text-slate-600" placeholder="Buscar assunto, módulo ou operação..." /></label>
      <section className="mb-6 grid gap-3 md:grid-cols-3"><a href="/dashboard/gateways" className="rounded-2xl border border-white/[.055] bg-[var(--althea-surface)] p-5 transition hover:bg-white/[.05]"><Zap size={18} className="mb-4 text-[var(--althea-brand)]" /><b className="block">Gateways</b><span className="mt-1 block text-xs text-slate-500">Conexões e saúde dos processadores externos.</span></a><a href="/dashboard/api" className="rounded-2xl border border-white/[.055] bg-[var(--althea-surface)] p-5 transition hover:bg-white/[.05]"><ExternalLink size={18} className="mb-4 text-[var(--althea-brand)]" /><b className="block">API</b><span className="mt-1 block text-xs text-slate-500">Credenciais, escopos e observabilidade de requests.</span></a><a href="/dashboard/security" className="rounded-2xl border border-white/[.055] bg-[var(--althea-surface)] p-5 transition hover:bg-white/[.05]"><ShieldCheck size={18} className="mb-4 text-[var(--althea-brand)]" /><b className="block">Segurança</b><span className="mt-1 block text-xs text-slate-500">Autenticação, senha e MFA da conta.</span></a></section>
      <section className="overflow-hidden rounded-2xl border border-white/[.055] bg-white/[.02]">{filtered.length === 0 ? <div className="p-8 text-center text-sm text-slate-500">Nenhum artigo encontrado.</div> : filtered.map(article => <article key={article.title} className="border-b border-white/[.06] last:border-0"><button type="button" onClick={() => setOpen(current => current === article.title ? null : article.title)} className="flex w-full items-center justify-between gap-4 px-5 py-5 text-left"><b>{article.title}</b><ChevronDown size={17} className={open === article.title ? 'rotate-180 transition' : 'transition'} /></button>{open === article.title && <p className="max-w-4xl px-5 pb-5 text-sm leading-7 text-slate-400">{article.text}</p>}</article>)}</section>
  </div>
}
