...
export default function GatewaysManagementPage() {
  ...
  const loadGatewayTelemetry = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.from('gateways').select('id,name,provider,status,priority,response_time_ms').eq('user_id', userId).order('priority', { ascending: true });
      ...
      const responseTimes = rows.map((row) => Number(row.response_time_ms)).filter((value) => Number.isFinite(value) && value >= 0);
      setGlobalResponseTime(responseTimes.length ? Math.round(responseTimes.reduce((sum, value) => sum + value, 0) / responseTimes.length) : null);
  ...
  useEffect(() => {
    ...
    const subscribe = async () => {
      ...
      channel = supabase.channel(`gateway-telemetry-${userId}`).on('postgres_changes', { event: '*', schema: 'public', table: 'gateways', filter: `user_id=eq.${userId}` }, (payload) => {
        ...
        appendLog({ id: `gateway-${row.id ?? provider}-${Date.now()}`, timestamp: formatTime(new Date()), provider, message: `STATUS -> ${statusLabel(nextStatus)}`, type: nextStatus === 'INDISPONÍVEL' ? 'CRITICAL' : nextStatus === 'FAILOVER_ATIVO' ? 'WARNING' : 'SUCCESS' });
      }).subscribe();
    };
    void subscribe();
    ...
    <div className="min-h-screen bg-[#020203] text-white font-sans antialiased selection:bg-emerald-500 selection:text-black">
      ...
      <main className="mx-auto w-full max-w-md px-5 py-6 pb-32 space-y-7">
        <section className="space-y-2">
          ...
          <p className="text-xs text-zinc-400"><span className="mr-1.5 inline-block h-1.5 w-1.5 rounded-full bg-zinc-500 align-middle" />Tempo de resposta médio global: <span className="font-mono font-semibold text-emerald-400">{globalResponseTime === null ? '—' : `${globalResponseTime}ms`}</span></p>
        </section>
        ...
        <section className="space-y-3">
          ...
          <div className="overflow-hidden rounded-xl border border-white/[0.12] bg-[#0a0a0c] divide-y divide-white/[0.08]">
            {loading ? <div className="px-4 py-8 text-center text-xs text-zinc-500">Carregando Gateways...</div> : gateways.length === 0 ? <div className="px-4 py-8 text-center text-xs text-zinc-500">Nenhum Gateway conectado. Conecte um Gateway para configurar a fila de prioridade.</div> : gateways.map((gateway, index) => (
              <div key={gateway.provider} className="flex min-h-[82px] items-center justify-between gap-3 px-4 py-4">
                <div className="flex min-w-0 items-center gap-3"><span className="w-4 shrink-0 font-mono text-sm text-emerald-400">{index + 1}.</span><div className="min-w-0"><div className="flex flex-wrap items-center gap-x-4 gap-y-1"><span className="font-semibold text-[18px] text-zinc-100">{gateway.provider}</span><span className={`font-mono text-[10px] ${gateway.status === 'INDISPONÍVEL' ? 'text-rose-400' : gateway.status === 'FAILOVER_ATIVO' ? 'text-amber-400' : 'text-emerald-400'}`}>{gateway.roleDescription}</span></div></div></div>
