...
export default function PagamentosPage() {
  ...
  const metrics = useMemo(() => ({ total: payments.reduce((sum, payment) => sum + Number(payment.amount || 0), 0), approved: payments.filter((payment) => payment.status === 'approved').length, pending: payments.filter((payment) => payment.status === 'pending').length, exceptions: payments.filter((payment) => ['failed', 'chargeback'].includes(payment.status)).length }), [payments])
  ...
  return <main className="min-h-screen bg-[#070809] px-4 py-6 text-white md:px-8 lg:px-10"><div className="mx-auto w-full max-w-[1500px] space-y-6">
    ...
    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[['Volume carregado', money(metrics.total, 'BRL')], ['Aprovados', String(metrics.approved)], ['Pendentes', String(metrics.pending)], ['Exceções', String(metrics.exceptions)]].map(([label, value]) => <article key={label} className="rounded-2xl border border-white/10 bg-white/[0.035] p-5"><p className="text-xs uppercase tracking-wider text-white/40">{label}</p><strong className="mt-2 block text-2xl font-semibold">{value}</strong></article>)}</section>
