...
export default function AnalyticsPage() {
  ...
  const metrics = useMemo(() => {
    ...
    const currencies = [...new Set(approved.map((t) => t.currency?.trim().toUpperCase() || 'BRL'))]
    const gross = currencies.length === 1 ? approved.reduce((sum, t) => sum + Number(t.amount ?? 0), 0) : null
    ...
    const average = gross != null && approved.length ? gross / approved.length : null
    return { approved, pending, failed, gross, conversion, average, currency: currencies[0] || 'BRL', currencyCount: currencies.length }
  }, [transactions, checkouts])

  const funnelRows = useMemo(() => {
    const map = new Map<string, { total: number; approved: number; gross: number; currencies: Set<string> }>()
    for (const t of transactions) {
      const key = t.funnel_id || 'sem_funil'
      const row = map.get(key) ?? { total: 0, approved: 0, gross: 0, currencies: new Set<string>() }
      row.total++
      if (t.status === 'approved') {
        row.approved++
        row.gross += Number(t.amount ?? 0)
        row.currencies.add(t.currency?.trim().toUpperCase() || 'BRL')
      }
      map.set(key, row)
    }
    return [...map.entries()].sort((a, b) => b[1].approved - a[1].approved).slice(0, 8)
  }, [transactions])
