...
export default function PerfilSettingsPage() {
  ...
  async function handleSave(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (newPassword) {
        const { error: reauthError } = await db.auth.signInWithPassword({ email: currentEmail, password: currentPassword })
        if (reauthError) throw new Error('A senha atual não pôde ser validada.')
      }

      const { error: updateError } = await db.auth.updateUser({
        ...(cleanEmail !== currentEmail ? { email: cleanEmail } : {}),
        data: { ...user.user_metadata, full_name: cleanName, name: cleanName },
