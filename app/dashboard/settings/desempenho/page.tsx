...
function statusMeta(status: HealthState) {
  switch (status) {
    case 'operational': return { label: 'SISTEMA OPERACIONAL', className: 'border-[#1DB854]/20 bg-[#1DB854]/10 text-[var(--althea-brand)]', icon: CheckCircle2 }
    ...
    default: return { label: 'COLETANDO DADOS', className: 'border-white/[0.06] bg-[var(--althea-surface)] text-[var(--althea-muted)]', icon: Activity }
  }
}
  ...
  return (
    <div className="w-full space-y-5">
      <section className="flex flex-col gap-5 border-b border-white/[.055] pb-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[.2em] text-[var(--althea-brand)]">Observabilidade</p>
          <h1 className="mt-2 text-[30px] font-semibold tracking-[-.04em] text-white sm:text-[34px]">Desempenho</h1>
          <p className="mt-1 max-w-2xl text-xs leading-5 text-[var(--althea-muted)]">Telemetria real de borda, adquirência e estabilidade observada na operação.</p>
        </div>
        <button type="button" onClick={() => router.push('/dashboard/settings')} className="inline-flex h-10 items-center gap-2 self-start rounded-xl border border-white/[.06] bg-[var(--althea-surface)] px-4 text-[10px] font-semibold text-[var(--althea-muted)] transition hover:text-white lg:self-auto">← Configurações</button>
      </section>
        <section className="rounded-2xl border border-white/[.055] bg-[var(--althea-surface)] p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-[rgba(29,184,84,.12)] bg-[rgba(29,184,84,.055)] text-[var(--althea-brand)]"><ServerCog size={18} aria-hidden="true" /></span><div><h1 className="text-lg font-bold tracking-tight text-white">Saúde da Plataforma</h1><p className="text-[11px] text-[var(--althea-muted)]">Telemetria real de borda e roteamento transacional.</p></div></div><div className={`inline-flex min-h-9 items-center gap-2 self-start rounded-full border px-3 text-[10px] font-bold tracking-wide ${meta.className}`}><StatusIcon size={13} aria-hidden="true" />{meta.label}</div></div><div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2"><Metric label="Latência de Borda" value={telemetry.edgeLatencyMs} suffix="ms" description="Tempo medido do cliente até a resposta da rota de health."/><Metric label="Latência de Adquirentes" value={telemetry.acquirerLatencyMs} suffix="ms" description="Média dos tempos observados nos logs reais de roteamento."/></div></section>
        ...
        <section className="rounded-2xl border border-white/[.055] bg-[var(--althea-surface)] p-5"><div className="flex items-center justify-between gap-3"><div><div className="flex items-center gap-2"><Clock3 size={14} className="text-[var(--althea-muted)]" aria-hidden="true"/><h2 className="text-sm font-semibold text-white">Histórico de Latência</h2></div><p className="mt-1 text-[10px] text-[var(--althea-muted)]">Últimos probes realizados nesta sessão.</p></div><button type="button" disabled={refreshing} onClick={() => void refresh()} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-white/[0.06] bg-[var(--althea-bg)] px-3 text-[10px] font-mono text-[var(--althea-muted)] transition hover:text-white disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1DB854]/40">{refreshing ? <Loader2 size={13} className="animate-spin" aria-hidden="true"/> : <RefreshCw size={13} aria-hidden="true"/>} ATUALIZAR</button></div><div className="mt-4 overflow-x-auto rounded-xl border border-white/[0.06] bg-[var(--althea-bg)]"><div className="grid min-w-[420px] grid-cols-[1fr_auto_auto] gap-3 border-b border-white/[0.06] px-4 py-2 text-[9px] font-mono uppercase tracking-wider text-[var(--althea-muted)]"><span>Sequência</span><span>Resposta</span><span>Status</span></div>{pings.length === 0 ? <div className="p-6 text-center text-[11px] text-[var(--althea-muted)]">Aguardando telemetria.</div> : pings.map((ping, index) => <div key={ping.id} className="grid min-w-[420px] grid-cols-[1fr_auto_auto] items-center gap-3 border-b border-white/[0.04] px-4 py-2.5 text-[10px] font-mono last:border-0"><span className="truncate text-[var(--althea-muted)]">PULSE_{String(index + 1).padStart(2, '0')} · {new Date(ping.at).toLocaleTimeString('pt-BR')}</span><span className={ping.latency > 300 ? 'font-semibold text-[#D4AF37]' : 'text-white'}>{ping.latency} ms</span><span className={ping.ok ? 'text-[var(--althea-brand)]' : 'text-rose-400'}>{ping.ok ? 'OK' : 'FALHA'}</span></div>)}</div><div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-[10px] font-mono text-[var(--althea-muted)]"><span>ÚLTIMA VARREDURA · {lastCheck}</span><span>{logs.length} logs transacionais carregados</span></div></section>
        {error && <div className="rounded-xl border border-rose-500/20 bg-rose-500/5 p-3 text-[11px] text-rose-300" role="alert">{error}</div>}
        {loading && <div className="flex items-center justify-center gap-2 py-8 text-xs text-[var(--althea-muted)]" role="status"><Loader2 size={14} className="animate-spin" aria-hidden="true"/> Inicializando telemetria…</div>}
    </div>
  )
}

function Metric({ label, value, suffix, description }: { label: string; value: number | null; suffix: string; description: string }) { return <div className="rounded-xl border border-white/[0.06] bg-[var(--althea-bg)] p-4"><span className="text-[10px] font-mono uppercase tracking-wider text-[var(--althea-muted)]">{label}</span><div className="mt-2 flex items-baseline gap-1.5"><span className="text-3xl font-bold tracking-tight text-white">{value === null ? '—' : value}</span><span className="text-xs font-mono text-[var(--althea-muted)]">{value === null ? '' : suffix}</span></div><p className="mt-2 text-[10px] leading-4 text-[var(--althea-muted)]">{description}</p></div> }
function MetricCard({ label, value, icon, description }: { label: string; value: string; icon: React.ReactNode; description: string }) { return <div className="rounded-2xl border border-white/[.055] bg-[var(--althea-surface)] p-4"><div className="flex items-center gap-2 text-[10px] font-mono uppercase tracking-wider text-[var(--althea-muted)]">{icon}{label}</div><p className="mt-2 text-2xl font-bold tracking-tight text-white">{value}</p><p className="mt-1 text-[10px] leading-4 text-[var(--althea-muted)]">{description}</p></div> }