...
function statusMeta(status: HealthState) {
  switch (status) {
    ...
    default: return { label: 'COLETANDO DADOS', className: 'border-white/[0.06] bg-[#0F1A16] text-[var(--althea-muted)]', icon: Activity }
  }
}
...
function Metric({ label, value, suffix, description }: { label: string; value: number | null; suffix: string; description: string }) { return <div className="rounded-xl border border-white/[0.06] bg-[#0B0B0D] p-4"><span className="text-[10px] font-mono uppercase tracking-wider text-[var(--althea-muted)]">{label}</span><div className="mt-2 flex items-baseline gap-1.5"><span className="text-3xl font-bold tracking-tight text-white">{value === null ? '—' : value}</span><span className="text-xs font-mono text-[var(--althea-muted)]">{value === null ? '' : suffix}</span></div><p className="mt-2 text-[10px] leading-4 text-[var(--althea-muted)]">{description}</p></div> }
