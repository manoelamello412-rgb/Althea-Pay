...
export default function EmpresaSettingsPage() {
  ...
  return <div className="w-full space-y-5 text-white">
    ...
    <form onSubmit={handleSave} className="space-y-4">
      ...
      <section className="rounded-2xl border border-white/[.055] bg-[var(--althea-surface)] p-4 sm:p-5"><SectionHeading eyebrow="Ledger operacional" title="Faturamento & retenções" description="Indicadores de leitura preservados no perfil operacional." /><div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3"><Metric label="Faturamento mensal" value={formatCurrency(metrics.monthly_billing_cents)} /><Metric label="Impostos retidos" value={formatCurrency(metrics.tax_withheld_cents)} /><Metric label="Categoria operacional" value={metrics.current_tier ?? '—'} /></div><div className="mt-4 flex items-start gap-3 rounded-xl border border-white/[.045] bg-[var(--althea-bg)] p-3 text-[11px] leading-5 text-[var(--althea-muted)]"><ShieldCheck size={15} className="mt-0.5 shrink-0 text-[var(--althea-brand)]" /><p>Identificadores fiscais ficam protegidos após o primeiro cadastro. Alterações homologadas devem passar por fluxo administrativo específico.</p></div></section>
    ...
    <div className="mt-6 flex flex-col gap-2 text-[10px] text-zinc-600 sm:flex-row sm:items-center sm:justify-between"><span className="inline-flex items-center gap-1.5"><LockKeyhole size={12} />Identificadores fiscais protegidos após homologação</span><span>Última atualização: {formatUpdatedAt(profile?.updated_at ?? '')}</span><button type="button" onClick={() => void loadCompany()} className="inline-flex min-h-11 items-center gap-1.5 self-start rounded-lg px-2 text-[var(--althea-muted)] hover:text-white"><RefreshCw size={12} />Atualizar</button></div>
  </div>
}

function SectionHeading({ eyebrow, title, description }: { eyebrow: string; title: string; description: string }) { return <div><span className="text-[10px] font-bold uppercase tracking-[0.16em] text-[var(--althea-brand)]">{eyebrow}</span><h2 className="mt-1 text-sm font-semibold text-white">{title}</h2><p className="mt-1 text-[11px] text-[var(--althea-muted)]">{description}</p></div> }
function Field({ label, value, onChange, placeholder, type = 'text', required = false }: { label: string; value: string; onChange: (value: string) => void; placeholder: string; type?: 'text' | 'email'; required?: boolean }) { return <label className="block space-y-1.5"><span className="text-[10px] font-bold uppercase tracking-wider text-[var(--althea-muted)]">{label}</span><input required={required} type={type} value={value} onChange={event => onChange(event.target.value)} placeholder={placeholder} className="min-h-11 w-full rounded-xl border border-white/[.045] bg-[var(--althea-bg)] px-3 text-xs text-white outline-none placeholder:text-zinc-700 focus:border-[#1DB854]/50" /></label> }
function FiscalField({ label, value, locked, placeholder, onChange }: { label: string; value: string; locked: boolean; placeholder: string; onChange: (value: string) => void }) { if (!locked) return <Field label={label} value={value} onChange={onChange} placeholder={placeholder} />; return <div className="space-y-1.5"><span className="text-[10px] font-bold uppercase tracking-wider text-[var(--althea-muted)]">{label}</span><div className="flex min-h-11 items-center justify-between gap-3 rounded-xl border border-white/[.045] bg-[var(--althea-bg)] px-3 text-xs text-[var(--althea-muted)]"><span className="truncate font-mono">{maskDocument(value) || 'Homologado'}</span><span className="inline-flex shrink-0 items-center gap-1 text-[9px] uppercase tracking-wider text-zinc-600"><LockKeyhole size={11} />Protegido</span></div></div> }
function Metric({ label, value }: { label: string; value: string }) { return <div className="rounded-xl border border-white/[.045] bg-[var(--althea-bg)] p-3"><span className="text-[9px] font-bold uppercase tracking-wider text-zinc-600">{label}</span><strong className="mt-2 block text-sm text-white">{value}</strong></div> }
