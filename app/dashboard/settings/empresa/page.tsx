...
export default function EmpresaSettingsPage() {
  ...
  if (loading) return <div className="min-h-64 animate-pulse rounded-2xl border border-[#191921] bg-[#0b0b0f]" />
  ...
  return <div className="min-h-full pb-32 font-['Space_Grotesk'] text-white">
    <header className="mb-6 flex items-start justify-between gap-4"><div className="flex min-w-0 items-start gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-[#191921] bg-[#0b0b0f] text-[#1DB854]"><Building2 size={18} /></div><div><h1 className="text-xl font-bold tracking-tight">Minha Empresa</h1><p className="mt-1 max-w-2xl text-xs leading-5 text-zinc-500">Identidade jurídica e fiscal da operação, usando o cadastro comercial canônico.</p></div></div><button type="button" onClick={() => router.push('/dashboard/settings')} className="min-h-11 rounded-xl border border-[#191921] bg-[#0b0b0f] px-3 text-xs font-semibold text-zinc-300">Voltar</button></header>
    <form onSubmit={handleSave} className="space-y-4">
      <section className="rounded-2xl border border-[#191921] bg-[#0b0b0f] p-4 sm:p-5"><SectionHeading eyebrow="Identidade corporativa" title="Dados cadastrais" description="Informações comerciais utilizadas pela operação." /><div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2"><Field label="Razão Social" value={legalName} onChange={setLegalName} placeholder="Razão social" required /><Field label="Nome Fantasia" value={tradeName} onChange={setTradeName} placeholder="Nome comercial" /><FiscalField label="CNPJ / CPF" value={documentId} locked={documentLocked} placeholder="CPF ou CNPJ" onChange={setDocumentId} /><FiscalField label="Inscrição Estadual" value={stateRegistration} locked={ieLocked} placeholder="Número ou isento" onChange={setStateRegistration} /></div><div className="mt-4"><Field label="E-mail fiscal e de repasses" type="email" value={billingEmail} onChange={setBillingEmail} placeholder="financeiro@empresa.com" required /></div></section>
      <section className="rounded-2xl border border-[#191921] bg-[#0b0b0f] p-4 sm:p-5"><SectionHeading eyebrow="Ledger operacional" title="Faturamento & retenções" description="Indicadores de leitura preservados no perfil operacional." /><div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3"><Metric label="Faturamento mensal" value={formatCurrency(metrics.monthly_billing_cents)} /><Metric label="Impostos retidos" value={formatCurrency(metrics.tax_withheld_cents)} /><Metric label="Categoria operacional" value={metrics.current_tier ?? '—'} /></div><div className="mt-4 flex items-start gap-3 rounded-xl border border-[#191921] bg-[#060608] p-3 text-[11px] leading-5 text-zinc-500"><ShieldCheck size={15} className="mt-0.5 shrink-0 text-[#1DB854]" /><p>Identificadores fiscais ficam protegidos após o primeiro cadastro. Alterações homologadas devem passar por fluxo administrativo específico.</p></div></section>
      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="min-h-5 text-[11px]" aria-live="polite">{error && <p className="rounded-xl border border-red-900/40 bg-red-950/20 px-3 py-2 text-red-300" role="alert">{error}</p>}{message && !error && <p className="flex items-center gap-1.5 text-[#1DB854]" role="status"><Check size={13} />{message}</p>}</div><button type="submit" disabled={saving} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#1DB854] px-5 text-xs font-bold text-black disabled:opacity-60 sm:w-auto">{saving ? <><Loader2 size={14} className="animate-spin" />Salvando...</> : <><Check size={14} />Salvar dados da empresa</>}</button></div>
    </form>
...
function SectionHeading({ eyebrow, title, description }: { eyebrow: string; title: string; description: string }) { return <div><span className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#1DB854]">{eyebrow}</span><h2 className="mt-1 text-sm font-semibold text-white">{title}</h2><p className="mt-1 text-[11px] text-zinc-500">{description}</p></div> }
function Field({ label, value, onChange, placeholder, type = 'text', required = false }: { label: string; value: string; onChange: (value: string) => void; placeholder: string; type?: 'text' | 'email'; required?: boolean }) { return <label className="block space-y-1.5"><span className="text-[10px] font-bold uppercase tracking-wider text-zinc-400">{label}</span><input required={required} type={type} value={value} onChange={event => onChange(event.target.value)} placeholder={placeholder} className="min-h-11 w-full rounded-xl border border-[#191921] bg-[#060608] px-3 text-xs text-white outline-none placeholder:text-zinc-700 focus:border-[#1DB854]/50" /></label> }
function FiscalField({ label, value, locked, placeholder, onChange }: { label: string; value: string; locked: boolean; placeholder: string; onChange: (value: string) => void }) { if (!locked) return <Field label={label} value={value} onChange={onChange} placeholder={placeholder} />; return <div className="space-y-1.5"><span className="text-[10px] font-bold uppercase tracking-wider text-zinc-400">{label}</span><div className="flex min-h-11 items-center justify-between gap-3 rounded-xl border border-[#191921] bg-[#060608] px-3 text-xs text-zinc-500"><span className="truncate font-mono">{maskDocument(value) || 'Homologado'}</span><span className="inline-flex shrink-0 items-center gap-1 text-[9px] uppercase tracking-wider text-zinc-600"><LockKeyhole size={11} />Protegido</span></div></div> }
function Metric({ label, value }: { label: string; value: string }) { return <div className="rounded-xl border border-[#191921] bg-[#060608] p-3"><span className="text-[9px] font-bold uppercase tracking-wider text-zinc-600">{label}</span><strong className="mt-2 block text-sm text-zinc-200">{value}</strong></div> }
