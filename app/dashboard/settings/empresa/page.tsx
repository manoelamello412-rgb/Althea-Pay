...
export default function EmpresaSettingsPage() {
  ...
  async function handleSave(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      if (!currentDocumentDigits && documentDigits) setDocumentId(documentDigits)
      ...
      <header className="mb-6 flex items-start justify-between gap-4">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <h1 className="text-xl font-bold tracking-tight">Minha Empresa</h1>
            <p className="mt-1 max-w-2xl text-xs leading-5 text-zinc-500">Gestão centralizada da identidade jurídica, fiscal e de faturamento da operação.</p>
          </div>
        </div>
      ...
      <form onSubmit={handleSave} className="space-y-4">
        <section className="rounded-2xl border border-[#191921] bg-[#0b0b0f] p-4 sm:p-5">
          ...
          <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <ProtectedField label="CNPJ / CPF" value={documentId ? maskDocument(documentId) : 'Não homologado'} locked={documentLocked} />
            <ProtectedField label="Inscrição Estadual" value={stateRegistration || 'Não homologada / isento'} locked={ieLocked} />
          </div>
          <div className="mt-4">
            <Field label="E-mail fiscal e de repasses" type="email" value={billingEmail} onChange={setBillingEmail} placeholder="financeiro@empresa.com" required />
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-[#191921] bg-[#0b0b0f] p-4 sm:p-5">
          ...
          <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
            <Metric label="Faturamento mensal" value={formatCurrency(metrics.monthly_billing_cents ?? 0)} />
            <Metric label="Impostos retidos" value={formatCurrency(metrics.tax_withheld_cents ?? 0)} />
            <Metric label="Categoria operacional" value={metrics.current_tier ?? 'Standard'} />
          </div>
          <div className="mt-4 flex items-start gap-3 rounded-xl border border-[#191921] bg-[#060608] p-3 text-[11px] leading-5 text-zinc-500">
            <ShieldCheck size={15} className="mt-0.5 shrink-0 text-[#1DB854]" />
            <p>Os indicadores são somente leitura. Alterações de enquadramento fiscal ou identidade homologada exigem revisão documental e não podem ser realizadas por este painel.</p>
          </div>
        </section>

        <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-h-5 text-[11px]" aria-live="polite">
            {error && <p className="rounded-xl border border-red-900/40 bg-red-950/20 px-3 py-2 text-red-300" role="alert">{error}</p>}
            {message && !error && <p className="flex items-center gap-1.5 text-[#1DB854]" role="status"><Check size={13} />{message}</p>}
          </div>
          <button type="submit" disabled={saving || !tenant} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#1DB854] px-5 text-xs font-bold text-black transition hover:brightness-105 active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto">
            {saving ? <><Loader2 size={14} className="animate-spin" />Salvando...</> : <><Check size={14} />Salvar dados da empresa</>}
          </button>
        </div>
      </form>

      <div className="mt-6 flex flex-col gap-2 text-[10px] text-zinc-600 sm:flex-row sm:items-center sm:justify-between">
        <span className="inline-flex items-center gap-1.5"><LockKeyhole size={12} />Identificadores fiscais protegidos</span>
        <span>Última atualização: {formatUpdatedAt(tenant?.updated_at ?? '')}</span>
        <button type="button" onClick={() => void loadCompany()} className="inline-flex min-h-11 items-center gap-1.5 self-start rounded-lg px-2 text-zinc-500 hover:text-white"><RefreshCw size={12} />Atualizar</button>
      </div>
    </div>
  )
}
...
function ProtectedField({ label, value, locked }: { label: string; value: string; locked: boolean }) {
  return <div className="space-y-1.5"><span className="text-[10px] font-bold uppercase tracking-wider text-zinc-400">{label}</span><div className="flex min-h-11 items-center justify-between gap-3 rounded-xl border border-[#191921] bg-[#060608] px-3 text-xs text-zinc-500"><span className="truncate font-mono">{value}</span><span className="inline-flex shrink-0 items-center gap-1 text-[9px] uppercase tracking-wider text-zinc-600"><LockKeyhole size={11} />{locked ? 'Protegido' : 'Aguardando homologação'}</span></div></div>
}
