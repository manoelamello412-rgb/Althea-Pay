    ...
    <div className="w-full space-y-5 text-left text-white">
      ...
      <form onSubmit={handleSave} className="space-y-4">
        <section className="space-y-4 rounded-2xl border border-white/[.055] bg-[var(--althea-surface)] p-4">
          <div><span className="text-[10px] font-bold uppercase tracking-wider text-[var(--althea-brand)]">Identidade</span><p className="mt-1 text-[10px] text-zinc-500">Defina como a assistente será apresentada na operação.</p></div>
          <label className="block space-y-1.5"><span className="text-[10px] font-bold uppercase tracking-wider text-zinc-400">Nome de exibição</span><input type="text" maxLength={40} value={settings.name} onChange={(event) => setSettings((current) => ({ ...current, name: event.target.value }))} className="w-full rounded-xl border border-white/[.045] bg-[var(--althea-bg)] p-3 text-xs text-white outline-none focus:border-[rgba(29,184,84,.32)]" /></label>
        </section>

        <section className="space-y-4 rounded-2xl border border-white/[.055] bg-[var(--althea-surface)] p-4">
          <div><span className="text-[10px] font-bold uppercase tracking-wider text-[var(--althea-brand)]">Contexto Comercial & Tom</span><p className="mt-1 text-[10px] text-zinc-500">Dados armazenados no banco da operação.</p></div>
          ...
          <label className="block space-y-1.5"><span className="text-[10px] font-bold uppercase tracking-wider text-zinc-400">Tom de voz</span><select value={settings.tone} onChange={(event) => setSettings((current) => ({ ...current, tone: event.target.value }))} className="w-full rounded-xl border border-white/[.045] bg-[var(--althea-bg)] p-3 text-xs text-white outline-none focus:border-[rgba(29,184,84,.32)]"><option value="prestativo">Amigável e Prestativo</option><option value="analitico">Analítico e Focado em Métricas</option><option value="agressivo-vendas">Persuasivo e Focado em Conversão</option></select></label>
          <div className="rounded-xl border border-white/[.045] bg-[var(--althea-bg)] px-3 py-2.5 text-[10px] text-zinc-500">Tom selecionado: <span className="font-semibold text-zinc-300">{toneLabel}</span></div>
        </section>
        ...
        <section className="space-y-4 rounded-2xl border border-white/[.055] bg-[var(--althea-surface)] p-4">
          ...
          <label className="flex cursor-pointer items-center justify-between gap-3 rounded-xl border border-white/[.045] bg-[var(--althea-bg)] p-3"><span className="flex items-center gap-3"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#0F1A16] text-[var(--althea-brand)]"><Globe2 size={15} /></span><span><span className="block text-xs font-bold text-zinc-200">Busca externa permitida</span><span className="block text-[10px] text-zinc-500">Controla apenas a preferência registrada para a assistente.</span></span></span><input type="checkbox" className="h-4 w-4" checked={settings.allowWeb} onChange={(event) => setSettings((current) => ({ ...current, allowWeb: event.target.checked }))} /></label>
