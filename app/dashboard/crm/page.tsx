...
export default function CRMPage(){
 ...
 useEffect(() => {
   ...
   const channel = supabase.channel(`crm:${uid}`)
   channel.on('postgres_changes',{event:'*',schema:'public',table:'crm_conversations',filter:`user_id=eq.${uid}`},p => {
     if (disposed) return
     if (p.eventType === 'DELETE' && selectedIdRef.current === String(p.old.id)) setSelectedId(null)
     refresh()
   })
   channel.on('postgres_changes',{event:'*',schema:'public',table:'crm_messages',filter:`user_id=eq.${uid}`},p => {
     if (disposed) return
   ...
   channel.on('postgres_changes',{event:'*',schema:'public',table:'crm_webhook_events',filter:`user_id=eq.${uid}`},p => {
     if (disposed) return
   ...
   return () => {
     disposed=true;clearTimeout(timer);requests.current.invalidate('reconcile')
