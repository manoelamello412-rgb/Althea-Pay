...
export default function ProductsPage() {
  ...
  return <main className="space-y-7">
    ...
    <section className="overflow-hidden rounded-2xl border border-white/[0.08] bg-[#090b0a]">
      ...
      {loading ? <div className="px-5 py-14 text-center text-sm text-zinc-500">Carregando catálogo...</div> : products.length === 0 ? <div className="px-5 py-14 text-center"><Package className="mx-auto h-9 w-9 text-zinc-700" /><p className="mt-3 text-sm text-zinc-400">Nenhum produto cadastrado.</p><button type="button" onClick={openCreate} className="mt-4 text-sm font-medium text-emerald-400 hover:text-emerald-300">Criar o primeiro produto</button></div> : <div className="divide-y divide-white/[0.06]">{products.map(product => <article key={product.id} className="flex flex-col gap-4 px-5 py-5 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="truncate text-base font-semibold text-white">{product.name}</h3><span className={`rounded-full border px-2 py-0.5 text-[10px] ${product.status === 'active' ? 'border-emerald-400/20 text-emerald-300' : product.status === 'archived' ? 'border-zinc-500/20 text-zinc-500' : 'border-amber-400/20 text-amber-300'}`}>{product.status}</span></div><p className="mt-1 text-xs text-zinc-500">{product.slug} {product.sku ? `• SKU ${product.sku}` : ''}</p></div><div className="flex flex-wrap items-center justify-end gap-3"><div className="text-right"><p className="text-sm font-semibold text-zinc-100">{money(Number(product.unit_amount), product.currency)}</p><p className="text-[10px] text-zinc-500">{product.billing_type === 'subscription' ? `recorrente / ${product.billing_interval}` : product.billing_type === 'free' ? 'gratuito' : 'pagamento único'}</p></div><div className="flex flex-wrap gap-1">{product.status !== 'archived' && <button type="button" onClick={() => openEdit(product)} disabled={statusSaving === product.id} className="rounded-lg px-3 py-2 text-xs text-zinc-300 hover:bg-white/[0.04] disabled:opacity-30">Editar</button>}{product.status === 'draft' && <button type="button" onClick={() => void setStatus(product,'active')} disabled={statusSaving === product.id} className="inline-flex items-center gap-1 rounded-lg px-3 py-2 text-xs text-emerald-300 hover:bg-emerald-400/[0.06] disabled:opacity-40"><Play className="h-3.5 w-3.5" />Ativar</button>}{product.status === 'active' && <button type="button" onClick={() => void setStatus(product,'paused')} disabled={statusSaving === product.id} className="inline-flex items-center gap-1 rounded-lg px-3 py-2 text-xs text-amber-300 hover:bg-amber-400/[0.06] disabled:opacity-40"><Pause className="h-3.5 w-3.5" />Pausar</button>}{product.status === 'paused' && <button type="button" onClick={() => void setStatus(product,'active')} disabled={statusSaving === product.id} className="inline-flex items-center gap-1 rounded-lg px-3 py-2 text-xs text-emerald-300 hover:bg-emerald-400/[0.06] disabled:opacity-40"><Play className="h-3.5 w-3.5" />Reativar</button>}{product.status === 'archived' ? <button type="button" onClick={() => void restore(product)} className="inline-flex items-center gap-1 rounded-lg px-3 py-2 text-xs text-emerald-300 hover:bg-emerald-400/[0.06]"><RotateCcw className="h-3.5 w-3.5" />Restaurar</button> : <button type="button" onClick={() => void archive(product)} disabled={statusSaving === product.id} className="inline-flex items-center gap-1 rounded-lg px-3 py-2 text-xs text-rose-300 hover:bg-rose-400/[0.06] disabled:opacity-40"><Archive className="h-3.5 w-3.5" />Arquivar</button>}</div></div></article>)}</div>}
    </section>
  </main>
}
