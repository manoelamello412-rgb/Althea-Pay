...
import { Archive, Check, Package, Pause, Play, Plus, RefreshCw, RotateCcw, Save, X } from 'lucide-react'
...
export default function ProductsPage() {
  ...
  const setStatus = async (product: Product, status: 'draft'|'active'|'paused') => {
    ...
    setStatusSaving(product.id); setError(''); setNotice('')
    const { error: rpcError } = await db.rpc('set_product_status', { p_product_id:product.id, p_version:product.version, p_status:status })
    if (rpcError) setError(rpcError.message); else { setNotice(`Produto ${status === 'active' ? 'ativado' : status === 'paused' ? 'pausado' : 'retornado para rascunho'}.`); await load() }
  ...
  const archive = async (product: Product) => {
    ...
    else { setNotice('Produto arquivado.'); setArchiveTarget(null); await load() }
    setStatusSaving(null)
  }
  ...
  return <main className="space-y-7">
    ...
    <section className="overflow-hidden rounded-2xl border border-white/[0.08] bg-[#090b0a]">
      <div className="border-b border-white/[0.06] px-5 py-4"><p className="text-[10px] font-bold tracking-[0.18em] text-zinc-500">CATÁLOGO</p><p className="mt-1 text-sm text-zinc-300">{products.length} produto{products.length === 1 ? '' : 's'} no catálogo</p></div>
      {loading ? <div className="px-5 py-14 text-center text-sm text-zinc-500">Carregando catálogo...</div> : products.length === 0 ? <div className="px-5 py-14 text-center"><Package className="mx-auto h-9 w-9 text-zinc-700" /><p className="mt-3 text-sm text-zinc-400">Nenhum produto cadastrado.</p><button type="button" onClick={openCreate} className="mt-4 text-sm font-medium text-emerald-400 hover:text-emerald-300">Criar o primeiro produto</button></div> : <div className="divide-y divide-white/[0.06]">{products.map(product => <article key={product.id} className="flex flex-col gap-4 px-5 py-5 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="truncate text-base font-semibold text-white">{product.name}</h3><span className={`rounded-full border px-2 py-0.5 text-[10px] ${product.status === 'active' ? 'border-emerald-400/20 text-emerald-300' : product.status === 'archived' ? 'border-zinc-500/20 text-zinc-500' : 'border-amber-400/20 text-amber-300'}`}>{product.status}</span></div><p className="mt-1 text-xs text-zinc-500">{product.slug} {product.sku ? `• SKU ${product.sku}` : ''}</p></div><div className="flex flex-wrap items-center justify-end gap-3"><div className="text-right"><p className="text-sm font-semibold text-zinc-100">{money(Number(product.unit_amount), product.currency)}</p><p className="text-[10px] text-zinc-500">{product.billing_type === 'subscription' ? `recorrente / ${product.billing_interval}` : product.billing_type === 'free' ? 'gratuito' : 'pagamento único'}</p></div><div className="flex flex-wrap gap-1">{product.status !== 'archived' && <button type="button" onClick={() => openEdit(product)} disabled={statusSaving === product.id} className="rounded-lg px-3 py-2 text-xs text-zinc-300 hover:bg-white/[0.04] disabled:opacity-30">Editar</button>}{product.status === 'draft' && <button type="button" onClick={() => void setStatus(product,'active')} disabled={statusSaving === product.id} className="inline-flex items-center gap-1 rounded-lg px-3 py-2 text-xs text-emerald-300 hover:bg-emerald-400/[0.06] disabled:opacity-40"><Play className="h-3.5 w-3.5" />Ativar</button>}{product.status === 'active' && <button type="button" onClick={() => void setStatus(product,'paused')} disabled={statusSaving === product.id} className="inline-flex items-center gap-1 rounded-lg px-3 py-2 text-xs text-amber-300 hover:bg-amber-400/[0.06] disabled:opacity-40"><Pause className="h-3.5 w-3.5" />Pausar</button>}{product.status === 'paused' && <button type="button" onClick={() => void setStatus(product,'active')} disabled={statusSaving === product.id} className="inline-flex items-center gap-1 rounded-lg px-3 py-2 text-xs text-emerald-300 hover:bg-emerald-400/[0.06] disabled:opacity-40"><Play className="h-3.5 w-3.5" />Reativar</button>}{product.status === 'archived' ? <button type="button" onClick={() => void restore(product)} className="inline-flex items-center gap-1 rounded-lg px-3 py-2 text-xs text-emerald-300 hover:bg-emerald-400/[0.06]"><RotateCcw className="h-3.5 w-3.5" />Restaurar</button> : <button type="button" onClick={() => setArchiveTarget(product)} disabled={statusSaving === product.id} className="inline-flex items-center gap-1 rounded-lg px-3 py-2 text-xs text-rose-300 hover:bg-rose-400/[0.06] disabled:opacity-40"><Archive className="h-3.5 w-3.5" />Arquivar</button>}</div></div></article>)}</div>}
    </section>
    {archiveTarget && <div className="fixed inset-0 z-[90] grid place-items-center bg-black/70 px-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="archive-product-title"><div className="w-full max-w-md rounded-2xl border border-white/[0.1] bg-[#0a0d0b] p-5 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-rose-300">Arquivar produto</p><h2 id="archive-product-title" className="mt-2 text-lg font-semibold text-white">{archiveTarget.name}</h2><p className="mt-2 text-sm leading-6 text-zinc-400">O produto sairá das opções ativas de venda, mas o histórico será preservado e ele poderá ser restaurado depois.</p></div><button type="button" onClick={() => setArchiveTarget(null)} disabled={statusSaving === archiveTarget.id} className="grid h-9 w-9 shrink-0 place-items-center rounded-lg text-zinc-500 hover:bg-white/[0.04] hover:text-white" aria-label="Fechar confirmação"><X className="h-5 w-5" /></button></div><div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={() => setArchiveTarget(null)} disabled={statusSaving === archiveTarget.id} className="min-h-11 rounded-xl border border-white/[0.08] px-4 text-sm text-zinc-300 disabled:opacity-50">Cancelar</button><button type="button" onClick={() => void archive(archiveTarget)} disabled={statusSaving === archiveTarget.id} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-rose-500 px-4 text-sm font-semibold text-white disabled:opacity-50"><Archive className="h-4 w-4" />{statusSaving === archiveTarget.id ? 'Arquivando...' : 'Confirmar arquivamento'}</button></div></div></div>}
  </main>
}
