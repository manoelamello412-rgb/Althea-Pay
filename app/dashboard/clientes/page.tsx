...
export default function ClientesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const result = await db.from('clients').select('id,data,created_at,user_id').eq('user_id', auth.user.id).order('created_at', { ascending: false }).limit(5000)
  ...
  return <main className="min-h-screen bg-[#070A09] px-4 py-6 text-slate-100 lg:px-8">
    <div className="mx-auto max-w-[1700px]">
      ...
      <section className="overflow-hidden rounded-2xl border border-white/10 bg-white/[.02]"><div className="hidden grid-cols-[minmax(240px,1.4fr)_minmax(220px,1fr)_180px_120px] border-b border-white/10 px-5 py-3 text-[10px] font-black uppercase tracking-widest text-slate-600 lg:grid"><span>Cliente</span><span>Contato</span><span>Telefone</span><span>Cadastro</span></div>{loading ? <div className="flex min-h-56 flex-col items-center justify-center gap-2 text-center"><RefreshCw size={22} className="animate-spin text-emerald-400" /><strong>Carregando clientes</strong><p className="text-sm text-slate-600">Sincronizando dados reais.</p></div> : error ? <div className="flex min-h-56 flex-col items-center justify-center gap-3 text-center"><X size={22} className="text-red-400" /><strong>Falha na sincronização</strong><p className="text-sm text-slate-500">{error}</p><button type="button" onClick={() => void load()} className="rounded-lg border border-white/10 px-4 py-2 text-sm">Tentar novamente</button></div> : rows.length === 0 ? <div className="flex min-h-56 flex-col items-center justify-center gap-2 text-center"><Users size={24} className="text-slate-600" /><strong>Nenhum cliente encontrado</strong><p className="text-sm text-slate-600">{clients.length ? 'Ajuste a busca para encontrar o cliente.' : 'Os clientes aparecerão aqui quando forem recebidos pelos seus fluxos.'}</p></div> : <div>{rows.map(client => <button key={client.id} type="button" onClick={() => setSelected(client)} className="grid w-full grid-cols-1 gap-2 border-b border-white/[.06] px-5 py-4 text-left transition hover:bg-white/[.035] lg:grid-cols-[minmax(240px,1.4fr)_minmax(220px,1fr)_180px_120px] lg:items-center"><span className="min-w-0"><b className="block truncate text-sm text-slate-100">{nameOf(client)}</b><small className="block truncate pt-1 text-[11px] text-slate-600">{client.id}</small></span><span className="truncate text-sm text-slate-400">{emailOf(client) || '—'}</span><span className="text-sm text-slate-400">{phoneOf(client) || '—'}</span><span className="text-xs text-slate-500">{date(client.created_at)}</span></button>)}</div>}</section>
    </div>
