...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  for (const rule of matched) {
    if (rule.action_type === 'automated_whatsapp_dispatch') {
      ...
      if (!webhookUrl) { results.push({ rule_id: rule.id, action: rule.action_type, dispatched: false, detail: 'WHATSAPP_DISPATCH_URL_NOT_CONFIGURED' }); continue }
      const controller = new AbortController()
      const timer = setTimeout(() => controller.abort(), 3500)
      try {
        const response = await fetch(webhookUrl, { method: 'POST', signal: controller.signal, headers: { 'Content-Type': 'application/json', 'X-Althea-Event': event.id }, body: JSON.stringify({ to: event.buyer_email, buyer_name: event.buyer_name, transaction_id: event.transaction_id, dynamic_discount_link: rule.action_config.dynamic_discount_link ?? null, trigger_rule_id: rule.id }) })
      ...
      if (!reverseUrl) { results.push({ rule_id: rule.id, action: rule.action_type, dispatched: false, detail: 'reverse_webhook_url_missing' }); continue }
      const response = await fetch(reverseUrl, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Althea-Event': event.id }, body: JSON.stringify({ event, rule }) }).catch(() => null)
      results.push({ rule_id: rule.id, action: rule.action_type, dispatched: response?.ok === true, detail: response ? `provider_http_${response.status}` : 'dispatch_failed' })
    }
  }
