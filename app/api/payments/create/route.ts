...
export async function POST(request: Request) {
  try {
    ...
    const adapterPayload = { operation: 'create_payment', gateway_id: selectedRoute.gateway_id, environment: 'production', transaction_id: pendingTransaction.id, amount: pendingTransaction.amount, currency: pendingTransaction.currency, payment_method: { type: paymentMethod, details: paymentDetails ?? {} }, customer: session.customer ?? {}, metadata: { ...metadata, ...(session.metadata && typeof session.metadata === 'object' ? session.metadata : {}) }, idempotency_key: `payment:${idempotencyKey}`, funnel_id: session.funnel_id, product_id: session.product_id }
    ...
    try {
      adapterResponse = await fetch(`${supabaseUrl.replace(/\/$/, '')}/functions/v1/gateway-provider-adapter`, { method: 'POST', headers: { 'content-type': 'application/json', 'x-althea-internal-secret': internalSecret, 'x-althea-gateway-id': selectedRoute.gateway_id }, body: JSON.stringify(adapterPayload), signal: controller.signal, cache: 'no-store' })
    } catch (error) {
      clearTimeout(timeout); console.error('[ALTHEA-PAYMENTS-ADAPTER]', error)
      const { error: attemptError } = await admin.rpc('allocate_and_insert_gateway_payment_attempt', { p_user_id: session.user_id, p_transaction_id: pendingTransaction.id, p_gateway_id: selectedRoute.gateway_id, p_gateway_name: text(selectedRoute.gateway_id), p_idempotency_key: `attempt:${idempotencyKey}`, p_status: 'unknown', p_failure_class: 'timeout', p_external_transaction_id: null, p_routing_rule_id: selectedRoute.route_id ?? null, p_routing_policy_id: null, p_routing_policy_version: null, p_decision_reason: selectedRoute.decision_reason ?? 'configured_route', p_provider_request_id: null, p_duration_ms: ADAPTER_TIMEOUT_MS, p_sale_id: null, p_product_id: session.product_id })
    ...
    const { error: attemptError } = await admin.rpc('allocate_and_insert_gateway_payment_attempt', { p_user_id: session.user_id, p_transaction_id: pendingTransaction.id, p_gateway_id: selectedRoute.gateway_id, p_gateway_name: text(selectedRoute.gateway_id), p_idempotency_key: `attempt:${idempotencyKey}`, p_status: attemptStatus, p_failure_class: attemptStatus === 'declined' ? 'declined' : attemptStatus === 'unknown' ? 'technical' : null, p_external_transaction_id: providerExternalId || null, p_routing_rule_id: selectedRoute.route_id ?? null, p_routing_policy_id: null, p_routing_policy_version: null, p_decision_reason: selectedRoute.decision_reason ?? 'configured_route', p_provider_request_id: providerRequestId || null, p_duration_ms: durationMs, p_sale_id: null, p_product_id: session.product_id })
