...
export async function POST(request: Request) {
  try {
    ...
    const { data: session, error: sessionError } = await admin
      ...
      .select('id,user_id,organization_id,funnel_id,product_id,amount,currency,customer,metadata,status,idempotency_key')
    ...
    if (transactionError || !transaction) {
      console.error('[ALTHEA-PAYMENTS-CREATE]', transactionError)
      return NextResponse.json({ ok: false, code: safeErrorCode(transactionError) }, { status: 500 })
    }

    if (TERMINAL_TRANSACTION_STATES.has(text(transaction.status))) {
      return NextResponse.json({ ok: true, transaction: { id: transaction.id, status: transaction.status, amount: transaction.amount, currency: transaction.currency, gateway_id: transaction.gateway_id }, action: null, next: 'complete' })
    }
    if (text(transaction.status) === 'pending') {
      return NextResponse.json({ ok: true, transaction: { id: transaction.id, status: transaction.status, amount: transaction.amount, currency: transaction.currency, gateway_id: transaction.gateway_id }, action: null, next: 'awaiting_gateway' })
    }
    ...
    const { data: binding, error: bindingError } = await admin
      ...
      .eq('status', 'active')
      .maybeSingle()
    if (bindingError || !binding?.gateway_id) {
      console.error('[ALTHEA-PAYMENTS-ROUTE]', bindingError)
      return NextResponse.json({ ok: false, code: 'NO_PRIMARY_GATEWAY_ROUTE' }, { status: 409 })
    }

    const gateway = Array.isArray(binding.gateways) ? binding.gateways[0] : binding.gateways
    if (!gateway || gateway.organization_id !== session.organization_id || !['active','connected','degraded'].includes(text(gateway.status).toLowerCase())) {
      return NextResponse.json({ ok: false, code: 'GATEWAY_NOT_OPERATIONAL' }, { status: 409 })
    }

    const selectedGatewayId = text(binding.gateway_id)
    const expectedVersion = Number(transaction.version)
    const { data: boundTransaction, error: bindError } = await admin.rpc('bind_gateway_transaction_gateway', {
      p_transaction_id: transaction.id,
      p_user_id: session.user_id,
      p_gateway_id: selectedGatewayId,
      p_expected_version: Number.isFinite(expectedVersion) ? expectedVersion : null,
    })
    if (bindError || !boundTransaction) {
      console.error('[ALTHEA-PAYMENTS-BIND]', bindError)
      return NextResponse.json({ ok: false, code: 'GATEWAY_BIND_FAILED' }, { status: 409 })
    }

    const { data: pendingTransaction, error: pendingError } = await admin.rpc('transition_gateway_transaction_status', {
      p_transaction_id: boundTransaction.id,
      p_user_id: session.user_id,
      p_next_status: 'pending',
      p_failure_code: null,
      p_external_id: null,
      p_expected_version: Number(boundTransaction.version),
    })
    if (pendingError || !pendingTransaction) {
      console.error('[ALTHEA-PAYMENTS-PENDING]', pendingError)
      return NextResponse.json({ ok: false, code: 'PAYMENT_STATE_UPDATE_FAILED' }, { status: 500 })
    }
    ...
    if (!internalSecret || !supabaseUrl) {
      console.error('[ALTHEA-PAYMENTS-CONFIG] gateway execution secret/url is not configured')
      return NextResponse.json({ ok: false, code: 'PAYMENT_PROCESSOR_NOT_CONFIGURED' }, { status: 503 })
    }

    const adapterPayload = {
      operation: 'create_payment',
      gateway_id: selectedGatewayId,
      environment: text(gateway.environment) || 'production',
      transaction_id: pendingTransaction.id,
      amount: pendingTransaction.amount,
      currency: pendingTransaction.currency,
      payment_method: { type: paymentMethod, details: paymentDetails ?? {} },
      customer: session.customer ?? {},
      metadata: { ...metadata, ...(session.metadata && typeof session.metadata === 'object' ? session.metadata : {}) },
    ...
    try {
      adapterResponse = await fetch(`${supabaseUrl.replace(/\/$/, '')}/functions/v1/gateway-provider-adapter`, {
        ...
        headers: {
          ...
          'x-althea-gateway-id': selectedGatewayId,
        },
      ...
      console.error('[ALTHEA-PAYMENTS-ADAPTER]', error)
      const { error: attemptError } = await admin.rpc('allocate_and_insert_gateway_payment_attempt', {
        ...
        p_gateway_id: selectedGatewayId,
        p_gateway_name: selectedGatewayId,
        p_idempotency_key: `attempt:${idempotencyKey}`,
        p_status: 'unknown',
        p_failure_class: 'timeout',
        p_external_transaction_id: null,
        p_routing_rule_id: null,
        p_routing_policy_id: null,
        p_routing_policy_version: null,
        p_decision_reason: 'funnel_primary_binding',
        p_provider_request_id: null,
        p_duration_ms: ADAPTER_TIMEOUT_MS,
        p_sale_id: null,
        p_product_id: session.product_id,
      })
    ...
    const action = normalizePaymentAction(adapterPayloadResult, paymentMethod)
    let attemptStatus: 'approved' | 'declined' | 'pending' | 'processing' | 'unknown' = 'unknown'
    ...
    const { error: attemptError } = await admin.rpc('allocate_and_insert_gateway_payment_attempt', {
      ...
      p_failure_class: attemptStatus === 'declined' ? 'declined' : attemptStatus === 'unknown' ? 'technical' : null,
      p_external_transaction_id: providerExternalId || null,
      p_routing_rule_id: null,
      p_routing_policy_id: null,
      p_routing_policy_version: null,
      p_decision_reason: 'funnel_primary_binding',
    ...
    if (attemptError) {
      console.error('[ALTHEA-PAYMENTS-ATTEMPT]', attemptError)
      return NextResponse.json({ ok: false, code: 'PAYMENT_ATTEMPT_PERSIST_FAILED', transaction_id: pendingTransaction.id }, { status: 500 })
    }

    const { data: finalTransaction, error: transitionError } = await admin.rpc('transition_gateway_transaction_status', {
      p_transaction_id: pendingTransaction.id,
      p_user_id: session.user_id,
      p_next_status: transactionStatus,
      p_failure_code: attemptStatus === 'declined' || attemptStatus === 'unknown' ? text(adapterPayloadResult.failure_code ?? adapterPayloadResult.error) || 'GATEWAY_EXECUTION_PENDING_OR_FAILED' : null,
      p_external_id: providerExternalId || null,
      p_expected_version: Number(pendingTransaction.version),
    })
    if (transitionError || !finalTransaction) {
      console.error('[ALTHEA-PAYMENTS-TRANSITION]', transitionError)
      return NextResponse.json({ ok: false, code: 'PAYMENT_STATE_UPDATE_FAILED', transaction_id: pendingTransaction.id }, { status: 500 })
    }
    ...
    return NextResponse.json({
      ...
      transaction: {
        ...
        gateway_id: finalTransaction.gateway_id,
        external_id: finalTransaction.external_id,
      },
      gateway: { id: selectedGatewayId, provider: gateway.provider, provider_status: adapterPayloadResult.provider_status ?? null },
      action,
      next: transactionStatus === 'approved' ? 'complete' : transactionStatus === 'pending' ? 'awaiting_gateway' : 'manual_retry_required',
    }, { status: 200 })
  } catch (error) {
    console.error('[ALTHEA-PAYMENTS-CREATE]', error)
    return NextResponse.json({ ok: false, code: 'PAYMENT_PROCESSING_FAILED' }, { status: 500 })
  }
}
