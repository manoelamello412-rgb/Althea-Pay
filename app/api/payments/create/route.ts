...
export async function POST(request: Request) {
  try {
    ...
    // Canonical route: the funnel's active primary binding is authoritative.
    // No provider is selected or inferred by the public checkout request.
    ...
    if (!gateway || gateway.organization_id !== session.organization_id || gateway.status !== 'active') return NextResponse.json({ ok: false, code: 'GATEWAY_NOT_OPERATIONAL' }, { status: 409 })
