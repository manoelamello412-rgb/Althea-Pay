...
export default function DashboardPage(){
  ...
  async function loadAll(){
    ...
    const first=[f,p,g,s,c,ch,tx,conn,events].find(x=>x.error)
  ...
  const descriptions:Record<Module,string>={'Visão geral':'Seu centro de comando financeiro, conectado aos dados reais da operação.','Funis':'Crie, acompanhe e remova seus funis conectados.','Produtos':'Gerencie os produtos associados à sua operação.','Gateways':'Cadastre e acompanhe configurações de gateways.','Vendas':'Visualize e gerencie registros de vendas.','Clientes':'Centralize os registros dos seus clientes.','Chats':'Acompanhe os registros de atendimento e conversas.','Analytics':'Indicadores calculados diretamente dos dados disponíveis.','Integrações':'Base de integrações, eventos e webhooks da operação.','Configurações':'Perfil, conta e identidade visual da Althea Pay.'}

  const now=Date.now();const start=now-period*86400000
  const periodTx=transactions.filter(t=>new Date(t.created_at||0).getTime()>=start)
  const approved=periodTx.filter(t=>['approved','paid','completed','success','succeeded'].includes(String(t.status||'').toLowerCase()))
  const pending=periodTx.filter(t=>['pending','created','processing'].includes(String(t.status||'').toLowerCase()))
  const refunded=periodTx.filter(t=>['refunded','chargeback'].includes(String(t.status||'').toLowerCase()))
  ...
  const chart=Array.from({length:period},(_,i)=>{const day=new Date();day.setHours(0,0,0,0);day.setDate(day.getDate()-(period-1-i));const next=new Date(day);next.setDate(day.getDate()+1);const value=approved.filter(t=>{const d=new Date(t.created_at||0);return d>=day&&d<next}).reduce((sum,t)=>sum+(Number(t.amount)||0),0);return {label:period===7?day.toLocaleDateString('pt-BR',{weekday:'short'}).replace('.',''):day.getDate().toString().padStart(2,'0'),value}})
  const maxChart=Math.max(...chart.map(x=>x.value),1)
  const chartPoints=chart.map((x,i)=>`${i*(100/(Math.max(chart.length-1,1)))},${100-(x.value/maxChart)*82-8}`).join(' ')

  return <main className="althea-app">
    <aside className="althea-sidebar">
      <div className="app-brand"><img src="/althea-leaf.svg" alt="Althea Pay"/><div><div className="app-brand-name">ALTHEA</div><div className="app-brand-pay">PAY</div></div><small className="app-brand-tag">CONSTRUA SUAS RAÍZES FINANCEIRAS.</small></div>
    ...
    <section className="althea-main">
      ...
      {active==='Visão geral'&&<>
        <section className="dashboard-hero-row">
          <div className="revenue-card althea-card">
            <div className="revenue-top"><div><span>FATURAMENTO</span><small>Últimos {period} dias · vendas aprovadas</small></div><div className="period-switch"><button className={period===7?'selected':''} onClick={()=>setPeriod(7)}>7D</button><button className={period===30?'selected':''} onClick={()=>setPeriod(30)}>30D</button></div></div>
            <strong>{money.format(revenue)}</strong><div className="revenue-meta"><span className="trend">● Dados reais da operação</span><span>Ticket médio {money.format(average)}</span></div>
            <div className="chart-wrap"><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-label="Faturamento por dia"><defs><linearGradient id="area" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stopColor="#1DB854" stopOpacity=".30"/><stop offset="1" stopColor="#1DB854" stopOpacity="0"/></linearGradient></defs><polyline points={`0,92 ${chartPoints} 100,92`} fill="url(#area)" stroke="none"/><polyline points={chartPoints} fill="none" stroke="#1DB854" strokeWidth="1.7" vectorEffect="non-scaling-stroke" strokeLinecap="round" strokeLinejoin="round"/></svg><div className="chart-labels">{chart.filter((_,i)=>period===7||i%5===0||i===period-1).map((x,i)=><span key={`${x.label}-${i}`}>{x.label}</span>)}</div></div>
          </div>
          <div className="quick-panel althea-card"><div className="panel-heading"><div><span>OPERAÇÃO</span><h2>Status agora</h2></div><span className="live-dot">LIVE</span></div><div className="health-list"><div><i className="health-good"/><span>Banco de dados</span><strong>Online</strong></div><div><i className={connectedFunnels?'health-good':'health-muted'}/><span>Funis conectados</span><strong>{connectedFunnels}</strong></div><div><i className={counts.gateways?'health-good':'health-muted'}/><span>Gateways cadastrados</span><strong>{counts.gateways||0}</strong></div><div><i className={integrationEvents?'health-good':'health-muted'}/><span>Eventos recentes</span><strong>{integrationEvents}</strong></div></div><button className="ghost-action" onClick={()=>setActive('Integrações')}>Ver central de integrações →</button></div>
        </section>

        <section className="dashboard-kpis">
          <Metric title="Vendas aprovadas" value={approved.length.toLocaleString('pt-BR')} note={`${approvalRate.toFixed(1).replace('.',',')}% das transações`} icon="✓" tone="green"/>
          <Metric title="Pendentes" value={pending.length.toLocaleString('pt-BR')} note="Aguardando confirmação" icon="◷" tone="gold"/>
          <Metric title="Reembolsos / chargebacks" value={refunded.length.toLocaleString('pt-BR')} note="No período selecionado" icon="↩" tone="red"/>
          <Metric title="Ticket médio" value={money.format(average)} note="Entre vendas aprovadas" icon="◆" tone="silver"/>
        </section>

        <section className="dashboard-lower-grid">
          <div className="althea-card dashboard-table-card"><div className="panel-heading"><div><span>TRANSAÇÕES</span><h2>Últimas movimentações</h2></div><button className="text-action" onClick={()=>setActive('Vendas')}>Ver todas →</button></div>{transactions.slice(0,7).length===0?<div className="empty-state"><strong>A operação ainda não recebeu transações</strong><p>Assim que um gateway registrar uma transação, ela aparecerá aqui automaticamente.</p></div>:<div className="transaction-list">{transactions.slice(0,7).map(tx=><div className="transaction-row" key={tx.id}><div className="transaction-avatar">{customerName(tx.customer).slice(0,1).toUpperCase()}</div><div className="transaction-main"><strong>{customerName(tx.customer)}</strong><small>{tx.gateway_id||'Gateway não informado'} · {tx.created_at?dateTime.format(new Date(tx.created_at)):'agora'}</small></div><div className="transaction-amount"><strong>{money.format(Number(tx.amount)||0)}</strong><span className={`status ${statusClass(tx.status)}`}>{statusLabel(tx.status)}</span></div></div>)}</div>}</div>
          <div className="althea-card funnel-health-card"><div className="panel-heading"><div><span>FUNIS</span><h2>Performance da operação</h2></div><button className="text-action" onClick={()=>setActive('Funis')}>Gerenciar →</button></div>{funnels.length===0?<div className="empty-state"><strong>Nenhum funil criado</strong><p>Crie seu primeiro funil e acompanhe a operação por aqui.</p><button className="primary small" onClick={()=>setActive('Funis')}>+ Criar funil</button></div>:<div className="funnel-mini-list">{funnels.slice(0,5).map(f=><div className="funnel-mini" key={f.id}><div className="funnel-mini-icon">⌁</div><div><strong>{String(f.nome)}</strong><small>{String(f.url||'URL não configurada')}</small></div><span className={`althea-pill ${String(f.status)==='active'?'pill-active':''}`}>{String(f.status||'draft')}</span></div>)}</div>}</div>
        </section>
      </>}
      ...
      {active==='Integrações'&&<section className="althea-card althea-panel"><div className="panel-heading"><div><span>INTEGRAÇÕES</span><h2>Conectividade</h2></div></div><div className="empty-state"><strong>Tempo real habilitado</strong><p>O painel acompanha transações, vendas, funis e conexões por eventos do Supabase Realtime. Credenciais secretas permanecem no servidor.</p></div></section>}
