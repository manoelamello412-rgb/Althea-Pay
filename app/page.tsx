...
export default function DashboardPage() {
  ...
  const currentIcon = nav.find(item => item.label === active)?.icon || LayoutDashboard

  function renderOverview() {
    return (
      <>
        <section className="dashboard-kpis">
          <article className="kpi-card althea-card">
            <div className="kpi-heading"><span>FATURAMENTO</span><span className="kpi-icon"><DollarSign size={17} /></span></div>
            <strong>{money.format(revenue)}</strong>
            <small>Últimos {period} dias · aprovadas</small>
          </article>
          <article className="kpi-card althea-card">
            <div className="kpi-heading"><span>APROVAÇÃO</span><span className="kpi-icon"><Tag size={17} /></span></div>
            <strong>{approvalRate.toFixed(1)}%</strong>
            <small>{approved.length} aprovadas</small>
          </article>
          <article className="kpi-card althea-card">
            <div className="kpi-heading"><span>PENDENTES</span><span className="kpi-icon"><Clock3 size={17} /></span></div>
            <strong>{pending.length}</strong>
            <small>aguardando processamento</small>
          </article>
          <article className="kpi-card althea-card">
            <div className="kpi-heading"><span>TICKET MÉDIO</span><span className="kpi-icon"><CreditCard size={17} /></span></div>
            <strong>{money.format(average)}</strong>
            <small>{refunded.length} reembolsos/chargebacks</small>
          </article>
        </section>
        ...
        <section className="dashboard-hero-row">
          <div className="revenue-card althea-card">
            <div className="revenue-top">
              <div><span>VISÃO DA OPERAÇÃO</span><small>Dados reais sincronizados com o banco</small></div>
              <div className="period-switch"><button className={period === 7 ? 'selected' : ''} onClick={() => setPeriod(7)}>7D</button><button className={period === 30 ? 'selected' : ''} onClick={() => setPeriod(30)}>30D</button></div>
            </div>
          ...
          <div className="quick-panel althea-card">
            ...
            <div className="health-list">
              <div><i className="health-good" /><span>Banco de dados</span><strong>Online</strong></div>
              <div><i className={connectedFunnels ? 'health-good' : 'health-muted'} /><span>Funis conectados</span><strong>{connectedFunnels}</strong></div>
              <div><i className={counts.gateways ? 'health-good' : 'health-muted'} /><span>Gateways cadastrados</span><strong>{counts.gateways || 0}</strong></div>
              <div><i className={integrationEvents ? 'health-good' : 'health-muted'} /><span>Eventos recentes</span><strong>{integrationEvents}</strong></div>
            </div>
          </div>
        </section>
        ...
        <section className="dashboard-lower-grid">
          <div className="dashboard-table-card althea-card">
            ...
            <div className="transaction-list">
              {loading ? Array.from({ length: 4 }).map((_, index) => <div className="transaction-row skeleton-row" key={index}><span className="skeleton skeleton-avatar" /><span className="skeleton-block"><i /><i /></span><span className="skeleton-block right"><i /><i /></span></div>) : transactions.slice(0, 5).map(transaction => {
                const customer = customerName(transaction.customer)
                return <div className="transaction-row" key={transaction.id} onClick={() => setSelectedTransaction(transaction)}><span className="transaction-avatar">{initials(customer)}</span><span className="transaction-main"><strong>{customer}</strong><small>{transaction.id} · {transaction.gateway_id || 'gateway não informado'}</small></span><span className="transaction-amount"><strong>{money.format(Number(transaction.amount) || 0)}</strong><span className={`status ${statusClass(transaction.status)}`}>{statusLabel(transaction.status)}</span></span></div>
              })}
          ...
          <div className="funnel-health-card althea-card">
            ...
            <div className="funnel-mini-list">
              {funnels.slice(0, 4).map(funnel => <div className="funnel-mini" key={funnel.id}><span className="funnel-mini-icon"><GitBranch size={15} /></span><span><strong>{rowName(funnel)}</strong><small>{funnel.url || funnel.endpoint || 'URL não configurada'}</small></span><span className="pill-active">{String(funnel.status || 'draft')}</span></div>)}
              {funnels.length === 0 && <div className="empty-state"><strong>Sem funis cadastrados.</strong><p>Crie seu primeiro funil para acompanhar conexão e eventos em tempo real.</p></div>}
            </div>
          </div>
        </section>
      </>
    ...
    return (
      <section className="sales-center">
        <div className="sales-summary">
          <article className="sales-stat althea-card"><span><DollarSign size={15} />Faturamento</span><strong>{money.format(revenue)}</strong><small>Período selecionado</small></article>
          <article className="sales-stat althea-card"><span><CheckCircle2 size={15} />Aprovadas</span><strong>{approved.length}</strong><small>Transações concluídas</small></article>
          <article className="sales-stat althea-card"><span><Clock3 size={15} />Pendentes</span><strong>{pending.length}</strong><small>Aguardando processamento</small></article>
        </div>
        <section className="sales-panel althea-card">
          <div className="sales-toolbar"><div><span className="sales-eyebrow">CENTRAL DE VENDAS</span><h2>Transações</h2><p>Busca e filtros aplicados localmente sobre os dados sincronizados.</p></div><span className="althea-status"><i /> Tempo real ativo</span></div>
          <div className="sales-filters">
            <label className="sales-search"><Search size={15} /><input value={salesQuery} onChange={event => setSalesQuery(event.target.value)} placeholder="Buscar venda, cliente, gateway..." /></label>
            <select value={salesStatus} onChange={event => setSalesStatus(event.target.value)}><option value="all">Todos os status</option><option value="ok">Aprovadas</option><option value="pending">Pendentes</option><option value="warning">Reembolsadas</option><option value="danger">Falhas</option></select>
            <select value={period} onChange={event => setPeriod(Number(event.target.value) as 7 | 30)}><option value="7">Últimos 7 dias</option><option value="30">Últimos 30 dias</option></select>
            <button className="filter-clear" onClick={() => { setSalesQuery(''); setSalesStatus('all') }}>Limpar</button>
          </div>
          <div className="sales-mobile-list">
            {loading ? Array.from({ length: 5 }).map((_, index) => <div className="sales-mobile-card skeleton-card" key={index}><span className="skeleton" /><span className="skeleton wide" /><span className="skeleton" /></div>) : sales.map(transaction => <button className="sales-mobile-card" key={transaction.id} onClick={() => setSelectedTransaction(transaction)}><span className="sales-mobile-icon"><ShoppingCart size={15} /></span><span className="sales-mobile-main"><strong>{customerName(transaction.customer)}</strong><small>{transaction.id}</small><small>{transaction.gateway_id || 'Gateway não informado'}</small></span><span className="sales-mobile-right"><strong>{money.format(Number(transaction.amount) || 0)}</strong><span className={`status ${statusClass(transaction.status)}`}>{statusLabel(transaction.status)}</span></span></button>)}
          </div>
          <div className="sales-table-wrap">
            <table className="sales-table"><thead><tr><th>Transação</th><th>Cliente</th><th>Status</th><th>Valor</th><th>Data</th></tr></thead><tbody>
              {loading ? Array.from({ length: 6 }).map((_, index) => <tr key={index}><td colSpan={5}><div className="table-skeleton"><span /><span /><span /></div></td></tr>) : sales.map(transaction => <tr key={transaction.id} onClick={() => setSelectedTransaction(transaction)}><td><strong>{transaction.id}</strong><small>{transaction.external_id || transaction.transaction_id || 'ID externo não informado'}</small></td><td><strong>{customerName(transaction.customer)}</strong><small>{transaction.funnel_id || 'Funil não informado'}</small></td><td><span className={`status ${statusClass(transaction.status)}`}>{statusLabel(transaction.status)}</span></td><td className="sales-value">{money.format(Number(transaction.amount) || 0)}</td><td>{transaction.created_at ? dateTime.format(new Date(transaction.created_at)) : '—'}</td></tr>)}
            </tbody></table>
          </div>
  ...
  function renderGeneric() {
    ...
    return (
      <section className="records-panel althea-card">
        <div className="panel-heading"><div><span>{active.toUpperCase()}</span><h2>Registros</h2></div><span className="record-count">{rows.length} registros</span></div>
        <form className="record-create" onSubmit={createGeneric}><textarea value={json} onChange={event => setJson(event.target.value)} rows={5} aria-label={`JSON de ${active}`} /><button className="primary" type="submit" disabled={saving}>{saving ? 'Salvando...' : `Adicionar ${labels[table]}`}</button></form>
        <div className="records">{rows.map(row => <div className="record" key={row.id}><div><strong>{rowName(row)}</strong><small>{row.id}{row.created_at ? ` · ${dateTime.format(new Date(row.created_at))}` : ''}</small></div><button className="danger" onClick={() => deleteRow(table, row.id)}>Excluir</button></div>)}{rows.length === 0 && <div className="empty-state"><strong>Nenhum registro encontrado.</strong><p>Os novos registros aparecerão aqui após serem gravados no banco.</p></div>}</div>
      </section>
    )
  }
    ...
    <main className="althea-app">
      ...
      <section className="althea-main">
        <header className="althea-header dashboard-header">
          <div><div className="althea-kicker">{ALTHEA_PAY.tagline}</div><div className="mobile-section-label"><currentIcon size={15} /></div><h1>{active === 'Visão geral' ? (fullName ? `Olá, ${fullName.split(' ')[0]}` : 'Visão geral') : active}</h1><p>{descriptions[active]}</p></div>
          <div className="dashboard-header-actions"><div className="althea-status"><i /> <span>{loading ? 'Sincronizando' : 'Tempo real ativo'}</span></div></div>
        </header>
