...
import { Check, Layers, Loader2, Sparkles, Webhook, X } from 'lucide-react'
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch('/api/funnels/provision', {
        ...
        cache: 'no-store',
        body: JSON.stringify({ name: name.trim(), url: url.trim() || null, connection_type: connectionType, funnel_type: funnelType }),
      })
      const body: unknown = await response.json().catch(() => ({}))
      const payload = body && typeof body === 'object' && !Array.isArray(body) ? body as Record<string, unknown> : {}
      if (!response.ok) throw new Error(typeof payload.error === 'string' ? payload.error : 'Não foi possível criar o funil.')
      ...
      const { error: seedError } = await supabase.rpc('seed_funnel_structure', { target_funnel: funnelId })
      if (seedError) throw seedError
      router.push('/dashboard/funil/central')
      router.refresh()
    } catch (cause) {
      ...
      <div className="mx-auto max-w-2xl space-y-6">
        <header className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-xs leading-relaxed text-zinc-500">O funil orquestra Produtos, Checkout, Gateways, CRM e Pagamentos existentes. Nenhuma dessas entidades é duplicada.</p>
          </div>
        ...
        <form onSubmit={create} className="space-y-5">
          ...
          <button type="submit" disabled={saving || !name.trim()} className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-500 px-4 text-xs font-bold text-black transition hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-40">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}{saving ? 'CRIANDO...' : 'CRIAR E PREPARAR JORNADA'}</button>
        </form>
      </div>
    </main>
  )
