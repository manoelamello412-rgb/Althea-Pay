...
export default function ResetPasswordPage() {
  ...
  const recoverySession = useRef<Promise<Session | null> | null>(null)
  ...
  useEffect(() => {
    ...
    let active = true
    let recovered = false
    const markReady = () => { if (active) { recovered = true; setReady(true); setError('') } }
    ...
    if (!recoverySession.current) {
      recoverySession.current = (async () => {
        // createBrowserClient detects PKCE callbacks. getSession waits for that
        // initialization; exchanging the same one-time code here would repeat it.
        const { data, error } = await supabase.auth.getSession()
        if (error) throw error
        return data.session
      })()
    }
    void recoverySession.current.then(session => {
    ...
    const timer = window.setTimeout(() => {
      if (active && !recovered) setError(current => current || 'Este link de recuperação é inválido, expirou ou já foi usado. Solicite um novo link.')
    }, 5000)
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      window.setTimeout(() => router.replace(authLink('/login', window.location.search)), 1200)
    } catch (err) {
  ...
  return <main className="auth-shell"><section className="auth-card">
    <div className="brand-lockup"><Image src="/althea-logo.png" alt="Althea Pay" width={294} height={230} priority style={{ width: 'min(100%, 294px)', height: 'auto', objectFit: 'contain', filter: 'none' }} /></div>
