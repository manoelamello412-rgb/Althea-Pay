(() => {
  ...
  async function startRealtimeCore() {
    ...
    try {
      ...
      const channelName = `althea-core-${user.id}`;
      liveChannel = sb.channel(channelName);
      tables.forEach(table => {
        liveChannel.on('postgres_changes', { event: '*', schema: 'public', table }, () => {
          const state = document.getElementById('syncState');
          if (state) state.innerHTML = '<i></i> Atualizado agora';
          clearTimeout(refreshTimer);
          refreshTimer = setTimeout(() => {
            if (typeof go === 'function' && typeof active !== 'undefined' && document.getElementById('view')) go(active);
          }, 250);
        });
      });
