...
describe('Multi-CRM reliability contracts',()=>{
 ...
 it('keeps automation governance on the physical execution graph',()=>{
  const worker=read('supabase/functions/automation-engine-v2/index.ts')
  expect(worker).toContain('automation_rules')
  expect(worker).toContain('automation_executions')
  expect(worker).toContain('automation_execution_attempts')
  expect(worker).toContain('integration_events')
