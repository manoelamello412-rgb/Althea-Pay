...
const cancel = readFileSync(resolve(root, "supabase/migrations/20260910170000_crm_advanced_automation_cancel_ownership_v1.sql"), "utf8");
...
describe("advanced automation governance", () => {
  ...
  it("uses row locking and skip-locked claiming for concurrent workers", () => {
    expect(governance).toContain("for update skip locked");
  });
  ...
  it("makes replay idempotent and auditable", () => {
    expect(replay).toContain("scope='automation_replay'");
    expect(replay).toContain("for update");
    expect(replay).toContain("automation.replay");
    expect(replay).toContain("response_code=200");
  });

  it("enforces tenant ownership on cancellation", () => {
    expect(cancel).toContain("id=p_execution_id and user_id=auth.uid()");
    expect(cancel).toContain("REVOKE ALL ON FUNCTION public.crm_cancel_automation_execution(uuid,text) FROM PUBLIC, anon");
  });

  it("does not introduce the rejected parallel workflow schema", () => {
    expect(governance).not.toContain("crm_workflows");
    expect(governance).not.toContain("crm_workflow_steps");
    expect(governance).not.toContain("crm_workflow_executions");
    expect(governance).not.toContain("crm_automation_rate_limits");
  });
});
